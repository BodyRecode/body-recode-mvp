...
<p class="note" style="margin-bottom:10px"><b>The four meaning colours are the only part of this palette chosen rather than inherited.</b> Six directions were rendered on paper, on graphite, simulated for red-green colour blindness, and shown in the list a coach actually scans, before this one was picked. <b>Attention also carries a ring, and that is a rule rather than a flourish.</b> Under red-green colour blindness, roughly one man in twelve, Remediation and Attention are both olive and separate only by lightness. On a page somebody reads that is harmless, because the level is always named in words beside the dot. On a list of forty clients that somebody scans, the dot is working alone. So Attention is a different shape as well as a different colour: a ring, which reads at eight pixels and survives any colour vision, any screen and a greyscale print. <b>The general rule: when a colour is the only thing separating two states, it is not enough. Add the shape.</b></p>
