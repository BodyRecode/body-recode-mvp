...
import {
  darkEmailShell,
  emailLogo, emailEyebrow, emailDivider, emailBody,
...
function buildHtml(firstName: string, portalLink: string): string {
  const inner = `
${emailLogo()}
${emailImage('email-hero.png', 'Body Recode product update: a few new things inside your portal')}
...
${emailImage('email-health-markers.png', 'Health Markers: every blood marker checked against your lab\'s own range, anything out of range flagged')}
...
${emailImage('email-trajectory.png', 'Your signal across a training block: where you started, the dip and recovery, and where you are now')}
...
${emailBody('<strong style="color:#1A1A1A;">A reading at the end of each block.</strong> When you finish a training block, you now get a block-end reading: a step back from the week-to-week that reads how your body moved across the whole block, not just where it sits on any one day. Where you started, what shifted, what held steady, and what it sets up next. It is the same idea as the blood work above, applied to your training: the trend over time, not the snapshot.', { bottom: 16 })}
