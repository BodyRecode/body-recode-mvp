...
async function main() {
  ...
  const cards = rows.map((p, i) => {
    ...
    return `<article><div class="w"><b>${i + 1}</b><span>${day} ${p.date}</span><span>07:00</span>
      <em style="background:${pillar[2]};color:${pillar[1]}">${pillar[0]}</em></div>
      <div class="p"><img src="file://${process.cwd()}/public${p.graphic}"></div>
...
.s b{color:#1a1a1a}
article{display:grid;grid-template-columns:110px 300px 1fr;gap:22px;background:#fff;border:1px solid #e6e3de;
...
.p img{width:300px;border-radius:8px;border:1px solid #e6e3de;display:block}
