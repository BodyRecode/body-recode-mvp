...
export default function OverrideSubscriptionButton({ clientId }: { clientId: string }) {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className="text-xs text-stone-500 hover:text-teal-400 border border-stone-800 hover:border-teal-800/50 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50 whitespace-nowrap"
    >
