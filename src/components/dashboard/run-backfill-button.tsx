...
export default function RunBackfillButton() {
  ...
  return (
    <div>
      ...
      {result && (
        <div className="mt-3 bg-[#1A1E26] br-card p-4 text-[12.5px] space-y-2">
          <div className="flex items-center gap-1.5 text-[#FAFAF8] mb-2">
            <CheckCircle2 size={13} />
            <span className="font-medium">Backfill complete</span>
            <span className="text-[#8A9099] ml-2">{new Date(result.ran_at).toLocaleTimeString('en-AU')}</span>
          </div>
          <p className="text-[#FAFAF8]">
            <span className="text-[#8A9099]">Stripe customers scanned:</span> {result.stripe_customers_seen}
            <span className="text-[#8A9099] ml-3">Matched to clients:</span> {result.matched_clients}
          </p>
          <p className="text-[#FAFAF8]">
            <span className="text-[#8A9099]">Subscriptions synced:</span> {result.subscriptions_synced}
            {Object.keys(result.subscriptions_by_status).length > 0 && (
              <span className="text-[#8A9099] ml-2">
                ({Object.entries(result.subscriptions_by_status).map(([k, v]) => `${k}: ${v}`).join(', ')})
              </span>
            )}
          </p>
          <p className="text-[#FAFAF8]">
            <span className="text-[#8A9099]">Orphan Stripe customers:</span> {result.unmatched_stripe_customers.length}
            <span className="text-[#8A9099] ml-3">Clients with no Stripe link:</span> {result.unmatched_clients.length}
          </p>
          {result.errors.length > 0 && (
            <div className="mt-2 pt-2 border-t border-[#2A2F39]">
              <p className="text-[#A96A12] font-medium mb-1">Warnings:</p>
              {result.errors.slice(0, 5).map((e, i) => (
                <p key={i} className="text-[#8A9099] text-[11px]">• {e}</p>
              ))}
            </div>
          )}
        </div>
