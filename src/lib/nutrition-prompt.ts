...
export function buildNutritionUserPrompt(
  ...
  if (medications) {
    ...
    lines.push('  (b) No single meal may exceed 40g protein. Distribute the anchor across more meals if needed.')
    lines.push('  (c) For STIMULANT users specifically (Vyvanse, Adderall, etc.): the FIRST meal of the day (lowest meal_number, typically breakfast or morning) must cap at 30g protein. Stimulant suppression peaks in the morning; loading protein there guarantees the meal is skipped. Shift the protein load to lunch / dinner / evening meals when the suppression window has passed.')
    ...
    lines.push('  (e) Always include an afternoon protein snack between lunch and dinner — the 3-5pm window often coincides with stimulant rebound and is the most reliable additional eating opportunity.')
