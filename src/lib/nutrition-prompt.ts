...
- MEAL SCHEDULE vs THE CLIENT'S DOCUMENTED SNACKING WINDOW: for Stabilisation / Recovery Reset entry states where the client context indicates reactive/emotional snacking (grazing, post-dinner sweets, ice cream / chocolate / gummies, stress-eating), FIRST identify from the intake/synthesis WHEN that snacking actually fires — read the risk flags, do NOT assume or fabricate a time. It may be afternoon, evening, or specifically post-dinner. Then place structured food to occupy THAT documented window. Do NOT invent a different window (e.g. claiming "3-5pm" when the data says "evening/post-dinner") to justify a meal placed elsewhere, and do NOT leave the real window uncovered while asserting it is handled. If food in that window is genuinely contraindicated (e.g. an evening window that would collide with the sleep-protection boundary of finishing food 2-3h before bed), do NOT silently ignore it — either place a small structured item at the earliest edge of the window that still respects the sleep boundary, OR name an explicit non-food strategy for that window in the execution_rules. Never claim coverage the schedule does not provide.
