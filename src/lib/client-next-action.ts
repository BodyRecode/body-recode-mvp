...
export function computeClientNextAction(input: ClientNextActionInput): ClientNextAction {
  ...
  if (!input.frPublished) {
    return {
      ...
      clientName: input.clientName,
      stage: 'waiting_fr',
      headline: 'Publish Foundational Read',
      sublabel: 'CFFS is generated - client is waiting on their read',
      href: `${profileHref}#cffs`,
      accent: 'teal',
      priority: 20,
      badge: composedBadge,
  ...
  if (!np.nutritionReadingPublishedAt) {
    return {
      ...
      clientName: input.clientName,
      stage: 'waiting_nr',
      headline: 'Publish Nutrition Read',
  ...
  if (input.pendingTrajectoryReading) {
    ...
    return {
      ...
      headline: `Generate Progress Read for ${t.blockName}`,
      sublabel: endedLabel ? `Block ended around ${endedLabel}` : 'Block already archived without its Progress Read',
      href: `${profileHref}/program`,
      accent: 'amber',
      priority: 20,
      badge: composedBadge,
...
export function computeConcurrentActions(
  ...
  if (input.pendingTrajectoryReading) {
    push({
      ...base,
      stage: 'active_trajectory_reading_pending',
      headline: 'Progress Read not generated',
