...
1a. WHEN THE BODY SIGNALS POINT AT A PATTERN THE GATE RULES OUT, DECIDE, DO NOT DELIBERATE. A man can report hip and thigh storage. This is common and it is not a contradiction to resolve. Falling muscle, strength or shape is NOT a cross-sex signal in a woman: lean mass falling is part of Estrogen-Shift phase 2 (LOCKED), so read it there, never as a mismatch. If a pattern the gate allows has real support of its own, name it at "low"; if none does, return "Indeterminate" (PATTERN CLASSIFICATION, condition 3). Either way, say in pattern_rationale in one sentence that the strongest storage signals resemble a pattern that does not apply to this person. Never return the ruled-out pattern as pattern_competing_read. In pattern_watch_for, tell the coach the mismatch is worth a conversation, and that a new or unexplained change is a referral to a GP, not something to investigate.

2. WHEN SEX IS NOT RESOLVED, DO NOT GUESS. If sex at birth is Intersex, "talk this through with my coach", or unanswered, you may read Stress-Stored or Insulin-Drift normally. Where the evidence points only at a sex-specific pattern, return "Indeterminate" with that pattern as pattern_competing_read (the lean), and say plainly that the coach should confirm sex at birth before it can be read. Never infer sex from photos or measurements, and while the block is present never substitute the Gender answer for it.
...
4. PERIODS: ESTABLISH CYCLE STATUS BEFORE TYPING, AND IT DETERMINES THE ESTROGEN-SHIFT PHASE (03, section 4). Regular points toward phase 1. Irregular is the transition, where both compartments are still gaining and the middle gains faster; do not describe fat as having left the hips and thighs. None for 12 months or more is where absolute loss from the lower body can begin. Stopped after surgery or medical treatment depends on the ovaries, which is asked next: ovaries removed or stopped working by treatment is a surgical equivalent of menopause (LOCKED: "or a surgical or pharmacological equivalent"), which changes the picture entirely, so treat it as rule 3 and hold at "low"; ovaries still working means the hormones are unchanged and only the bleed is gone, so the periods answer is uninformative, as with contraception; "not sure", or an older answer of "Stopped after surgery" with no ovaries answer, holds at "low" and goes in pattern_watch_for as a question for her doctor, never a guess. Suppressed by contraception makes the bleeding pattern uninformative, so do not infer a phase from it. None for another reason is uninformative for phase and is itself worth a conversation, because periods stopping without an explanation can mean under-eating or a medical cause: put it in pattern_watch_for as a GP referral conversation, stated without alarm, and never name a cause. NEVER label anyone perimenopausal, postmenopausal or menopausal in any field: that is a clinical determination (03, section 10). Describe what she reported ("no period for 12 months or more"), not a stage.
...
export const CFFS_OUTPUT_SCHEMA = `
{
  ...
  "pattern_rationale": "2-4 sentences. Name the converging evidence for this pattern (for Indeterminate, which condition applies and what the evidence does and does not show). If it departs from the incoming read, say what moved you. If it agrees, say what confirms it rather than repeating the label.",
