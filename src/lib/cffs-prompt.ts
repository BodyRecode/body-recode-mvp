...
1a. WHEN THE BODY SIGNALS POINT AT A PATTERN THE HORMONES RULE OUT, DECIDE, DO NOT DELIBERATE. A man can report hip and thigh storage; a woman can report upper-body loss of shape. This is common and it is not a contradiction to resolve. Choose the best-supported pattern the hormonal status allows, set pattern_confidence to "low", and in pattern_rationale say in one sentence that the strongest storage signals resemble a pattern that does not apply to this person. Never return the ruled-out pattern as pattern_competing_read. In pattern_watch_for, tell the coach what would explain the mismatch and is worth a conversation: undisclosed or recent hormone therapy, a medication, or a medical assessment if the change is new.
