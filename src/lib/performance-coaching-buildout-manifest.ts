...
const APP_PHASES: Phase[] = [
  ...
  },
  {
    ...
    description: 'The coaching engine itself, audited against the code on 14 Sep 2026. What genuinely runs.',
    longDescription: [
      'Until 14 Sep 2026 this board tracked almost nothing but the plumbing for other coaches running the system under their own brand: tenancy, partner billing, per-coach messaging, the co-pilot, the operator console. The engines that actually coach a client were marked "not yet audited".',
      'This phase is that audit. Every step here was checked against the code, not taken from a note, because the auto-memory has twice recorded a feature as working that was not connected to anything. Where a note and the code disagreed, the code won and the note was corrected.',
      'The headline finding: the engine was built to HELP a coach, not to replace one. Every piece assumes Kade reviews the output, decides when the next block is built and judges when progress happens. That is fine for coaching. It is the exact gap the next phase closes.',
    ],
    order: 7,
    steps: [
      {
        id: 'engine-program-generation',
        title: 'Training generation against the doctrine',
        description: 'Blocks generated through the full program doctrine: patterns, phases, volume, load and progression rules.',
        status: 'shipped',
        effort: 'L',
        surfaces: ['src/app/api/generate-program/route.ts', 'src/lib/program-prompt.ts'],
        notes: 'Runs daily in the practice. The doctrine reference is Program_Generation_Engine_Doctrine_Reference_v1.0. Works well with a coach reviewing every block.',
      },
      {
        id: 'engine-set-ceiling-clamp',
        title: 'Session volume ceiling checked in code',
        description: 'Working sets trimmed back to the doctrine ceiling after generation, whatever the model returned.',
        status: 'shipped',
        effort: 'S',
        surfaces: ['src/lib/training-doctrine.ts'],
        notes: 'Verified 14 Sep: clampProgramToDoctrine has 4 call sites and enforceUpperLowerBias has 3. Trunk sets are deliberately excluded from the working-set count. One of the few doctrine rules that is enforced in code rather than only asked for in the instructions.',
      },
      {
        id: 'engine-block-brief-wired',
        title: 'The per-block brief reaches the generator',
        description: 'Block-specific instructions are actually sent to the model, ranked above standing guidance.',
        status: 'shipped',
        shippedAt: '2026-09-08',
        effort: 'S',
        commits: ['df218b37'],
        surfaces: ['src/app/api/generate-program/route.ts', 'src/lib/program-prompt.ts'],
        notes: 'Verified 14 Sep: passed as prescription_rationale into buildProgramUserPrompt. For weeks before 8 Sep it was saved to the database and never sent, which read as the model disobeying.',
      },
      {
        id: 'engine-recovery-soft-gate',
        title: 'Recovery and regulation gate on generation',
        description: 'Recovery state constrains what training and nutrition may be generated.',
        status: 'shipped',
        shippedAt: '2026-05-10',
        effort: 'M',
        surfaces: ['src/app/api/generate-program/route.ts'],
        notes: 'A soft gate: it steers generation rather than refusing output. Verified present in the program generation route 14 Sep.',
      },
      {
        id: 'engine-nutrition-validation',
        title: 'Nutrition safety floors checked in code',
        description: 'Macros computed from the foods, never authored by the model. Protein, carb and fat floors enforced, with one retry.',
        status: 'shipped',
        shippedAt: '2026-05-18',
        effort: 'M',
        surfaces: ['src/lib/nutrition-validation.ts'],
        notes: 'Verified 14 Sep: validateNutritionPlan has 7 call sites. Built after a plan published at about 1,011 kcal against a stated 2,200 to 2,400 band. The breakfast-highest-protein rule exists but is OFF by default and coach opt-in only, because every plan breaks it.',
      },
      {
        id: 'engine-supplements-attached',
        title: 'Supplements generated with the food plan',
        description: 'Food and supplements produced in one act, as the consumption plan was specified.',
        status: 'shipped',
        shippedAt: '2026-09-08',
        effort: 'S',
        surfaces: ['src/lib/consumption-plan-generate.ts'],
        notes: 'Verified 14 Sep: attachSupplementsToPlan has 4 call sites. Before 8 Sep it had none and every plan had an empty supplements field.',
      },
      {
        id: 'engine-nutrition-revise',
        title: 'Revise a plan one change at a time',
        description: 'Carry the live plan forward with only the named change. Same foods, same meals, nothing else moves.',
        status: 'shipped',
      ...
      },
      {
        ...
        title: 'Live session logging',
        description: 'Sets, reps, load and effort recorded during a session, with block progress.',
        status: 'shipped',
        shippedAt: '2026-05-10',
        effort: 'M',
        surfaces: ['src/lib/workout-log-write.ts', 'src/app/portal/[token]/program/log', 'src/app/dashboard/clients/[id]/train'],
        notes: 'Three serious faults found and fixed 7 to 8 Sep: the block week rolled at the time of day it was activated, today\'s session never matched its own label, and a block past its end date refused every session and lost them. Together they made in-person logging close to impossible for weeks. There is still no automated test that would catch a fourth.',
      },
      {
        id: 'engine-effort-creep',
        title: 'Effort creep monitor',
        description: 'Flags when logged effort runs above prescribed effort in the current block week.',
        status: 'shipped',
        shippedAt: '2026-05-11',
        effort: 'S',
        notes: 'Verified 14 Sep: surfaced in the session logger, the Today dashboard and the client file.',
      },
      {
        id: 'engine-progress-read',
        title: 'Block-end progress read',
        description: 'A read at block end that re-scores body state from a progress check.',
        status: 'shipped',
        shippedAt: '2026-08-11',
        effort: 'M',
        notes: 'Shipped, and at the time of the audit still awaiting its first real block-end test on a client. Note the Body Recode loop decided on 9 Sep that the re-read trigger is TIME, not block end, so this and the time-based re-read on the Body Recode board need reconciling.',
      },
    ],
  },
  {
    ...
    steps: [
      {
        id: 'solo-doctrine-checked-in-code',
        title: 'Every plan checked against the doctrine before it saves',
        description: 'One validation pass after every generation that refuses to save output breaking a doctrine rule.',
        status: 'in_progress',
        effort: 'L',
        notes: 'STARTED 14 Sep 2026, TRAINING: src/lib/program-doctrine-check.ts checks four prompt-only rules in code (regulation Red = no axial loading; Remediation = no axial outside the primary slot, no high stability demand; an injured joint excludes exercises stressing it), runs on every generated block and writes each break into the coach-only doctrine note. REPORTED NOT ENFORCED: run against the 8 live and draft blocks, 7 broke the rules as worded, all coach-approved (barbell squats and deadlifts on regulation-Red clients, Dead Bug, Bird Dog and Plank for lower back pain, leg press and step-ups for knee pain), so enforcing would block the practice. Waiting on Kade: per rule, refuse / retry / warn, and whether the rules or the practice change. Retry note and red-day RPE clamp built, not wired. npm run test:program-doctrine (10). ALSO FOUND AND FIXED: the approved-library check in generate-program read ex.name, which is undefined on every exercise (the field is exercise_name), so it never refused anything; Luke\'s live block has 6 exercises not in the library. NUTRITION day calorie target already checked (DAY_KCAL_OUTSIDE_TARGET, 31 Aug). BEFORE: Every rule that currently lives only in the instructions gets a code check: set ranges, effort ceilings, no axial loading on a red readiness day, upper and lower balance, contraindicated movements, and a declared daily calorie band. Real failures this would have caught: three barbell squats prescribed on a red day, an 11 upper to 23 lower block, and a client left on 1,409 kcal for twelve weeks because nothing stores a daily calorie total to object to. Partial precedents already work this way: the nutrition floors and the volume clamp.',
      },
      {
        id: 'solo-next-block-auto',
        title: 'The next block builds itself',
        description: 'When a block ends, the next one generates from the check-ins and progress data, without a coach deciding when.',
        status: 'planned',
        effort: 'M',
        blockedBy: 'solo-doctrine-checked-in-code',
        notes: 'Today blocks routinely overrun because the next one is built when Kade has the data, not on the day the old one ends. That is workable with a coach. With no coach, nothing would ever build it. Must not auto-generate until doctrine checks exist, or it automates unreviewed output.',
      },
      {
        id: 'solo-progression',
        title: 'Progression without a coach deciding',
        description: 'Load, volume and exercise progression decided by rules, from logged performance and readiness.',
        status: 'planned',
        effort: 'L',
        blockedBy: 'solo-doctrine-checked-in-code',
        notes: 'The engine writes a block and a coach judges when someone is ready to progress. The progression rules exist in the doctrine reference; they are not yet applied session to session by the system.',
      },
      {
        id: 'solo-equipment-programming',
        title: 'Programming to the equipment she has',
      ...
      },
      {
        ...
        title: 'Automated tests for the engine',
        description: 'Tests that run on every change and fail if a plan breaks doctrine or logging stops working.',
        status: 'planned',
        effort: 'M',
        notes: 'Added in the 14 Sep gap audit. The auto-memory\'s own summary: "the engine is good, there is no test harness." Three separate logging faults were found only by Kade logging real sessions. With no coach, the first person to find a fourth would be a paying customer.',
      },
      {
        id: 'solo-model-change-evaluation',
        title: 'A fixed test set run before any AI model change',
        description: 'A set of real, anonymised cases whose outputs are compared before a model upgrade reaches customers.',
        status: 'planned',
        effort: 'M',
        notes: 'Added in the 14 Sep gap audit. A model tier change once left weekly syntheses silently empty. With a coach that is caught within a week; with thousands of subscribers a model upgrade could change what every woman is told overnight. A nutrition test corpus already exists as a starting point.',
      },
      {
        id: 'solo-ai-call-hardening',
        title: 'Every AI call made reliable',
        description: 'Streaming, dead-connection detection, time budgets and retries on every generation, not only the read.',
        status: 'planned',
        effort: 'M',
        notes: 'Added in the 14 Sep gap audit. The read itself was fixed on 13 Sep after reads hung for ten minutes or came back empty. Weekly syntheses, programs, nutrition and readings still use the old pattern, with the same failure risk, unaudited.',
      },
      {
        id: 'solo-cost-at-scale',
        title: 'Cost and load at a thousand subscribers',
        description: 'What reads, plans and conversations cost per subscriber, and whether the providers\' rate limits hold at consumer volume.',
        status: 'planned',
        effort: 'S',
        notes: 'Added in the 14 Sep gap audit. The August measurement of about $1.18 per client lifetime predates the current read, which now produces 9,000 to 15,000 output tokens and takes one and a half to two and a half minutes. Voice and open conversation are not in that figure at all. The $199 price depends on this number.',
      },
    ],
  },
  {
    ...
    steps: [
      {
        id: 'session-readiness-adapts',
        title: 'Today\'s session changes with her readiness',
        description: 'A readiness check before the session: green runs as planned, amber reduces intensity, red swaps to recovery or rest.',
        status: 'planned',
        effort: 'M',
        blockedBy: 'solo-doctrine-checked-in-code',
      ...
      },
      {
        ...
        title: 'The system chooses a safe swap',
        description: 'When equipment is taken or a movement hurts, the system picks an equivalent that respects the read.',
        status: 'planned',
        effort: 'M',
        notes: 'Half exists. Verified 14 Sep: the session logger lets the client RECORD a substitution she chose herself, with a name and reason. The system cannot choose one. Choosing needs the exercise metadata to find an equivalent movement pattern and to exclude anything the read restricts.',
      },
      {
        id: 'session-soreness-triage',
        title: 'Soreness versus pain, as written doctrine',
 ...
 * work; then the capability layer, then the engine audit (14 Sep 2026) and the
 * two phases that make the engine run without a coach.
 */
