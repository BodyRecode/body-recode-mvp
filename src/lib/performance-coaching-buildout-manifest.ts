 ...
 * and everything built to sell it to other coaches.
 *
 * The sibling of `saas-buildout-manifest.ts`. Split 9 Sep 2026 along the
 ...
 *                          nutrition, the portal, the coaching loop.
 *
 * ⚠ THE COLLECTIVE LIVES HERE, not on the Body Recode board (Kade, 9 Sep 2026).
 * The Body Recode Collective was designed for coaches running the PERFORMANCE
 * COACHING product: their whole coaching business — programs, nutrition, portal
 * — under their own brand, on BR doctrine. That is Layer 2 white-labelled, so
 * every Collective phase (decide, pilot-zero, tenancy, billing, doctrine mode B)
 * belongs to this board. It was on the Body Recode board only because there used
 * to be one board.
 *
 * Note the multi-coach plumbing built for the Collective — tenant_config, the
 * resolver, per-tenant branding and Stripe Connect — is genuinely shared, and
 * door 2 on the Body Recode board depends on it. It is tracked here because this
 * is what it was built for; that dependency is called out on the other board
 * rather than duplicated.
 *
 * Coach Co-Pilot and the Operator Console are here too: they draft and operate
 * Layer 2 artefacts.
 *
 * MAINTENANCE RULE (from `feedback_ship_checklist`): every commit that ships or
 * changes state on a Layer 2 or Collective step MUST update the corresponding
 * entry here in the same commit. Silent drift is not allowed.
 *
 * ⚠ The coaching APPLICATION itself — program engine, nutrition engine, portal,
 * weekly loop — has NOT been audited into phases yet. Statuses for those would be
 * inference, and an invented status is worse than an empty board. See the
...
const APP_PHASES: Phase[] = [
  ...
  },
  {
    ...
    description: 'The coaching application itself. Real work, not yet broken into tracked steps.',
    longDescription: [
    ...
    steps: [
      {
        id: 'audit-layer-2',
        title: 'Audit the coaching application into tracked phases',
...
/**
 * The board, in the order it should be read. Ids and order are assigned BY
 * POSITION so inserting a phase never means renumbering literals by hand.
 *
 * The Collective phases come first because they are the older, further-along
 * work; the capability layer and the un-audited application follow.
 */
