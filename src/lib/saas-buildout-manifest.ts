  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        description: 'The packaging that turns the co-pilot into a sellable licensed capability for the Collective: per-tenant branding (co-pilot name, glyph, terminology), a per-practice reviewer for the flagged-answers queue (so each practice\'s lead coach reviews their own team), and airtight per-coach scoping so a licensee only ever sees their own roster and clients.',
        status: 'planned',
        effort: 'L',
        blockedBy: 'copilot-broaden',
        notes: 'Everything is already coach_id-scoped and uses a neutral (non-photo) glyph, so this is a settings/packaging layer, not a rebuild. Ties to project_sot_white_label_licensing_model + project_br_pc_powered_by_strategy. The flagged-review reviewer setting was explicitly deferred here at Phase 1.',
      },
    ],
  },
]
