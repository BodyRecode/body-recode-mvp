  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        status: 'shipped',
        shippedAt: '2026-09-14',
        surfaces: ['src/lib/current-read.ts', 'src/lib/body-state-current.ts'],
        effort: 'M',
        notes: 'SHIPPED 14 Sep 2026 (Kade: yes). src/lib/current-read.ts: the live Foundational Read row with the newest PUBLISHED Progress Read\'s re-derived findings laid over it when newer; keeps the CFFS id for foreign keys; enforced columns win over raw content. 21 call sites swapped one line each: weekly synthesis (both paths) and weekly feedback, program generation (new blocks; regenerate of an existing block stays pinned to its read), nutrition generation, all four suggesters and both guidance drafts, the co-pilot, Direction and program suggest pages, recovery and supplement suggestions (pattern too), medication and blood panel analysis and reads. The readiness label on her portal, the client profile and the coaching roster (readiness monitor included) follow it. Deliberately NOT switched: the Foundational Read document and its publishing, preview, report, onboarding status, export. Drafts never count; unpublishing reverts with nothing to undo; a Foundational Read regenerated later wins again. PROVEN: 10 checks on a temporary client. No live client has a published Progress Read, so nothing any client receives changed on the day. BEFORE: Today everything downstream (weekly synthesis readiness anchor, program and nutrition generators, suggestions) still reads the live Foundational Read, so a Progress Read changes nothing a coach generates next. Spec 3 says v2 re-derives the read; for that to mean anything the newest published read must be what they read. A decision for Kade before building, because it changes what every generator is anchored to.',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        status: 'shipped',
        effort: 'S',
        notes: 'DONE 20 Sep, after finding there was no backup of anything at all. Every row of every table plus the 96 uploaded files (progress photographs, blood panels, clearance letters) now copy out on demand, with a verify step that reads the copy back. The database structure itself was not in version control either, so it was captured. The restore was then rehearsed end to end into a local database, which found six bugs that would each have stopped a real recovery, including one where the structure could not rebuild itself at all. Open: prune old copies on a schedule, decide whether they belong somewhere access-controlled rather than a synced folder, and include them when acting on a deletion request.',
      },
      {
      ...
      },
      {
        ...
        title: 'Test Strenn against people trying to talk it into harm',
        description: 'Deliberate attempts to make Strenn ignore its limits, tested before open conversation reaches the public.',
        status: 'planned',
        effort: 'M',
