  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        title: 'Founding Ten legal pack (9 documents)',
        description: 'The full contract chain a partner signs, not just the headline agreement. Discovery: Mutual NDA. Signing: Founding Partner Agreement + Partner IP Sublicence Deed. Upstream: Head Licence Deed (Kade → operating co) + Contractor IP Assignment (inbound copyright). Plus Cover Note, DECISIONS_NEEDED and the IP Protection Map. Chain: Kade owns the IP personally → head licence → operating co → sublicence → partner.',
        status: 'in_progress',
        effort: 'M',
        commits: ['pending'],
        surfaces: [
          '~/Dropbox/03_BODY_RECODE_COLLECTIVE/00_FOUNDING_TEN/legal/ (9 designed docs)',
        ],
    ...
    steps: [
      {
        ...
        surfaces: [
          ...
          '03_BODY_RECODE_COLLECTIVE/00_FOUNDING_TEN/COLLECTIVE_FIT_SCORECARD.md',
        ],
  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        description: 'Work out what a client actually costs in Anthropic tokens across their whole life (CFFS, program, nutrition, the readings, the weekly loop, co-pilot usage) and check it against the per-active-client price. The pricing was set before anyone measured the cost of serving it.',
        status: 'planned',
        effort: 'S',
        notes: 'TRIGGERED BY A REAL EVENT, not a hypothetical: the API account drained in days once usage ramped (onboarding Cristobal, repeated regenerations, co-pilot on Sonnet, retry loops). Kade, verbatim: "I\'m not paying hundreds a day when I\'ve set my prices low." The API is pay-per-token and has NOTHING to do with the Claude Max subscription — it is real COGS that scales with every coach and every client. Do: (1) measure real per-client cost from console.anthropic.com/settings/usage; (2) model against per-active-client pricing so COGS is covered with margin; (3) cut drivers (model tier where quality allows, fewer retry/follow-up calls, avoid needless regenerations, caching); (4) turn on auto-reload + low-balance alert so it never dies mid-session once partners are live. Not launch-blocking; blocking for CONFIDENT white-label economics, and a prerequisite for the Operator Console, which runs tool loops all day and costs materially more per user. See project_collective_pricing_vs_api_cost.',
      },
    ],
  },
  // ─────────────────────────────────────────────────────────────
    ...
    steps: [
      {
        ...
        status: 'planned',
        effort: 'M',
        notes: 'Deliberately NOT a widening of the co-pilot bubble. The bubble is for a question about the page you are on; the console is for work that IS the conversation, which needs room and needs to be resumable. Note the tension with copilot-session-reset: the bubble is now deliberately amnesiac, whereas the console needs durable threads — different surfaces, different memory rules, and that is the point of separating them.',
      },
      {
        ...
        notes: 'THE ONE THAT CANNOT BE RETROFITTED, and the reason it sits before the tools rather than after. A prompt can be talked around; a scoped query cannot. This also absorbs the multi-tenant roster/data scoping that Phase 9 of the co-pilot deferred for want of a second tenant — the console makes it blocking rather than theoretical, because the console can actually go and fetch things.',
      },
      {
        ...
        notes: 'Read-only is where most of the value is and none of the danger. Ship this and stop, before anything can send. It is also the honest test of whether the console is useful: if it cannot answer the dormant-reactivation questions from live data, the writing tools will not save it.',
      },
      {
        id: 'console-tools-action',
        title: 'Action tools — go and do, behind a gate',
        description: 'Triggering things that already exist: draft a re-engagement, work out who is eligible, run a dry run, then stage the send. Anything that sends, charges or deletes stops and waits for an explicit human click.',
        status: 'planned',
        effort: 'L',
        blockedBy: 'console-tools-read',
        notes: 'Never autonomous. The pattern is already proven in production by the dormant reactivation: dry run → human reads the actual list → explicit confirm. The console orchestrates existing safeguarded endpoints rather than inventing new mutation paths, exactly as the co-pilot draft flow reuses suggest→generate. Same reasoning: the gate is what makes it shippable to someone who is not Kade.',
      },
      {
        ...
        notes: 'Needed the moment a licensee can act on their own data: without it there is no answer to "why did my client get that email?", which is both a support burden and a liability. Cheap to build alongside the action tools, expensive to reconstruct afterwards.',
      },
    ],
  },
]
