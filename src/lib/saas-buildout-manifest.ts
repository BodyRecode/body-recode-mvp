  ...
  // ─────────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        notes: 'HARD BLOCKER ON TENANT #2, deliberately recorded rather than quietly omitted. Under one coach an unscoped read returns the same rows as a scoped one, so nothing is wrong today — it stops being the same the moment a second practice\'s posts land in this table, and that is exactly when the console is being licensed. The alternative was to leave the content calendar out of the tool entirely, but a content tool that cannot see the content calendar is close to useless, so the capability ships and the debt is written down. Flagged inline in tools-read.ts with the same warning. Every OTHER console read is scoped: 7 scoped + 5 derived-from-scoped, verified by scripts/verify-console-tools.ts including a leak assertion.',
      },
    ],
  },
]
