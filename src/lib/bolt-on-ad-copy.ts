// Sales copy for each bolt-on pack, keyed by digital_asset_metadata.slug.
//
// Lives next to the bolt-on store route. As more packs ship, add an entry
// per slug. When the inventory grows past ~8 packs this should move into a
// `digital_asset_ad_copy` table or a markdown registry.
//
// Image convention: covers live in the SAME bucket folder as the PDF, with
// the suffix "-cover.png". E.g. protocols/sleep-reset-v1.pdf →
// protocols/sleep-reset-v1-cover.png. The detail page server-side-signs
// the cover when it renders.

export type BoltOnAdCopy = {
  tagline: string                  // one-line index tagline (under the card title)
  hero_headline: string            // detail page hero — the promise
  hero_sub: string                 // detail page hero sub
  who_its_for: string[]            // 3-5 bullets describing the symptom set
  whats_inside: string[]           // 3-6 bullets describing the contents
  format: { label: string; value: string }[]   // bottom card: pages, format, etc
  cover_path: string               // Storage path inside library-assets bucket
}

export const BOLT_ON_AD_COPY: Record<string, BoltOnAdCopy> = {
  'weekly-pattern-report': {
    tagline: 'One personalised reading every week for four weeks. Read in 90 seconds. Walk into the weekend oriented.',
    hero_headline: 'Four weeks. Four personalised readings. One $19.',
    hero_sub: 'Every Friday for the next four weeks, the Body Recode engine reads your latest check-in and your last four weeks of signal and writes you a short, prescriptive reading. Each one is one page. Three observations, one focus. Designed for a Friday afternoon read so you walk into the weekend with one thing to hold.',
    who_its_for: [
      'You like the standard programme but want a personalised voice on top of it.',
      'You journal weekly and want a reference document to write back into.',
      'You miss having a coach read your week and want a substitute that respects your pattern.',
      'You want to make Membership stickier without ramping up to full coaching prices.',
      'You like rituals - Friday-afternoon-read-the-week is your rhythm.',
    ],
    whats_inside: [
      'This week, what your signal is doing - reads your most recent check-in markers and names which is leading.',
      'The trend across the last four weeks - direction held is the signal; one bad week is never the verdict.',
      'The one thing to hold this week - one move, not three, tied to mechanism.',
      'In Kade\'s declarative voice. Mechanism first, instruction second.',
      'One report per week. Four weeks. Delivered Fridays.',
    ],
    format: [
      { label: 'Format', value: '1-page personalised PDF, weekly' },
      { label: 'Length', value: '4 reports across 4 weeks' },
      { label: 'Generated by', value: 'Your member data + Claude trained on BR' },
      { label: 'Delivery', value: 'First report 5-10 min after purchase; then weekly' },
    ],
    cover_path: 'deep-dives/weekly-pattern-report-cover.png',
  },
  'your-custom-block': {
    tagline: 'A two-week adapted training block designed around your specific constraint.',
    hero_headline: 'Your constraint. Your pattern. A two-week block written for it.',
    hero_sub: 'Travel. Injury. Less time. Limited equipment. Describe what is changing for the next two weeks. The Body Recode engine adapts a fortnight of training around your pattern + body state + the constraint you describe. Print it, take it with you, return to your standard programme on day 15.',
    who_its_for: [
      'You are travelling and will not have your usual gym access.',
      'You picked up a niggle and want a routine that works around the joint, not through it.',
      'Your week structure has compressed and 60-minute sessions are not realistic.',
      'You want a deload block written for your situation, not a generic one.',
      'You are between phases and want a bridge block before re-committing to a new cycle.',
    ],
    whats_inside: [
      'The reading - how your pattern and your constraint actually interact.',
      'Week one - intent + 4-5 specific sessions, each with focus, movements, and notes.',
      'Week two - either progressed or held depending on what week one tells your system.',
      'Stepping back to normal - the first session back, deload-or-not, what to watch for.',
      'Built off your Membership pattern. Honours your training rules. No generic one-size-fits-all.',
    ],
    format: [
      { label: 'Format', value: 'Personalised PDF block' },
      { label: 'Length', value: '2 weeks, 8-10 sessions' },
      { label: 'Generated by', value: 'Your member data + Claude trained on BR' },
      { label: 'Delivery', value: '5-10 minutes after purchase' },
    ],
    cover_path: 'deep-dives/your-custom-block-cover.png',
  },
  'a-question-for-kade': {
    tagline: 'Ask Kade your specific question. Get a personalised written response in your inbox.',
    hero_headline: 'Your specific question. Kade\'s framework. A personalised reading.',
    hero_sub: 'Write what is actually going on with your body. The Body Recode engine, trained on the same framework Kade uses with coaching clients, reads your pattern, your last four weekly check-ins, and writes a personalised response. Delivered as a PDF within 10 minutes. Re-read it. Mark it up. Bring it to your next check-in.',
    who_its_for: [
      'You have a specific question that the standard programme does not directly answer.',
      'You feel something has shifted in your body and you want a second voice on it.',
      'You are weighing a decision (changing training intensity, adding a supplement, breaking from the plan) and want a framework-grounded read.',
      'You are between coaching cycles or never went into full coaching and want a one-shot personalised consultation.',
      'You journal and want a reference document to write back into.',
    ],
    whats_inside: [
      'What you are actually asking - your question reframed in Body Recode terms.',
      'What the data says - your last four check-ins read for trend, not single-week verdict.',
      'The Body Recode reading - the mechanism behind what is happening in your body, given your pattern.',
      'This week, do this - three or fewer concrete prescriptions tied to mechanism.',
      'In Kade\'s declarative voice. No motivational padding. No generic advice.',
    ],
    format: [
      { label: 'Format', value: 'Personalised PDF response' },
      { label: 'Generated by', value: 'Your member data + Claude trained on BR' },
      { label: 'Time to read', value: '10 minutes' },
      { label: 'Delivery', value: '5-10 minutes after purchase' },
    ],
    cover_path: 'deep-dives/a-question-for-kade-cover.png',
  },
  'trajectory-deep-dive': {
    tagline: 'A fresh AI reading of how your signal moved across your last block.',
    hero_headline: 'A second reading of your block - on demand, between coaching cycles.',
    hero_sub: 'Your weekly check-ins paint a picture of how the block actually unfolded. This deep-dive runs the same engine your coach uses for the official Block-End Reading, on the schedule you choose, with your weekly syntheses as the input. You read it once, mark it up, and arrive at your next check-in already oriented.',
    who_its_for: [
      'You feel the block went somewhere but cannot quite name what shifted.',
      'You want a second perspective on the same data before the next block plan locks in.',
      'You missed your coach reading window or want a fresh reading mid-block.',
      'You journal and you want a reference document to write back into.',
      'You are between coaching cycles and want continuity in the reading discipline.',
    ],
    whats_inside: [
      'Where this block started - the signal you walked in with.',
      'How your signal moved - the trajectory across every weekly synthesis.',
      'What held steady - the constants that did not budge across the block.',
      'What this sets up next - the levers the next block can pull.',
      'A coach note - the one observation worth carrying forward.',
    ],
    format: [
      { label: 'Format', value: 'Personalised PDF reading' },
      { label: 'Generated by', value: 'Your block data + Claude' },
      { label: 'Time to read', value: '12 minutes' },
      { label: 'Delivery', value: '5-10 minutes after purchase' },
    ],
    cover_path: 'deep-dives/trajectory-deep-dive-cover.png',
  },
  'sleep-reset': {
    tagline: 'A seven-day intervention to restore sleep when it has drifted.',
    hero_headline: 'Your training is only as good as the sleep underneath it.',
    hero_sub: 'When sleep degrades the rest of the protocol underperforms. Output drops, hunger gets volatile, mood baseline sags, body composition stalls. This is the seven-day reset to bring sleep architecture back online.',
    who_its_for: [
      'Sleep onset is creeping past 30 minutes most nights.',
      'You wake at 3am and cannot get back down cleanly.',
      'Mornings have gone from sharp to slow over the last few weeks.',
      'Training output has dropped without a clear training-load reason.',
      'A travel block, work-stress stretch, or alcohol uptick recently disrupted your rhythm.',
    ],
    whats_inside: [
      'The two failure modes of sleep (onset vs maintenance) and how to know which one you have.',
      'The seven-day protocol, day by day, hour by hour. Wake window, light, caffeine cutoff, training timing, last meal, screens, room, bedtime.',
      'The five drift patterns (work-stress, training-too-late, alcohol, travel, screens) and the specific fix for each.',
      'A three-number daily tracking sheet so you can see the trend across the seven days.',
      'What to keep after Day 7 (and when to re-run the reset).',
    ],
    format: [
      { label: 'Format', value: '~10-page PDF' },
      { label: 'Time to read', value: '15 minutes' },
      { label: 'Duration to run', value: '7 days' },
      { label: 'Delivery', value: 'Instant - inbox + library' },
    ],
    cover_path: 'protocols/sleep-reset-v1-cover.png',
  },
}
