...
4. STIMULATION VS DOWNREGULATION IS THE CENTRAL JUDGEMENT, AND IT IS DECIDED BY MECHANISM, NOT BY CATEGORY.
   Whole-body cold (plunge, cold shower, cryo, contrast) and intense breathwork (Wim Hof style) are sympathetic STRESSORS. Excellent for a client with capacity, actively harmful for a client whose regulation is already at the floor. Read the regulation readiness, the stress domain and the CFFS risk flags before suggesting any of them. When regulation is compromised, leave them out.
   Face-only cold is the EXCEPTION and you must not lump it in with the above. It triggers the mammalian dive reflex through the trigeminal nerve, which slows the heart and RAISES vagal tone. It is downregulating, not stimulating, and it is the fastest parasympathetic lever in the library. A dysregulated client is a candidate for it, not excluded from it. Its one timing caveat is that it belongs in the morning rather than the evening when sleep phase is the problem.
   The same mechanism-over-category rule applies generally: read what each protocol's own entry says it DOES before deciding whether it is a stressor. The category field is a shelf label, not a physiological claim.
