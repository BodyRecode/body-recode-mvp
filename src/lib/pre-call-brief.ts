...
// =============================================================
// Call flow — the companion's structure
// =============================================================
...
/**
 * The Zoom call as an ordered set of stages.
 *
 * Built 2026-08-12 to replace a 1,700-line companion of tabs, modes and
 * accordions that Kade said he barely used. Same source as the written brief,
 * so the screen he reads during the call cannot drift from the one he read
 * before it.
 */
export function buildCallFlow(input: LeadBriefInput): CallStage[] {
  const state = input.scorecard_body_state
  const bank = STATE_BANKS[state]
  if (!bank) return []
  ...
  const floorName = floor ? SECTIONS[floor].name : 'unclear'
  const { profile, confidence } = typeFatMapProfile(scores, state, {
    sex: input.biological_sex, ageBand: input.age_band,
    fatStorage: input.fat_storage, cycleStatus: input.cycle_status,
  })
  const named = profile !== 'Indeterminate'
  const scopeFlags = detectScopeFlags(input.prep_notes)
  const firstName = input.name.split(' ')[0]
  const investmentFlagged = input.investment_readiness === 'C' || input.investment_readiness === 'D'

  const stages: CallStage[] = []

  // 01 Open
  const open: CallBlock[] = [
    { kind: 'say', text: `Here's how this'll go. I'll ask you a few things so I understand where you're actually at, then I'll walk you through what your scorecard is telling me and what I'd do about it. If it's a fit I'll tell you what working together looks like. No need to decide anything today.` },
    { kind: 'ask', text: 'What was going on that made you fill this in?' },
    { kind: 'ask', text: 'What have you already tried, and what happened?' },
    { kind: 'ask', text: 'When did you last feel really good? What was different then?' },
    { kind: 'ask', text: 'If we sorted this, what actually changes day to day?' },
    { kind: 'note', text: 'Capture their answers verbatim. You will use their words back in the offer.' },
  ]
  if (scopeFlags.length) {
    open.push({ kind: 'warn', text: `Then work through the scope flags: ${scopeFlags.map(f => f.flag).join(', ')}. Ask, note, move on. Do not interpret.` })
  }
  stages.push({ n: '01', key: 'open', eyebrow: 'Stage One', title: 'Set the agenda, then listen', blocks: open })

  // 02 Their words
  if (input.prep_notes) {
    stages.push({
      n: '02', key: 'words', eyebrow: 'Stage Two', title: 'What they already told you',
      blocks: [
        { kind: 'note', text: 'From their pre-call form. Use their language, do not paraphrase it back at them.' },
        { kind: 'verbatim', text: input.prep_notes },
      ],
    })
  }

  // 03 The read
  const read: CallBlock[] = [
    { kind: 'say', text: bank.readbackLeadWith(floorName, input.scorecard_score, scores) },
    { kind: 'bullets', items: bank.mechanismBullets(floorName) },
    { kind: 'say', text: bank.readbackThen(input.scorecard_score) },
  ]
  if (named) {
    read.push(confidence === 'low'
      ? { kind: 'say', text: `Where it sits on you points toward ${profile}, but I want the intake before I commit to that.` }
      : { kind: 'say', text: `Yours is ${profile.toLowerCase()} specifically.` })
    read.push({ kind: 'note', text: `${profile}: ${PROFILE_DESCRIPTORS[profile]}` })
  }
  if (scopeFlags.length) {
    read.push({ kind: 'warn', text: 'Scope flags are live on this lead. Hold the pattern loosely — a medical picture overlaps most of what the scorecard scores.' })
  }
  read.push({ kind: 'ask', text: 'What was your reaction when you saw the result?' })
  stages.push({ n: String(stages.length + 1).padStart(2, '0'), key: 'read', eyebrow: 'Stage Three', title: 'Walk them through the read', blocks: read })

  // 04 What coaching is
  stages.push({
    n: String(stages.length + 1).padStart(2, '0'), key: 'coaching', eyebrow: 'Stage Four', title: 'What coaching actually is',
    blocks: [
      { kind: 'note', text: 'Bridge from their own words: "Based on what you told me about [their words], here is exactly how my coaching gets to that."' },
      { kind: 'bullets', items: [
        '221-question intake into a Foundational Read. From you, not a template.',
        'Fat Map: four location-plus-signal pairs. Location narrows, the signal decides.',
        'Portal: training, nutrition, weekly check-ins, all driven by your read.',
        'Weekly check-in: I read your response and adjust. A loop, not a static plan.',
      ] },
      { kind: 'say', text: bank.tail(floorName) },
    ],
  })

  // 05 The offer
  const offer: CallBlock[] = []
  if (investmentFlagged) {
    offer.push({ kind: 'warn', text: 'They answered "exploring" or "free only". Do not pitch 1:1. Read them, give the picture, let the offer land flat.' })
  }
  offer.push({ kind: 'note', text: state === 'Depleted State'
    ? 'State maps to the 14-Day Challenge (free), then Blueprint at $97. A Depleted body has no capacity for a heavier prescription yet. Say so.'
    : state === 'Transitioning State'
      ? 'State maps to Blueprint at $97. Membership if they are already moving.'
      : 'State maps to Membership at $49/wk, or 1:1 if they want the read built in.' })
  bank.objections.forEach(o => {
    offer.push({ kind: 'note', text: `If "${o.trigger}" → "${o.response}"` })
  })
  stages.push({ n: String(stages.length + 1).padStart(2, '0'), key: 'offer', eyebrow: 'Stage Five', title: 'Where they go next', blocks: offer })

  // 06 Close
  stages.push({
    n: String(stages.length + 1).padStart(2, '0'), key: 'close', eyebrow: 'Stage Six', title: 'Close and hand off',
    blocks: [
      { kind: 'note', text: `Give ${firstName} these regardless of the outcome. ${STATE_SHORT[state]} state, floor on ${floorName}.` },
      { kind: 'bullets', items: bank.recommendations },
      { kind: 'say', text: `That's a real starting point. What it can't tell you is which one to pull first, or when to change what. Most people on their own end up doing the right things in the wrong order. If you want the read built in, the door's open.` },
    ],
  })
