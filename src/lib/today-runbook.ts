...
function generatePreLaunchDays(): RunbookEntry[] {
  ...
  days.push({
    ...
    checks: [
      ...
      {
        task: 'AM - Delete Wave 1 test enrolments (clean count for launch morning)',
        instructions: [
          'There are 4 test enrolments from the pre-launch build in Wave 1 (kade.dunstone@gmail.com + 3 *@bodyrecode.test rows). Deleting them means Wave 1 starts at 0/50 on Mon 13 Jul instead of 4/50 - cleaner launch metrics + no test noise in feedback dashboard.',
          '',
          '1. Reply to me in this thread "delete wave 1 test enrolments" and I\'ll run:',
          '```sql',
          'DELETE FROM public.challenge_enrollments',
          'WHERE wave = 1 AND (',
          '  lead_id IN (SELECT id FROM public.leads WHERE email LIKE \'%@bodyrecode.test\')',
          '  OR lead_id IN (SELECT id FROM public.leads WHERE email = \'kade.dunstone@gmail.com\')',
          ');',
          '```',
          '',
          'This deletes only the 4 known test rows. Any real enrolments that arrived pre-launch (unlikely but possible) stay untouched.',
          '',
          '2. Verify: refresh [Today dashboard](/dashboard/today) → 📊 Live metrics → Wave should show 0/50.',
          '',
          'Note: this does NOT undo Meta CAPI Lead events already fired for those test enrolments (those live on Meta\'s side). Future clean-count only.',
        ].join('\n'),
      },
