    ...
    <div className="rounded-2xl border border-[#E4E4E0] bg-[#FFFFFF] overflow-hidden">
      <div className="flex items-start gap-3 px-5 pt-5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[16px] font-semibold text-[#0F1115] mb-1">{title}</p>
          <p className="text-[12.5px] text-[#6E747D] leading-relaxed">{description}</p>
        </div>
      </div>
  ...
  return (
    <Section icon={UserIcon} title="Contact details" description="Keep these up to date so we can reach you.">
      <div className="space-y-3">
        <div>
          <label className="text-[12.5px] font-medium text-[#9CA2AB] mb-1.5 block">Email</label>
          <input
            type="email"
            value={emailValue}
            onChange={e => setEmailValue(e.target.value)}
            className="w-full bg-[#FFFFFF] border border-[#E4E4E0] rounded-xl px-3 py-2.5 text-[13.5px] text-[#0F1115] focus:outline-none focus:border-[#DCDCD7]"
          />
          <p className="text-[11px] text-[#9CA2AB] mt-1">Changing this also changes the email you sign in with.</p>
        </div>
        <div>
          <label className="text-[12.5px] font-medium text-[#9CA2AB] mb-1.5 block">Phone</label>
          <input
            type="tel"
            value={phoneValue}
            onChange={e => setPhoneValue(e.target.value)}
            placeholder="+61 ..."
            className="w-full bg-[#FFFFFF] border border-[#E4E4E0] rounded-xl px-3 py-2.5 text-[13.5px] text-[#0F1115] placeholder:text-[#9CA2AB] focus:outline-none focus:border-[#DCDCD7]"
          />
        </div>
        {error && <p className="text-[12.5px] text-[#8A5514]">{error}</p>}
        <button
          onClick={save}
          disabled={!dirty || saving}
          className="w-full inline-flex items-center justify-center gap-2 bg-[#0F1115] text-[#FFFFFF] text-[13.5px] font-bold py-2.5 rounded-xl hover:bg-[#000000] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
        >
    ...
    return (
      <Section icon={Pause} title="Pause coaching" description="Request received. Your coach will be in touch.">
        <div className="bg-[rgba(15,17,21,0.06)] border border-[#DCDCD7] rounded-xl px-4 py-3">
          <p className="text-[13.5px] text-[#0F1115]">Your coach has been notified and will reply to confirm.</p>
        </div>
      </Section>
    )
  }
    ...
    >
      <div className="space-y-3">
        <textarea
          ...
          rows={3}
          className="w-full bg-[#FFFFFF] border border-[#E4E4E0] rounded-xl px-3 py-2.5 text-[13.5px] text-[#0F1115] placeholder:text-[#9CA2AB] focus:outline-none focus:border-[#DCDCD7] leading-relaxed resize-y"
        />
        {error && <p className="text-[12.5px] text-[#8A5514]">{error}</p>}
        <button
          onClick={submit}
          disabled={submitting}
          className="w-full inline-flex items-center justify-center gap-2 bg-[#FFFFFF] border border-[#E4E4E0] text-[#4A4F57] text-[13.5px] font-bold py-2.5 rounded-xl hover:border-[#0F1115] hover:bg-[#F2F2EF] hover:text-[#0F1115] transition-colors disabled:opacity-40"
        >
...
function ReferCard({ clientId, clientName }: { clientId: string; clientName: string }) {
  ...
  return (
    <Section icon={Heart} title="Refer a friend" description="Know someone who would benefit from this work? Send their details and your coach will reach out personally.">
      <div className="space-y-3">
        <div>
          <label className="text-[12.5px] font-medium text-[#9CA2AB] mb-1.5 block">Their name</label>
          <input
            value={name}
        ...
        <div>
          <label className="text-[12.5px] font-medium text-[#9CA2AB] mb-1.5 block">Their email</label>
          <input
            type="email"
            value={contactEmail}
        ...
        <div>
          <label className="text-[12.5px] font-medium text-[#9CA2AB] mb-1.5 block">Note (optional)</label>
          <textarea
            value={note}
        ...
        {error && <p className="text-[12.5px] text-[#8A5514]">{error}</p>}
        {sent && <p className="text-[12.5px] text-[#0F1115]">Thanks. Your coach will reach out to {name || 'them'} this week.</p>}
        <button
          onClick={submit}
  ...
  return (
    <Section icon={Download} title="Download your data" description="Export everything we hold about you as a JSON file. Your intake, baselines, check-ins, readings.">
      <a
        ...
        className="w-full inline-flex items-center justify-center gap-2 bg-[#FFFFFF] border border-[#E4E4E0] text-[#4A4F57] text-[13.5px] font-bold py-2.5 rounded-xl hover:border-[#0F1115] hover:bg-[#F2F2EF] hover:text-[#0F1115] transition-colors"
      >
