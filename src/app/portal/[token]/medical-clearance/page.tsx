...
export default async function PortalMedicalClearancePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const SubmittedDocCard = () => docSignedUrl ? (
    <div className="rounded-2xl border border-[#E4E4E0] bg-[#FFFFFF] p-5">
      <p className="text-[11.5px] font-medium text-[#9CA2AB] mb-3">Your submitted form</p>
      {docSignedUrl.match(/\.(jpg|jpeg|png|gif|webp)(\?|$)/i) ? (
        ...
        <a
          ...
          className="flex items-center gap-3 text-sm text-[#0F1115] hover:text-[#6E747D] transition-colors"
        >
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4 mb-8">
        {[
          ...
          <div key={s.step} className="flex items-start gap-4 bg-[#FFFFFF] rounded-2xl p-5 border border-[#E4E4E0]">
            <div className="w-7 h-7 rounded-full bg-[#0F1115] flex items-center justify-center flex-shrink-0 text-white text-xs font-bold">{s.step}</div>
            <div>
              <p className="text-sm font-semibold text-[#0F1115] mb-0.5">{s.title}</p>
              <p className="text-xs text-[#9CA2AB]">{s.desc}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      >
        <div>
          <p className="text-sm font-semibold text-[#0F1115]">Download Medical Clearance Form</p>
          <p className="text-xs text-[#9CA2AB] mt-0.5">Single-page PDF, pre-filled with your name. Email it to your GP or print to take in.</p>
        </div>
      ...
      <Link
        ...
        target="_blank"
        className="block text-center text-xs text-[#9CA2AB] hover:text-[#6E747D] transition-colors mb-8"
      >
