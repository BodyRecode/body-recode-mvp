...
export default function AgreementForm({
  ...
  return (
    <PortalPageShell
      ...
      description={
        <>
          <span className="block text-[12.5px] text-[#9CA2AB] mb-1">Version 2.5 · Sole Trader, Queensland, Australia</span>
      ...
      <div className="space-y-8 mb-10">
          {AGREEMENT_SECTIONS.map((section) => (
            <div key={section.title}>
              <h2 className="text-[12.5px] font-bold tracking-widest text-[#0F1115] uppercase mb-4">{section.title}</h2>
              <div className="space-y-4">
                {section.subsections.map((sub) => (
                  <div key={sub.title}>
                    <p className="text-[13.5px] font-semibold text-[#0F1115] mb-1">{sub.title}</p>
                    <p className="text-[13.5px] text-[#6E747D] leading-relaxed">{sub.content}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="bg-[#FFFFFF] rounded-2xl p-5 border border-[#E4E4E0]">
            <p className="text-[13.5px] text-[#4A4F57] mb-4">By typing your full name and ticking below, you confirm that you have read, understood, and agree to the terms of this Coaching Agreement.</p>

            <div className="space-y-4">
              <div id="f-fullName" className="scroll-mt-24">
                <label className={`block text-[12.5px] font-semibold uppercase tracking-wide mb-2 ${missing.has('fullName') ? 'text-[#8F2D2D]' : 'text-[#6E747D]'}`}>Full name</label>
                <input
                  type="text"
                  value={fullName}
                  onChange={e => {
                  ...
                  className={`w-full bg-[#E4E4E0] text-[#0F1115] text-[13.5px] rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#0F1115]/50 placeholder-[#9CA2AB] border ${missing.has('fullName') ? 'border-[#E8C9C9]' : 'border-[#E4E4E0]'}`}
                />
                {missing.has('fullName') && <p className="text-[#8F2D2D] text-[12.5px] mt-2 font-medium">Please type your full name.</p>}
              </div>
              ...
              >
                ...
                />
                <span className={`text-[13.5px] ${missing.has('accepted') ? 'text-[#8F2D2D]' : 'text-[#4A4F57]'}`}>I have read and agree to the {brand().name}™ Coaching Agreement.</span>
              </label>
              {missing.has('accepted') && <p className="text-[#8F2D2D] text-[12.5px] -mt-2 ml-7 font-medium">Please tick this box to continue.</p>}
            </div>
          </div>
