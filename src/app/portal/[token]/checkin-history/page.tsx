    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="max-w-lg mx-auto px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-stone-400 text-sm">All your weekly check-in submissions.</p>
        </div>

        {!checkins || checkins.length === 0 ? (
          <div className="rounded-2xl border border-stone-800 bg-stone-900 p-6 text-center">
            <p className="text-stone-500 text-sm">No check-ins submitted yet.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {Object.entries(grouped).map(([weekLabel, entries]) => (
              <div key={weekLabel} className="bg-stone-900 border border-stone-800 rounded-2xl overflow-hidden">
                <div className="px-5 py-3 border-b border-stone-800">
                  <p className="text-xs font-bold text-stone-500 uppercase tracking-widest">{weekLabel}</p>
                </div>
                <div className="divide-y divide-stone-800/60">
                  {(entries || []).map((c) => (
                    <div key={c.id} className="px-5 py-3 flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div className="w-7 h-7 rounded-full bg-teal-400/10 border border-teal-400/20 flex items-center justify-center">
