    ...
    <div className="space-y-5">
                  ...
                  >
                    <span
                      className={`mt-0.5 flex h-4 w-4 flex-none items-center justify-center rounded border text-[10px] leading-none transition-colors ${
                          ...
                          : 'border-[#E4E4E0] bg-white text-transparent group-hover:border-[#9CA2AB]'
                      }`}
                      aria-hidden
                    >
                      ✓
      ...
      {props.exercises.map(ex => {
        ...
        return (
          <div
            key={ex.id}
            className={`bg-[#FFFFFF] border rounded-2xl p-4 ${allSetsSaved ? 'border-[#DCDCD7]' : 'border-[#E4E4E0]'}`}
          >
            {/* Exercise header */}
            <div className="mb-3">
            ...
            <div className="space-y-1.5 mb-3">
              ...
              {Array.from({ length: setCount }, (_, i) => i + 1).map(n => {
                ...
                return <p key={`err-${n}`} className="text-[11px] text-[#8F2D2D] pl-9">Set {n}: {s.error}</p>
              })}
            </div>
