                ...
                {byCategory[cat].map(a => (
                  <div key={a.id} className="rounded-2xl border border-[#E4E4E0] bg-[#FFFFFF] overflow-hidden">
                    <div className="px-5 py-4 border-b border-[#E4E4E0]">
                      <h2 className="text-[20px] font-semibold text-[#0F1115] leading-tight">{a.substance.name}</h2>
                      <p className="text-[13.5px] text-[#6E747D] mt-1">{a.substance.short_description}</p>
                    </div>

                    <div className="px-5 py-4 space-y-4">
                      <div>
                        <p className="text-[11px] font-medium text-[#6E747D] mb-2">What it does</p>
                        <p className="text-[13.5px] text-[#0F1115] leading-relaxed">{a.substance.what_it_does}</p>
                      </div>

                      <div>
                        <p className="text-[11px] font-medium text-[#6E747D] mb-2">Choose your tier</p>
                        ...
                        <div className="pt-3 border-t border-[#E4E4E0]">
                          <div className="flex items-start gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-[13.5px] text-[#6E747D] leading-relaxed whitespace-pre-line">{a.coach_note}</p>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {a.substance.contraindications.length > 0 && (
                        <details className="pt-2">
                          ...
                          <div className="mt-2 space-y-2">
                            <div>
                              <p className="text-[11px] font-medium text-[#8F2D2D] mb-1">Do not take if</p>
                              <ul className="text-[12.5px] text-[#6E747D] leading-relaxed space-y-0.5">
                                {a.substance.contraindications.map((c, i) => <li key={i}>- {c}</li>)}
                              </ul>
                            </div>
                            <div>
                              <p className="text-[11px] font-medium text-[#6E747D] mb-1">Safety</p>
                              <p className="text-[12.5px] text-[#6E747D] leading-relaxed">{a.substance.safety_notes}</p>
                            </div>
                          </div>
                        </details>
                      )}
...
function PortalTierCard({ tier }: { tier: SupplementTier }) {
  return (
    <div className="rounded-xl border border-[#E4E4E0] bg-[#F2F2EF] px-4 py-3">
      ...
      <div className="space-y-1.5 text-[13.5px] text-[#0F1115]">
      ...
      <p className="text-[12.5px] text-[#6E747D] mt-2 leading-relaxed">{tier.notes}</p>
      <p className="text-[12.5px] text-[#6E747D] mt-1 italic leading-relaxed">Best for: {tier.fits_client_profile}</p>
    </div>
  )
}
