  ...
  return (
    ...
    >
      {assignments.length === 0 ? (
        <div className="rounded-2xl border border-[#E4E4E0] bg-[#FFFFFF] px-6 py-10 text-center">
          <p className="text-[14px] text-[#6E747D] leading-relaxed">
        ...
        <div className="space-y-8">
          {categoriesWithData.map(cat => (
            <div key={cat}>
              <p className="text-[10px] font-bold text-[#0F1115] uppercase tracking-widest mb-3">{CATEGORY_LABELS[cat]}</p>
              <div className="space-y-6">
                {byCategory[cat].map(a => (
                  <div key={a.id} className="rounded-2xl border border-[#E4E4E0] bg-[#FFFFFF] overflow-hidden">
                    <div className="px-5 py-4 border-b border-[#E4E4E0]">
                      <h2 className="text-lg font-semibold text-[#0F1115] leading-tight">{a.substance.name}</h2>
                      <p className="text-[13px] text-[#6E747D] mt-1">{a.substance.short_description}</p>
                    </div>

                    <div className="px-5 py-4 space-y-4">
                      <div>
                        <p className="text-[11.5px] font-medium text-[#6E747D] mb-2">What it does</p>
                        <p className="text-[13px] text-[#0F1115] leading-relaxed">{a.substance.what_it_does}</p>
                      </div>

                      <div>
                        <p className="text-[11.5px] font-medium text-[#6E747D] mb-2">Choose your tier</p>
                      ...
                      {a.coach_note && (
                        <div className="pt-3 border-t border-[#E4E4E0]">
                          <div className="flex items-start gap-2">
                            <Sparkles size={12} className="text-[#0F1115] mt-0.5 shrink-0" />
                            <div>
                              <p className="text-[10px] font-bold text-[#0F1115] uppercase tracking-widest mb-1">A note from your coach</p>
                              <p className="text-[13px] text-[#6E747D] leading-relaxed whitespace-pre-line">{a.coach_note}</p>
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {a.substance.contraindications.length > 0 && (
                        <details className="pt-2">
                          <summary className="cursor-pointer text-[11px] text-[#6E747D] hover:text-[#0F1115]">Safety notes and when to skip</summary>
                          <div className="mt-2 space-y-2">
                            <div>
                              <p className="text-[11.5px] font-medium text-[#8F2D2D] mb-1">Do not take if</p>
                              <ul className="text-[12px] text-[#6E747D] leading-relaxed space-y-0.5">
                                {a.substance.contraindications.map((c, i) => <li key={i}>- {c}</li>)}
                              </ul>
                            </div>
                            <div>
                              <p className="text-[11.5px] font-medium text-[#6E747D] mb-1">Safety</p>
                              <p className="text-[12px] text-[#6E747D] leading-relaxed">{a.substance.safety_notes}</p>
                            </div>
                          </div>
                        </details>
                      )}
...
function PortalTierCard({ tier }: { tier: SupplementTier }) {
  return (
    <div className="rounded-xl border border-[#E4E4E0] bg-[#F2F2EF] px-4 py-3">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-[10px] font-bold text-[#0F1115] uppercase tracking-widest">{tier.label}</span>
      </div>
      <div className="space-y-1.5 text-[13px] text-[#0F1115]">
        ...
        <div><span className="text-[#6E747D] font-medium">Timing:</span> {tier.timing}</div>
      </div>
      <p className="text-[12px] text-[#6E747D] mt-2 leading-relaxed">{tier.notes}</p>
      <p className="text-[12px] text-[#6E747D] mt-1 italic leading-relaxed">Best for: {tier.fits_client_profile}</p>
    </div>
  )
}
