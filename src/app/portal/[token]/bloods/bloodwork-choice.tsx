...
export default function BloodworkChoice({
  ...
  if (arranged) {
    return (
      <div className="rounded-2xl border border-[#1B6DFC]/30 bg-blue-50 p-5 mb-8">
        <div className="flex items-start gap-3">
          <span className="mt-0.5 flex-shrink-0 w-5 h-5 rounded-full bg-[#1B6DFC] flex items-center justify-center">
            <Check size={13} className="text-white" strokeWidth={3} />
          </span>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-[#1A1A1A] mb-1">Noted, you will arrange a panel</p>
            ...
            <button
              ...
              disabled={saving}
              className="text-xs font-medium text-[#6B6B6B] hover:text-[#1B6DFC] transition-colors disabled:opacity-50"
            >
              {saving ? 'Saving...' : 'Undo'}
            </button>
          </div>
  ...
  return (
    <div className="rounded-2xl border border-[#1B6DFC] bg-[#FFFFFF] p-5 mb-8">
      ...
      <p className="text-sm text-[#1A1A1A] leading-relaxed mb-1 font-medium">A blood panel gives me the inside picture.</p>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button
          ...
          disabled={saving}
          className="inline-flex items-center gap-2 text-sm font-semibold px-4 py-2 rounded-lg bg-[#1B6DFC] text-white hover:bg-[#5390FF] transition-colors disabled:opacity-50"
        >
          {saving ? <Loader2 size={14} className="animate-spin" /> : null}
          I&apos;ll get a panel done
        </button>
        <span className="text-xs text-[#999999]">or upload recent results below</span>
      </div>
