...
export default function BloodworkChoice({
      ...
      <div className="rounded-2xl border border-[#0F1115]/30 bg-[#F2F2EF] p-5 mb-8">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-[#0F1115] mb-1">Noted, you will arrange a panel</p>
            <p className="text-xs text-[#6E747D] leading-relaxed mb-3">
            ...
            <button
              ...
              disabled={saving}
              className="text-xs font-medium text-[#6E747D] hover:text-[#0F1115] transition-colors disabled:opacity-50"
            >
              {saving ? 'Saving...' : 'Undo'}
            </button>
          </div>
  ...
  return (
    <div className="rounded-2xl border border-[#0F1115] bg-[#FFFFFF] p-5 mb-8">
      <p className="text-xs font-bold tracking-widest text-[#0F1115] uppercase mb-2">Part of your Foundational Read</p>
      <p className="text-sm text-[#0F1115] leading-relaxed mb-1 font-medium">A blood panel gives me the inside picture.</p>
      <p className="text-xs text-[#6E747D] leading-relaxed mb-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <button
          ...
          disabled={saving}
          className="inline-flex items-center gap-2 text-sm font-semibold px-4 py-2 rounded-lg bg-[#0F1115] text-white hover:bg-[#000000] transition-colors disabled:opacity-50"
        >
          {saving ? <Loader2 size={14} className="animate-spin" /> : null}
          I&apos;ll get a panel done
        </button>
        <span className="text-xs text-[#9CA2AB]">or upload recent results below</span>
      </div>
      {error && <p className="mt-2 text-xs text-[#8A5514]">{error}</p>}
    </div>
  )
}
