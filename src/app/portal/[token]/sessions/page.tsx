    ...
    >
      {!hasFixedSlots ? (
          <div className="rounded-2xl border border-[#E4E4E0] bg-[#FFFFFF]/50 p-6">
            <p className="text-sm font-semibold text-[#6E747D] mb-1">No fixed session assigned yet</p>
            <p className="text-xs text-[#9CA2AB] leading-relaxed">Your coach will assign your fixed session slot. You will see your schedule here once it is set up.</p>
          </div>
        ) : (
          <>
            {/* Fixed slots summary */}
            <div className="rounded-2xl border border-[#DCDCD7] bg-[#F2F2EF] p-5 mb-8">
              <p className="text-xs font-bold tracking-widest text-[#0F1115] uppercase mb-3">Your fixed slots</p>
              <div className="space-y-1">
                {(fixedSlots ?? []).map(slot => (
                  <div key={slot.id} className="flex items-center gap-2">
                    <span className="text-sm font-bold text-[#0F1115]">{DAYS[slot.day_of_week]}s</span>
                    <span className="text-sm text-[#6E747D]">
              ...
              <p className="text-xs text-[#9CA2AB] mt-2">AF Newstead</p>
            </div>
            ...
            <div className="mb-8">
              ...
              {allSessions.length === 0 ? (
                <p className="text-sm text-[#9CA2AB]">No sessions scheduled yet.</p>
              ) : (
                <div className="space-y-2">
                  {allSessions.slice(0, 10).map((session, i) => (
                    <div key={i} className="flex items-center justify-between rounded-xl bg-[#FFFFFF] border border-[#E4E4E0] px-4 py-3">
                      <div>
                        <p className="text-sm font-medium text-[#0F1115]">{session.displayDate}</p>
                        <p className="text-xs text-[#9CA2AB] mt-0.5">{session.displayTime} · {session.durMin} min</p>
                      </div>
                      {session.confirmed ? (
                        <span className="text-xs font-bold text-[#0F1115] bg-[#F2F2EF] px-2.5 py-1 rounded-full">Confirmed</span>
                      ) : (
                        <span className="text-xs text-[#9CA2AB]">Scheduled</span>
                      )}
                    </div>
                  ))}
                </div>
