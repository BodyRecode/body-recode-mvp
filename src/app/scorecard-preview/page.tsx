...
export default function ScorecardPreview() {
  ...
  return (
    <>
      ...
      {/* Page */}
      <div className="page" style={{ maxWidth: '794px', margin: '40px auto 80px', background: '#0c0a09', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 25px 60px rgba(0,0,0,0.3)', color: 'white' }}>
        ...
        <div style={{ padding: '0 64px' }}>
          {sections.map((section, i) => (
            <div key={section.number} style={{ borderBottom: i < sections.length - 1 ? '1px solid #1c1917' : 'none', padding: '36px 0' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
                <span style={{ fontSize: '11px', fontWeight: 700, color: '#14b8a6', letterSpacing: '0.12em', textTransform: 'uppercase' }}>{section.number}</span>
                <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#ffffff', letterSpacing: '-0.01em' }}>{section.title}</h2>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {section.rows.map(row => (
                  <div key={row.score} style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', background: '#111110', borderRadius: '10px', padding: '14px 18px' }}>
                    {/* Score circle */}
                    <div style={{
                    ...
                    <p style={{ fontSize: '14px', color: '#d4cfc9', lineHeight: 1.55, flex: 1 }}>{row.desc}</p>
                    {/* Checkbox */}
        ...
        {/* Total */}
        <div style={{ margin: '0 64px', padding: '28px 32px', background: '#111110', border: '1px solid #1c1917', borderRadius: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <p style={{ fontSize: '11px', fontWeight: 700, color: '#57534e', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '6px' }}>Total Score</p>
              <p style={{ fontSize: '13px', color: '#a8a29e' }}>Add up your five scores above</p>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <div style={{ width: '56px', height: '40px', borderRadius: '8px', border: '1.5px solid #2c2826' }} />
              <span style={{ fontSize: '16px', color: '#57534e', fontWeight: 600 }}>/&nbsp;15</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ padding: '48px 64px 0' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {results.map(r => (
              <div key={r.range} style={{ background: '#111110', border: '1px solid #1c1917', borderRadius: '12px', padding: '24px 28px', borderLeft: `3px solid ${r.color}` }}>
                ...
                <p style={{ fontSize: '13px', color: '#a8a29e', lineHeight: 1.7 }}>{r.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ margin: '48px 64px 64px', background: '#0d2d29', border: '1px solid #14b8a6', borderRadius: '14px', padding: '36px 40px' }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', background: '#14b8a6', color: '#0c0a09', fontWeight: 700, fontSize: '14px', padding: '12px 28px', borderRadius: '8px' }}>
