...
export default function ClientListColumn({ clients }: { clients: ClientListEntry[] }) {
  ...
  return (
    <aside className="hidden xl:block print:hidden">
      <div className="sticky top-4 max-h-[calc(100vh-2rem)] flex flex-col rounded-xl border border-[#2A2F39] bg-[#14171D] shadow-[0_1px_3px_rgba(16,24,40,0.09),0_1px_2px_-1px_rgba(16,24,40,0.05)] overflow-hidden">
        <div className="p-2.5 border-b border-[#1A1E26]">
          <div className="relative">
            <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#676D76]" />
            <input
              type="text"
              value={query}
              ...
              className="w-full text-[12.5px] text-[#FAFAF8] placeholder:text-[#676D76] pl-8 pr-7 py-[7px] rounded-lg border border-[#2A2F39] bg-[#0B0D10] focus:outline-none focus:border-[#2A2F39] focus:bg-[#14171D] focus:ring-[3px] focus:ring-[rgba(27,109,252,0.13)]"
            />
            {query && (
              <button
                type="button"
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 text-[#676D76] hover:text-[#FAFAF8]"
              >
                <X size={13} />
              </button>
            )}
        ...
        <div className="flex-1 min-h-0 overflow-y-auto py-1.5">
          {total === 0 ? (
            <p className="text-[12.5px] text-[#676D76] px-3 py-6 text-center">
              No client matches &ldquo;{query}&rdquo;.
            </p>
          ) : (
            groups.map(group => (
              <div key={group.status}>
                <p className="px-3 pt-2.5 pb-1 text-[10px] font-semibold text-[#676D76]">
