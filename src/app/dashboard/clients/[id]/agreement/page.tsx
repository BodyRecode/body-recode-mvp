            ...
            <div className="space-y-8">
              {AGREEMENT_SECTIONS.map((section) => (
                <div key={section.title}>
                  <p className="text-xs font-bold tracking-widest text-teal-400 uppercase mb-4">{section.title}</p>
                  <div className="space-y-4">
                    {section.subsections.map((sub) => (
                      <div key={sub.title}>
                        <p className="text-sm font-semibold text-stone-200 mb-1">{sub.title}</p>
