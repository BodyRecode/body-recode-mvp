  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl border border-[#2A2F39] bg-[#14171D] overflow-hidden">
        <div className="flex items-center justify-between gap-3 px-5 py-4 border-b border-[#2A2F39]">
          <div>
            <h2 className="text-base font-semibold text-[#FAFAF8]">Equipment access</h2>
            <p className="text-[11px] text-[#8A9099]">Tag what {clientName} can actually do. Protocol library filters to match.</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 text-[12.5px] font-medium px-4 py-2 bg-[#FAFAF8] text-[#0B0D10] rounded-lg hover:bg-[#FFFFFF] disabled:opacity-40 transition-colors"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 divide-y md:divide-y-0 md:divide-x divide-[#1A1E26]">
      ...
      <div>
        <div className="mb-3">
          <h2 className="text-base font-semibold text-[#FAFAF8]">Protocol library</h2>
                  ...
                  return (
                    <div key={p.slug}>
                      {isNewProgressionGroup && p.progression && (
                        <div className="rounded-xl border border-[#4A3A22] bg-[linear-gradient(180deg,#1A1E26,#1A1E26)]/50 px-4 py-3 mb-2">
                          <p className="text-[10px] font-medium text-[#E0A254] mb-1">{p.progression.group_label} - tiered progression</p>
                          <p className="text-[12.5px] text-[#E0A254] leading-relaxed">{p.progression.group_rule}</p>
                        </div>
                      )}
                    <div className={`rounded-xl border overflow-hidden transition-colors ${isActive ? 'border-[#FAFAF8]/30 bg-[rgba(27,109,252,0.08)]/30' : 'border-[#2A2F39] bg-[#14171D]'}`}>
                          ...
                          >
                            ...
                            <div>
                              <div className="flex items-center gap-2 flex-wrap">
                                {p.progression && (
                                  <span className="text-[10px] font-medium text-[#E0A254] bg-[#1A1E26] border border-[#4A3A22] px-1.5 py-0.5 rounded">
                                    Level {p.progression.level}
                                  </span>
                                )}
                                <span className="text-sm font-semibold text-[#FAFAF8]">{p.name}</span>
                                {isActive && <span className="text-[10px] font-medium text-[#FAFAF8] bg-[#FAFAF8]/10 px-1.5 py-0.5 rounded">Active</span>}
                              </div>
                              <p className="text-[12.5px] text-[#8A9099] mt-0.5">{p.short_description}</p>
                            </div>
                          </button>
                        </div>
                        <button
                      ...
                      {isExpanded && (
                        <div className="px-4 pb-4 pt-1 border-t border-[#2A2F39] bg-[#0B0D10] space-y-3">
                          <ProtocolDetail label="What it does" body={p.what_it_does} />
                          <div>
                            <p className="text-[10px] font-medium text-[#8A9099] mb-1">Steps</p>
                            <ol className="space-y-1">
                              {p.steps.map((s, i) => (
                                <li key={i} className="text-[12.5px] text-[#FAFAF8] leading-relaxed flex gap-2">
                                  <span className="text-[#676D76]">{i + 1}.</span>
                                  <span>{s}</span>
                                </li>
                              ))}
                            </ol>
                          ...
                          <div className="rounded-lg bg-[#14171D] border border-[#2A2F39] px-3 py-2">
                            ...
                            <p className="text-[12.5px] text-[#FAFAF8] leading-relaxed">{p.coach_doctrine}</p>
                          </div>
                        </div>
                      )}
                    </div>
        ...
        {availableProtocols.length === 0 && (
          <div className="rounded-xl border border-dashed border-[#2A2F39] bg-[#0B0D10] px-6 py-8 text-center">
            <p className="text-sm text-[#8A9099]">No protocols available. Tag {clientName}&apos;s equipment access above to unlock the library.</p>
          </div>
        )}
      </div>
    </div>
...
function RrsSuggestionBanner({
  ...
  const bannerBorder = showSbstRemovalAlert ? 'border-[#D4817E]' : 'border-[#4A3A22]'
  const bannerBg = showSbstRemovalAlert ? 'bg-[#1A1214]' : 'bg-[#1A1E26]'
  const textColour = showSbstRemovalAlert ? 'text-[#D4817E]' : 'text-[#E0A254]'
  const eyebrowColour = showSbstRemovalAlert ? 'text-[#D4817E]' : 'text-[#E0A254]'
    ...
    <div className={`rounded-xl border ${bannerBorder} ${bannerBg} overflow-hidden`}>
      <div className="px-5 py-4">
        ...
        {suggestion.sbst_action && (
          <div className={`ml-6 mb-3 rounded-lg border ${showSbstRemovalAlert ? 'border-[#2A2F39] bg-[#14171D]' : 'border-[#4A3A22] bg-[#14171D]'} px-3 py-2`}>
            ...
            {showSbstRemovalAlert && !sbstRemovalConfirmed && (
              <button
                ...
                className="mt-2 inline-flex items-center gap-1 text-[11px] font-medium px-3 py-1.5 bg-[#D4817E] text-[#FAFAF8] rounded-lg hover:bg-[#D4817E] transition-colors"
              >
                <Check size={12} /> Mark SBST removed
              </button>
            )}
            {sbstRemovalConfirmed && (
              <p className="mt-2 text-[11px] text-[#6FA98B] inline-flex items-center gap-1">
        ...
        {suggestedProtocols.length > 0 && (
          <div className="ml-6 space-y-1.5">
            ...
            {suggestedProtocols.map(p => {
              const isActive = activeSlugs.has(p.slug)
              return (
                <div key={p.slug} className="flex items-center justify-between gap-3 rounded-lg bg-[#14171D] border border-[#2A2F39] px-3 py-2">
                  <div className="flex-1 min-w-0">
                    <p className="text-[13.5px] font-semibold text-[#FAFAF8]">{p.name}</p>
                    <p className="text-[11px] text-[#8A9099] mt-0.5 line-clamp-1">{p.short_description}</p>
                  </div>
                  <button
        ...
        {suggestion.do_not_suggest.length > 0 && (
          <div className="ml-6 mt-3">
            ...
            <p className="text-[11px] text-[#8A9099] mt-1 leading-relaxed">
  ...
  return (
    <div className="p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[10px] font-medium text-[#8A9099]">{title}</span>
      </div>
        ...
        {tags.map(tag => (
          <label key={tag} className="flex items-center gap-2 cursor-pointer group">
            <input
              ...
              className="w-4 h-4 rounded border-[#2A2F39] text-[#FAFAF8] focus:ring-[#FAFAF8]"
            />
            <span className="text-[13.5px] text-[#FAFAF8] group-hover:text-[#FAFAF8]">{EQUIPMENT_LABELS[tag]}</span>
          </label>
        ))}
      </div>
    </div>
  ...
  if (!protocol) {
    return (
      <div className="rounded-xl border border-[#4A3A22] bg-[linear-gradient(180deg,#1A1E26,#1A1E26)] px-4 py-3">
        <p className="text-[12.5px] text-[#E0A254]">Unknown protocol slug: {assignment.protocol_slug}</p>
      ...
      <div className="flex items-start justify-between gap-3 px-4 py-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-[#FAFAF8]">{protocol.name}</span>
            <span className={`text-[10px] font-bold uppercase tracking-widest ${statusColour}`}>{assignment.status}</span>
          </div>
          <p className="text-[11px] text-[#8A9099] mt-0.5">{protocol.dosing.frequency} · {protocol.dosing.duration}</p>
