...
export default async function CoachMealLogPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!plan) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-8">
        <Link href={`/dashboard/clients/${id}/nutrition`} className="text-[12.5px] font-semibold text-[#FAFAF8] hover:text-[#242932]">← Back to nutrition</Link>
        <div className="mt-6 rounded-xl border border-[#2A2F39] bg-[#14171D] p-6 text-center">
          <p className="text-sm text-[#8A9099]">No active nutrition plan for {firstName} yet. Publish a plan first.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      <Link href={backHref} className="text-[12.5px] font-semibold text-[#FAFAF8] hover:text-[#242932]">← Back to nutrition</Link>
      <div className="mt-4 br-page-header sticky top-0 z-20 mb-7 pt-4 pb-3.5 border-b border-[#2A2F39] bg-[#14171D]/[0.88] backdrop-blur-md print:static print:bg-transparent">
        <p className="text-[10px] font-medium text-[#676D76] mb-1">Log meals · {client.name}</p>
        <h1 className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em] tracking-tight">{plan.plan_name}</h1>
      </div>
      <MealLogClient
        token=""
        clientId={client.id}
