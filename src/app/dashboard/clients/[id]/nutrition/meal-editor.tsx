...
export default function MealEditor({ planId, meal, proteinAnchor, siblingMeals, mealIndex, changedFoods }: Props) {
    ...
    return (
      <div className="bg-[#14171D] br-card overflow-hidden">
        <div className="px-5 py-3 border-b border-[#2A2F39] flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-[#FAFAF8] text-sm">{meal.meal_name}</h3>
            <p className="text-[10px] text-[#676D76] mt-0.5">{meal.timing}</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <p className="text-sm font-semibold text-[#FAFAF8] tabular-nums">{view.kcal} kcal</p>
              <p className="text-[10px] text-[#8A9099] mt-0.5 tabular-nums">
                {meal.protein_g}g P · {meal.carb_g}g C · {meal.fat_g}g F
              </p>
              <p className="text-[10px] text-[#676D76] mt-0.5 tabular-nums">
                P {pp}% · C {cp}% · F {fp}%
              </p>
            </div>
            <button
              onClick={() => setEditing(true)}
              className="text-[10px] font-semibold px-2.5 py-1 border border-[#2A2F39] text-[#8A9099] rounded-md hover:border-[#FAFAF8] hover:text-[#FAFAF8] transition-colors"
            >
              Edit
            </button>
          </div>
        ...
        <div className="px-5 py-4">
            ...
            {meal.foods.map((food, i) => {
              ...
              const macro = (field: string, label: string, value: number | null) =>
                changedFields.has(field)
                  ? <span className="text-[#D4817E] font-semibold"><span className="text-[10px]">{label}</span> {value}g</span>
                  : <span className="text-[#8A9099]"><span className="text-[10px] text-[#676D76]">{label}</span> {value}g</span>
              return (
                <div key={i} className={`flex items-start justify-between gap-3${wasChanged ? ' bg-[#1A1214] -mx-2 px-2 py-1 rounded' : ''}`}>
                  <div className="flex items-start gap-2 flex-1 min-w-0">
                    <span className={`mt-0.5 shrink-0 ${wasChanged ? 'text-[#D4817E]' : 'text-[#676D76]'}`}>•</span>
                    <p className="text-sm text-[#FAFAF8]">
                      {f.name}
                      {wasChanged && (
                        <span className="ml-2 text-[10px] font-semibold text-[#D4817E]">changed</span>
                      )}
                    </p>
                  </div>
                  {f.protein_g !== null && (
                    <div className="flex items-baseline gap-3 shrink-0 tabular-nums text-[12.5px] pt-0.5">
                      ...
                      <span className="text-[#8A9099] font-medium w-[60px] text-right">{f.kcal} kcal</span>
                    </div>
                  )}
                </div>
              )
          ...
          {meal.notes && (
            <p className="text-[12.5px] text-[#676D76] italic mt-2">{meal.notes}</p>
          )}
        </div>
      </div>
    )
  ...
  return (
    <div className="bg-[#14171D] border-2 border-[#FAFAF8] rounded-xl overflow-hidden">
      <div className="px-5 py-3 border-b border-[#2A2F39] bg-[rgba(27,109,252,0.08)]/40 flex items-center justify-between gap-4">
        <div className="flex-1 min-w-0 space-y-1">
          <input
            type="text"
            value={draftName}
            onChange={e => setDraftName(e.target.value)}
            className="w-full font-semibold text-[#FAFAF8] text-sm bg-[#14171D] border border-[#2A2F39] rounded px-2 py-1 focus:outline-none focus:border-[#FAFAF8]"
          ...
          <input
            ...
            className="w-full text-[10px] text-[#8A9099] bg-[#14171D] border border-[#2A2F39] rounded px-2 py-1 focus:outline-none focus:border-[#FAFAF8]"
            placeholder="Timing (e.g. 07:00–08:00)"
          />
        </div>
        <div className="text-right shrink-0">
          <p className="text-sm font-semibold text-[#FAFAF8] tabular-nums">{mealKcal} kcal</p>
          <p className="text-[10px] text-[#8A9099] mt-0.5 tabular-nums">
      ...
      <div className="px-5 py-4">
            ...
            <div key={i} className="flex items-start gap-2 group">
              <div className="flex-1 space-y-1.5">
                <input
                  ...
                  className="w-full text-sm text-[#FAFAF8] bg-[#0B0D10] border border-[#2A2F39] rounded px-2 py-1 focus:outline-none focus:border-[#FAFAF8]"
                />
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="text-[10px] text-[#8A9099] bg-[#14171D] border border-[#2A2F39] rounded px-1.5 py-0.5 hover:border-[#FAFAF8] focus:outline-none focus:border-[#FAFAF8] max-w-[220px]"
                  >
                  ...
                  {lookupFood(f.name) && (
                    <span className="text-[10px] text-[#6FA98B] bg-[#14171D] px-1.5 py-0.5 rounded">In table</span>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                <label className="flex items-center gap-1 text-[10px] text-[#676D76] tabular-nums">
                  P
                  <input
                    type="number"
                    value={f.protein_g}
                    onChange={e => updateFood(i, { protein_g: Number(e.target.value) })}
                    className="w-14 text-[12.5px] text-[#FAFAF8] bg-[#0B0D10] border border-[#2A2F39] rounded px-1.5 py-1 focus:outline-none focus:border-[#FAFAF8]"
                    step="0.1"
                    min="0"
                  />g
                </label>
                ...
                <div className="w-[60px] text-right text-[12.5px] font-medium text-[#FAFAF8] tabular-nums pt-1">
                  {kcalFromMacros(Number(f.protein_g) || 0, Number(f.carb_g) || 0, Number(f.fat_g) || 0)} kcal
                </div>
                <button
                  onClick={() => deleteFood(i)}
                  className="text-[11px] text-[#676D76] hover:text-[#D4817E] transition-colors px-1"
                  title="Delete food"
                >
                  ✕
                </button>
        ...
        <button
          onClick={addFood}
          className="mt-3 text-[12.5px] font-semibold px-3 py-1.5 border border-dashed border-[#2A2F39] text-[#8A9099] rounded hover:border-[#FAFAF8] hover:text-[#FAFAF8] transition-colors"
        >
          + Add food
        </button>

        <div className="mt-4">
          <label className="text-[11px] font-medium text-[#8A9099] mb-1 block">Meal notes</label>
          <textarea
            value={draftNotes}
            onChange={e => setDraftNotes(e.target.value)}
            placeholder="Optional preparation / execution note for this meal"
            rows={2}
            className="w-full text-[12.5px] text-[#FAFAF8] bg-[#0B0D10] border border-[#2A2F39] rounded px-2 py-1.5 focus:outline-none focus:border-[#FAFAF8]"
          />
        </div>

        <div className="mt-4 pt-3 border-t border-[#2A2F39] flex items-center justify-between">
          <div className="text-[11px] text-[#8A9099]">
            Daily preview:{' '}
            <span className="font-semibold text-[#FAFAF8] tabular-nums">
            ...
            <span className="tabular-nums">
              ...
              {proteinAnchor ? (
                <span className={`ml-1 text-[10px] ${Math.abs(dailyPreview.protein_g - proteinAnchor) > 20 ? 'text-[#E0A254]' : 'text-[#6FA98B]'}`}>
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={cancel}
              disabled={pending}
              className="text-[12.5px] font-semibold px-3 py-1.5 text-[#8A9099] rounded hover:text-[#FAFAF8] transition-colors disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              onClick={save}
              disabled={pending}
              className="text-[12.5px] font-semibold px-4 py-1.5 bg-[#FAFAF8] text-[#0B0D10] rounded hover:bg-[#FFFFFF] transition-colors disabled:opacity-50"
            >
        ...
        {error && (
          <p className="mt-2 text-[11px] text-[#D4817E]">{error}</p>
        )}
      </div>
    </div>
  )
