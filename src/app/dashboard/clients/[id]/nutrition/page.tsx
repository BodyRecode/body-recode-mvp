...
const carbColour: Record<string, string> = {
  low: 'text-[#FFFFFF] bg-[rgba(27,109,252,0.08)] border-[#2A2F39]',
  moderate: 'text-[#C2C6CC] bg-[#1A1E26]/10 border-[#2A2F39]/30',
  high: 'text-[#C2C6CC] bg-[#1A1E26]/10 border-[#2A2F39]/30',
}
    ...
    <div className="space-y-4">
      ...
      <div id={`${idPrefix}identity`} className="scroll-mt-8 bg-[#14171D] br-card p-5">
        <div className="flex items-start justify-between mb-3">
          <div>
            <h2 className="text-lg font-semibold text-[#FAFAF8]">{plan.plan_name}</h2>
          ...
          <div className="flex gap-1.5 flex-wrap justify-end">
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border capitalize ${entryStateColour[plan.entry_state] || 'text-[#8A9099] bg-[#1A1E26] border-[#2A2F39]'}`}>
              {plan.entry_state.replace(/_/g, ' ')}
            </span>
            <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border capitalize ${carbColour[plan.carb_demand_level] || 'text-[#8A9099] bg-[#1A1E26] border-[#2A2F39]'}`}>
              {plan.carb_demand_level} carbs
            </span>
          </div>
        </div>
        {plan.estimated_calorie_band && (
          <p className="text-[12.5px] text-[#8A9099]">~{plan.estimated_calorie_band}</p>
        )}
        ...
        <p className="text-[12.5px] text-[#676D76] mt-3">
          ...
          {plan.doctrine_version && (
            <span className="text-[#676D76]"> · Doctrine v{plan.doctrine_version}</span>
          )}
        </p>
      </div>
      ...
      {plan.doctrine_version && isDoctrineStale(plan.doctrine_version, 'nutrition_plan') && (
        <div className="scroll-mt-8 bg-[#14171D] br-card p-5">
          <div className="flex items-start gap-3">
            <svg className="w-5 h-5 text-[#8A9099] shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div className="min-w-0 flex-1">
              <p className="text-[12.5px] font-medium text-[#8A9099] mb-1">Doctrine update available</p>
              <p className="text-sm text-[#FAFAF8] leading-relaxed">
                This plan was generated under <span className="font-mono text-[#FAFAF8]">v{plan.doctrine_version}</span>. Current doctrine is <span className="font-mono text-[#FAFAF8]">v{DOCTRINE_VERSIONS.nutrition_plan}</span>. Regenerating will apply the latest validator rules (e.g. tightened appetite-suppression caps, bridge-mode behaviour, carb-demand mapping). Existing plan stays valid until you regenerate.
              </p>
              <Link
                href={`/dashboard/clients/${plan.client_id}/nutrition/suggest`}
                className="inline-block mt-2 text-[12.5px] font-semibold text-[#FFFFFF] hover:text-[#FFFFFF]"
              >
      ...
      {plan.transitional_override_active && (() => {
        ...
        const nextStageKcal = gapKcal && gapKcal > 0
          ...
          : startKcal
        return (
          <div id={`${idPrefix}bridge-mode`} className="scroll-mt-8 bg-[linear-gradient(180deg,#1A1E26,#1A1E26)] border border-[#4A3A22] rounded-xl p-5">
            <div className="flex items-start gap-3 mb-4">
              <svg className="w-5 h-5 text-[#E0A254] shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
              </svg>
              <div className="min-w-0 flex-1">
                <p className="text-[11px] font-black text-[#E0A254] mb-1">Bridge Mode (Staged Ramp)</p>
                <p className="text-sm text-[#FAFAF8] leading-relaxed">
            ...
            {targetKcal && (
              <div className="pl-8 mb-4">
                <div className="flex items-baseline justify-between text-[12.5px] mb-1.5">
                  <span className="text-[#8A9099]"><span className="font-mono tabular-nums">{startKcal}</span> current stage</span>
                  <span className="text-[#676D76] text-[10px]">
                  ...
                  <span className="text-[#8A9099]"><span className="font-mono tabular-nums">{targetKcal}</span> target</span>
                </div>
                <div className="h-2 bg-[#1A1E26] rounded-full overflow-hidden">
                  <div className="h-full bg-[#E0A254] rounded-full transition-all" style={{ width: `${progressPct}%` }} />
                </div>
            ...
            {bridgeReadinessSignal && (
              <div className="pl-8 mb-4">
                <p className="text-[10px] font-medium text-[#E0A254] mb-1.5">Step-up readiness (from recent check-ins)</p>
                {bridgeReadinessSignal.ready ? (
                  <div className="bg-[#14171D] border border-[#2A2F39] rounded-lg px-3 py-2.5">
                    <div className="flex items-start gap-2">
                      <span className="text-[#6FA98B] mt-0.5">✓</span>
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-semibold text-[#C2C6CC]">Ready to step up</p>
                        <p className="text-[12.5px] text-[#6FA98B] mt-1 leading-relaxed">{bridgeReadinessSignal.reason}</p>
                        <Link
                          href={`/dashboard/clients/${plan.client_id}/nutrition/suggest`}
                          className="inline-block mt-2 text-[12.5px] font-semibold text-[#C2C6CC] hover:text-[#C2C6CC] underline"
                        >
                  ...
                  <div className="bg-[#0B0D10] border border-[#2A2F39] rounded-lg px-3 py-2.5">
                    <div className="flex items-start gap-2">
                      <span className="text-[#8A9099] mt-0.5">·</span>
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-semibold text-[#FAFAF8]">Hold the current bridge</p>
                        <p className="text-[12.5px] text-[#8A9099] mt-1 leading-relaxed">{bridgeReadinessSignal.reason}</p>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {plan.transitional_override_justification && (
              <div className="pl-8 mb-4">
                <p className="text-[10px] font-medium text-[#E0A254] mb-1">Coach justification</p>
                <p className="text-[12.5px] text-[#FAFAF8] leading-relaxed italic">{plan.transitional_override_justification}</p>
              </div>
            )}

            {expiry && (
              <div className="pl-8 pt-3 border-t border-[#4A3A22]">
                <p className="text-[12.5px] text-[#8A9099]">
                  <span className="font-semibold text-[#E0A254]">Bridge window expires:</span>{' '}
      ...
      {plan.weekly_structure_notes && (() => {
        ...
        return (
          <div id={`${idPrefix}structure`} className="scroll-mt-8 bg-[#14171D] br-card overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-3 border-b border-[#2A2F39]">
              <span className="text-[11px] font-black text-[#FAFAF8]">01</span>
              <p className="text-[10px] font-medium text-[#8A9099]">Structure Logic</p>
            </div>
      ...
      {(() => {
        ...
        return (
          <div id={`${idPrefix}daily-totals`} className="scroll-mt-8 bg-[#14171D] br-card overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-3 border-b border-[#2A2F39]">
              <span className="text-[11px] font-black text-[#FAFAF8]">↑</span>
              <p className="text-[10px] font-medium text-[#8A9099]">Daily Totals (sum of meals)</p>
            </div>
            <div className="px-5 py-4 space-y-3">
              <div className="flex items-baseline justify-between">
                <div>
                  <p className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em] tabular-nums">{totals.kcal.toLocaleString()} <span className="text-sm font-normal text-[#8A9099]">kcal</span></p>
                  <p className="text-[12.5px] text-[#8A9099] mt-1 tabular-nums">
                    {totals.protein_g}g P · {totals.carb_g}g C · {totals.fat_g}g F
                  </p>
                </div>
                <div className="text-right text-[12.5px]">
                  {band ? (
                    <div className={kcalInBand ? 'text-[#FAFAF8]' : 'text-[#D4817E]'}>
                      <p className="font-semibold">{kcalInBand ? 'Inside band' : 'Outside band'}</p>
                      <p className="text-[#8A9099] mt-0.5 tabular-nums">Target {band.low}–{band.high} kcal</p>
                    </div>
                  ) : (
                    <p className="text-[#8A9099]">No band stated</p>
                  )}
                </div>
              </div>
              <div className="pt-1">
                <div className="flex h-2 rounded-full overflow-hidden bg-[#1A1E26]">
                  <div style={{ width: `${proteinPct}%` }} className="bg-[#FAFAF8]" />
                  <div style={{ width: `${carbPct}%` }} className="bg-[#E0A254]" />
                  <div style={{ width: `${fatPct}%` }} className="bg-[#1A1E26]" />
                </div>
                <div className="flex justify-between mt-2 text-[10px] tabular-nums">
                  <span className="text-[#FAFAF8]">P {proteinPct}%</span>
                  <span className="text-[#E0A254]">C {carbPct}%</span>
                  <span className="text-[#C2C6CC]">F {fatPct}%</span>
                </div>
              </div>
              {proteinAnchor > 0 && (
                <div className="flex items-center justify-between text-[12.5px] pt-3 border-t border-[#2A2F39]">
                  <p className="text-[#8A9099]">Protein anchor</p>
                  <p className={proteinOk ? 'text-[#FAFAF8]' : 'text-[#D4817E]'}>
                    <span className="tabular-nums">{totals.protein_g}g</span> vs anchor <span className="tabular-nums">{proteinAnchor}g</span>
                    {proteinDelta !== null && (
                      <span className="text-[#8A9099] ml-1">({proteinDelta > 0 ? '+' : ''}{proteinDelta}g)</span>
                    )}
                  </p>
                </div>
              )}
      ...
      <div id={`${idPrefix}meals`} className="scroll-mt-8">
        <p className="text-[12.5px] font-medium text-[#8A9099] mb-3 px-1">Meal Structure</p>
        {/* What the last adjustment did. Kade, 1 Sep 2026: adjustments change
            only the named thing, and the change must be easy to spot. */}
        {lastStep && (
          <div className="mb-3 rounded-lg border border-[#D4817E] bg-[#1A1214] px-4 py-3">
            <p className="text-[12.5px] font-semibold text-[#D4817E]">{lastStep.summary}</p>
            <p className="text-[11px] text-[#D4817E] opacity-80 mt-0.5">
            ...
            {Array.isArray(lastStep.notes) && lastStep.notes.length > 0 && (
              <ul className="mt-1.5 space-y-0.5">
                {lastStep.notes.map((n: string, i: number) => (
                  <li key={i} className="text-[11px] text-[#D4817E] opacity-80">{n}</li>
                ))}
              </ul>
            )}
          </div>
      ...
      {plan.training_day_adjustments && (
        <div id={`${idPrefix}adjustments`} className="scroll-mt-8 bg-[#14171D] br-card overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-3 border-b border-[#2A2F39]">
            <span className="text-[11px] font-black text-[#FAFAF8]">02</span>
            <p className="text-[10px] font-medium text-[#8A9099]">Training Day Adjustments</p>
          </div>
          <div className="px-5 py-4 space-y-2">
            <div className="flex gap-4 text-sm">
              <span className="text-[#8A9099]">+{plan.training_day_adjustments.carb_increase_g}g carbs</span>
              {plan.training_day_adjustments.fat_reduction_g > 0 && (
                <span className="text-[#8A9099]">−{plan.training_day_adjustments.fat_reduction_g}g fat</span>
              )}
            </div>
            {plan.training_day_adjustments.timing_note && (
              <p className="text-sm text-[#FAFAF8]">{clean(plan.training_day_adjustments.timing_note)}</p>
            )}
            {plan.training_day_adjustments.meals_affected?.length > 0 && (
              <p className="text-[12.5px] text-[#8A9099]">Applies to: {plan.training_day_adjustments.meals_affected.join(', ')}</p>
            )}
          </div>
        </div>
      )}
      ...
      {plan.execution_rules?.length > 0 && (
        <div id={`${idPrefix}execution`} className="scroll-mt-8 bg-[#14171D] br-card overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-3 border-b border-[#2A2F39]">
            <span className="text-[11px] font-black text-[#FAFAF8]">03</span>
            <p className="text-[10px] font-medium text-[#8A9099]">Execution Rules</p>
          </div>
          <div className="px-5 py-4 space-y-2">
            {plan.execution_rules.map((rule, i) => (
              <div key={i} className="flex items-start gap-2">
                <span className="text-[#FAFAF8] mt-0.5 shrink-0">•</span>
                <p className="text-sm text-[#FAFAF8]">{clean(rule)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {plan.what_not_to_change?.length > 0 && (
        <div className="bg-[#1A1E26]/40 br-card px-5 py-4">
          <p className="text-[10px] font-medium text-[#8A9099] mb-2">What Not to Change</p>
          <div className="space-y-1.5">
            {plan.what_not_to_change.map((item, i) => (
              <div key={i} className="flex items-start gap-2">
                <span className="text-[#676D76] mt-0.5 shrink-0">-</span>
                <p className="text-[12.5px] text-[#8A9099]">{clean(item)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {plan.progression_notes && (() => {
        ...
        return (
          <div id={`${idPrefix}progression`} className="scroll-mt-8 bg-[#14171D] br-card overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-3 border-b border-[#2A2F39]">
              <span className="text-[11px] font-black text-[#FAFAF8]">04</span>
              <p className="text-[10px] font-medium text-[#8A9099]">Progression Notes</p>
            </div>
            <div className="px-5 py-4 space-y-2">
              {intro && <p className="text-sm text-[#FAFAF8] leading-relaxed">{intro}</p>}
              {points.length > 1 ? points.map((point, i) => (
      ...
      {plan.substitution_options && (
        <div id={`${idPrefix}substitutions`} className="scroll-mt-8 bg-[#14171D] br-card p-5">
          <p className="text-[10px] font-medium text-[#8A9099] mb-3">Food Substitutions</p>
          <div className="grid grid-cols-3 gap-4">
            {(['protein', 'carbohydrate', 'fat'] as const).map(cat => (
              plan.substitution_options![cat]?.length > 0 && (
                <div key={cat}>
                  <p className="text-[10px] font-medium text-[#676D76] mb-2 capitalize">{cat}</p>
                  <div className="space-y-1">
                    {plan.substitution_options![cat].map((item, i) => (
                      <p key={i} className="text-[12.5px] text-[#8A9099]">• {clean(item)}</p>
                    ))}
                  </div>
                </div>
              )
  ...
  return (
    <div className="w-full">
      ...
      {activePlan && mealAdherence && mealAdherence.days > 0 && (
        <div
          className="mb-8 rounded-xl border border-[#2A2F39] p-4"
          style={{
        ...
        >
          <div className="flex items-center justify-between mb-2.5">
            <p className="text-[12.5px] text-[#8A9099]">Meal adherence, last 7 days</p>
            <span className="text-[13.5px] font-semibold text-[#FAFAF8] tabular-nums">{mealAdherence.pct}% on plan</span>
          </div>
          <div className="flex h-2 rounded-full overflow-hidden bg-[#1A1E26]">
            <div className="bg-[#FAFAF8]" style={{ width: `${mealAdherence.total ? (mealAdherence.ate / mealAdherence.total) * 100 : 0}%` }} />
            <div className="bg-[#E0A254]" style={{ width: `${mealAdherence.total ? (mealAdherence.swapped / mealAdherence.total) * 100 : 0}%` }} />
            <div className="bg-[#2A2F39]" style={{ width: `${mealAdherence.total ? (mealAdherence.skipped / mealAdherence.total) * 100 : 0}%` }} />
          </div>
          <div className="flex gap-4 mt-2.5 text-[12.5px] text-[#8A9099]">
            <span><span className="font-semibold text-[#FAFAF8] tabular-nums">{mealAdherence.ate}</span> ate</span>
            <span><span className="font-semibold text-[#E0A254] tabular-nums">{mealAdherence.swapped}</span> swapped</span>
            <span><span className="font-semibold text-[#8A9099] tabular-nums">{mealAdherence.skipped}</span> skipped</span>
            <span className="ml-auto text-[#676D76]">{mealAdherence.days} day{mealAdherence.days === 1 ? '' : 's'} logged</span>
          </div>
        </div>
      )}
      ...
      {activePlan?.current_direction === 'rebuild' && (
        <div className="mb-4 flex items-start gap-3 bg-[#1A1214] border border-[#4A2222]/60 rounded-xl px-4 py-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-[#D4817E]">Client is struggling with nutrition</p>
            <p className="text-[12.5px] text-[#D4817E]/70 mt-0.5">Latest check-in direction is Rebuild. Consider adjusting the plan or generating a new one.</p>
          </div>
          <Link href="./nutrition/suggest" className="text-[12.5px] font-semibold text-[#D4817E] hover:text-[#D4817E] shrink-0 mt-0.5">Regenerate →</Link>
        </div>
      )}
      ...
      {activePlan ? (
        <div>
          {draftPlan && (
            <div className="flex items-center gap-3 mb-4 mt-2">
              ...
              <p className="text-[12.5px] text-[#676D76]">Current Active Plan</p>
              <div className="flex-1 h-px bg-[#1A1E26]" />
            </div>
          )}
          ...
          {archivedPlans.length > 0 && (
            <div className="mt-6">
              <p className="text-[#8A9099] text-sm mb-3">Previous Plans ({archivedPlans.length})</p>
              <div className="space-y-2">
                {archivedPlans.map(p => (
                  <div key={p.id} className="bg-[#14171D]/50 border border-[#2A2F39] rounded-lg px-4 py-3 flex items-center justify-between">
                    <span className="text-sm text-[#8A9099] opacity-70">{p.plan_name}</span>
        ...
        <div className="text-center py-16 border-2 border-dashed border-[#2A2F39] rounded-xl">
          <p className="text-[#8A9099] mb-4">No nutrition plan generated yet.</p>
          <Link
            href={`/dashboard/clients/${id}/nutrition/suggest`}
            className="text-[12.5px] font-medium px-3 py-1.5 border border-[#2A2F39] text-[#8A9099] rounded-lg hover:border-[#2A2F39] hover:text-[#FAFAF8] transition-colors"
          >
            Generate Plan
          </Link>
        </div>
