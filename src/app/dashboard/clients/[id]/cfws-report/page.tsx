...
export default async function CFWSReportPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const sections = [
    ...
    { label: 'Tensions & Trade-Offs', content: cfws.weekly_tensions_tradeoffs },
    ...
    <>
      ...
      <div style={{ background: '#000000', padding: '44px 52px 0' }}>
        ...
        <p style={{ fontSize: '12px', fontWeight: 400, color: '#666666', marginBottom: '44px', letterSpacing: '0.02em' }}>
          Week {cfws.week_number} Interpretation · Rolling Window Analysis
        </p>
        ...
        <div style={{ display: 'flex', gap: '0', borderTop: '1px solid #1a1a1a' }}>
          {[
            ...
            <div key={i} style={{ padding: '20px 40px 20px 0', marginRight: '40px', borderRight: i < 3 ? '1px solid #1a1a1a' : 'none', paddingRight: i < 3 ? '40px' : '0' }}>
      ...
      <div style={{ background: PAGE_BG, padding: '48px 52px 64px' }}>
        ...
        <div style={{ background: G, padding: '36px 40px', marginBottom: '40px' }}>
          ...
          <p style={{ fontSize: '17px', fontWeight: 700, color: WHITE, lineHeight: '1.55', marginBottom: '20px', letterSpacing: '-0.01em' }}>
            This document is not a summary. It is a structured interpretation of how this client&apos;s system is behaving this week.
          </p>
          ...
          <p style={{ fontSize: '13px', fontWeight: 400, color: '#888888', lineHeight: '1.9', marginBottom: '12px' }}>
            The Coach-Facing Weekly Synthesis (CFWS) is generated by the Body Recode™ interpretation engine following submission of both weekly check-in forms. It translates training load, recovery state, regulation signals, lifestyle context, and behavioural patterns into a coherent weekly picture.
          </p>
          <p style={{ fontSize: '13px', fontWeight: 400, color: '#888888', lineHeight: '1.9' }}>
            Nothing in this document prescribes, diagnoses, or directs. It is an interpretive tool only. You remain the coaching authority — this document gives you the current signal picture to inform your decisions.
          </p>
        </div>

        {/* Exposure Readiness */}
        <div style={{ background: WHITE, border: `1px solid ${CARD_BORDER}`, padding: '32px 36px', marginBottom: '16px' }}>
