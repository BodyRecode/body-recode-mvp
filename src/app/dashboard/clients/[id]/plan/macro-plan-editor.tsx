...
const statusColour: Record<string, string> = {
  planned: 'text-[#8A9099] bg-[#1A1E26] border-[#2A2F39]',
  in_progress: 'text-[#E0A254] bg-[#1A1E26] border-[#E0A254]',
  complete: 'text-[#C2C6CC] bg-[#1A1E26]/10 border-[#6FA98B]',
  skipped: 'text-[#676D76] bg-[#14171D] border-[#2A2F39]',
}
...
const inputCls = 'w-full bg-[#1A1E26] border border-[#2A2F39] text-[#FAFAF8] rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#FAFAF8] focus:border-transparent'
const labelCls = 'block text-xs font-bold text-[#8A9099] uppercase tracking-wider mb-1.5'
...
export default function MacroPlanEditor({
  ...
  return (
    <div>
      {error && (
        <p className="text-sm text-[#D4817E] bg-[#1A1214] border border-[#4A2222] rounded-md px-3 py-2 mb-4">{error}</p>
      )}

      {/* No plan yet - create one */}
      {!plan ? (
        <div className="bg-[#14171D] br-card p-6">
          <h2 className="text-sm font-semibold text-[#FAFAF8] mb-4">Create Macro Plan</h2>
          <div className="space-y-4">
            ...
            <button
              ...
              disabled={saving}
              className="w-full py-2.5 bg-[#FAFAF8] text-[#0B0D10] font-semibold rounded-md hover:bg-[#FFFFFF] disabled:opacity-40 transition-colors"
            >
        ...
        <div>
          ...
          {/* Replace plan form */}
          {showCreatePlan && (
            <div className="bg-[#14171D] br-card p-5 mb-4">
              <p className="text-[12.5px] font-medium text-[#8A9099] mb-3">Replace Plan</p>
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <button onClick={handleCreatePlan} disabled={saving} className="flex-1 py-2 bg-[#FAFAF8] text-[#0B0D10] font-semibold rounded-md text-sm disabled:opacity-40">{saving ? 'Saving…' : 'Save'}</button>
                  <button onClick={() => setShowCreatePlan(false)} className="px-4 py-2 border border-[#2A2F39] text-[#8A9099] rounded-md text-sm hover:border-[#2A2F39]">Cancel</button>
                </div>
              </div>
            </div>
          )}
          ...
          {plan.plan_blocks.length === 0 ? (
            <div className="text-center py-10 border-2 border-dashed border-[#2A2F39] rounded-xl mb-4">
              <p className="text-[#8A9099] text-sm mb-1">No blocks planned yet</p>
              <p className="text-[#676D76] text-[12.5px]">Add the first meso block to begin building the arc</p>
            </div>
          ) : (
                ...
                <div key={block.id}>
                  ...
                  {i > 0 && (
                    <div className="flex items-center gap-2 px-6 py-1">
                      <div className="w-px h-4 bg-[#2A2F39] mx-auto" />
                    </div>
                  )}
                  ...
                  {editingBlockId === block.id ? (
                    ...
                    <div className={`bg-[#14171D] border rounded-xl p-4 transition-colors ${block.status === 'skipped' ? 'opacity-40 border-[#2A2F39]' : 'border-[#2A2F39]'}`}>
                      ...
                      <div className="flex items-center gap-4 text-[12.5px] text-[#8A9099] mb-2 pl-7">
                      ...
                      {block.notes && (
                        <p className="text-[12.5px] text-[#676D76] italic pl-7 mb-2">{block.notes}</p>
                      )}
                      ...
                      <div className="flex items-center gap-3 pl-7 mt-2">
                        ...
                        <button
                          ...
                          className="text-[12.5px] text-[#676D76] hover:text-[#D4817E] transition-colors"
                        >
                          Remove
                        </button>
                      </div>
          ...
          {showAddBlock ? (
            ...
            <button
              ...
              className="w-full py-3 border-2 border-dashed border-[#2A2F39] text-[#8A9099] rounded-xl text-sm hover:border-[#2A2F39] hover:text-[#FAFAF8] transition-colors"
            >
              + Add Block
            </button>
          )}
...
function BlockAddForm({
  ...
  return (
    <div className="bg-[#14171D] br-card p-5">
      <p className="text-[12.5px] font-medium text-[#8A9099] mb-4">Add Meso Block</p>
      <BlockFormFields form={blockForm} setForm={setBlockForm} />
      <div className="flex gap-2 mt-4">
        <button onClick={onAdd} disabled={saving} className="flex-1 py-2.5 bg-[#FAFAF8] text-[#0B0D10] font-semibold rounded-md text-sm disabled:opacity-40">
          {saving ? 'Adding…' : 'Add Block'}
        </button>
        <button onClick={onCancel} className="px-4 py-2 border border-[#2A2F39] text-[#8A9099] rounded-md text-sm hover:border-[#2A2F39]">
          Cancel
        </button>
      </div>
    </div>
...
function BlockEditForm({
  ...
  return (
    <div className="bg-[#14171D] border border-[#2A2F39] rounded-xl p-5">
      <p className="text-[12.5px] font-medium text-[#8A9099] mb-4">Edit Block</p>
      ...
      <div className="flex gap-2 mt-4">
        <button onClick={() => onSave(form)} disabled={saving} className="flex-1 py-2.5 bg-[#FAFAF8] text-[#0B0D10] font-semibold rounded-md text-sm disabled:opacity-40">
          {saving ? 'Saving…' : 'Save'}
        </button>
        <button onClick={onCancel} className="px-4 py-2 border border-[#2A2F39] text-[#8A9099] rounded-md text-sm hover:border-[#2A2F39]">
          Cancel
