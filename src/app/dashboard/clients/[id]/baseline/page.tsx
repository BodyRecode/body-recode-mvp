      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 text-[#8A9099] text-sm br-page-header sticky top-0 z-20 mb-7 pt-4 pb-3.5 border-b border-[#2A2F39] bg-[#14171D]/[0.88] backdrop-blur-md print:static print:bg-transparent">
            ...
            <span>/</span>
            <span className="text-[#FAFAF8]">Baseline</span>
          </div>
          <h1 className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em]">Baseline</h1>
          <p className="text-sm text-[#8A9099] mt-1">Baseline measurements and progress photos.</p>
        </div>
        {baselineToken && (
          <CopyLinkButton
            token={baselineToken}
      ...
      {latestBaseline ? (
        <div className="space-y-4">
          ...
          <div className="bg-[#14171D] br-card p-5">
            <p className="text-[10px] font-medium text-[#8A9099] mb-4">Measurements</p>
            <div className="grid grid-cols-4 gap-3">
              {[
                ...
                <div key={m.label} className="bg-[#1A1E26]/50 rounded-xl p-3 text-center">
                  <p className="text-[12.5px] text-[#8A9099] mb-1">{m.label}</p>
                  <p className="text-base font-semibold text-[#FAFAF8]">
                    {m.value ?? '-'}
                    <span className="text-[12.5px] text-[#8A9099] ml-1">{m.unit}</span>
                  </p>
                </div>
              ))}
            </div>
          ...
          {(baselinePhotos.front || baselinePhotos.side || baselinePhotos.back) && (
            <div className="bg-[#14171D] br-card p-5">
              <p className="text-[10px] font-medium text-[#8A9099] mb-4">Progress Photos</p>
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  <div key={photo.label} className="space-y-1.5">
                    <p className="text-[12.5px] text-[#8A9099] text-center">{photo.label}</p>
                    {photo.url ? (
                      ...
                      <div className="w-full aspect-[3/4] bg-[#1A1E26] rounded-xl flex items-center justify-center">
                        <p className="text-[#676D76] text-[12.5px]">No photo</p>
                      </div>
                    )}
                  </div>
                ))}
          ...
          {baselines && baselines.length > 1 && (
            <div>
              <p className="text-[#8A9099] text-sm mb-3">Previous Captures ({baselines.length - 1})</p>
              <div className="space-y-2">
                {baselines.slice(1).map(b => (
                  <div key={b.id} className="bg-[#14171D]/50 border border-[#2A2F39] rounded-lg px-4 py-3 flex items-center justify-between opacity-60">
                    <span className="text-sm text-[#8A9099]">Week {b.re_capture_week} capture</span>
        ...
        <div className="text-center py-16 border-2 border-dashed border-[#2A2F39] rounded-xl">
          <p className="text-[#8A9099] mb-2">No baseline submitted yet.</p>
          <p className="text-[#676D76] text-[12.5px]">Send the client their baseline link to begin.</p>
        </div>
      )}
    </div>
  )
