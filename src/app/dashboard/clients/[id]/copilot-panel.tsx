...
export default function CopilotPanel({
  ...
  return (
    <div className={`br-card overflow-hidden flex flex-col ${className ?? ''}`}>
      <div className="flex items-center gap-3 px-5 py-3 border-b border-[#2A2F39] bg-[#0B0D10] shrink-0">
        <p className="text-[13.5px] font-semibold text-[#FAFAF8] tracking-[-0.015em]">
          Co-Pilot · Doctrine tutor
        </p>
        <span className="ml-auto text-[11px] text-[#676D76]">coach only · you approve every change</span>
        {onClose && (
          <button onClick={onClose} aria-label="Close co-pilot" className="text-[#676D76] hover:text-[#FAFAF8] text-lg leading-none -my-1">✕</button>
        )}
      </div>
      ...
      <button
        ...
        className="flex items-center gap-2 px-5 py-2 border-b border-[#1A1E26] bg-[#14171D] text-[#FAFAF8] hover:bg-[rgba(27,109,252,0.04)] transition-colors shrink-0"
      >
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <circle cx="12" cy="12" r="9" stroke="#FAFAF8" strokeWidth="1.6" />
          <path d="M9.5 9.2a2.5 2.5 0 1 1 3.2 2.4c-.7.25-1.2.9-1.2 1.65v.35" stroke="#FAFAF8" strokeWidth="1.6" strokeLinecap="round" />
          <circle cx="12" cy="16.4" r="1" fill="#FAFAF8" />
        </svg>
        <span className="text-[13.5px] font-semibold">What I can help with</span>
        <ChevronDown size={14} className={`ml-auto text-[#676D76] transition-transform duration-150 ${showHelp ? 'rotate-180' : ''}`} />
      </button>

      <div ref={scrollRef} className="px-5 py-4 space-y-4 flex-1 min-h-0 overflow-y-auto">
        {showHelp && (
          <div className="text-sm text-[#C2C6CC]">
            <p className="mb-3 text-[#FAFAF8] font-semibold">Here for {clientFirstName}. What I can help with:</p>
            <div className="space-y-2.5 mb-3">
              {CAPABILITIES.map(c => (
                <div key={c.title} className="border border-[#1A1E26] rounded-xl px-3.5 py-2.5">
                  <p className="text-[13.5px] font-semibold text-[#FAFAF8] mb-0.5">{c.title}</p>
                  <p className="text-[12.5px] text-[#8A9099] leading-relaxed">{c.body}</p>
                </div>
              ))}
            </div>
            <p className="text-[12.5px] text-[#8A9099] leading-relaxed">I read {clientFirstName}’s file to answer. I never publish anything to the client, and any plan I draft is a draft you review and approve.</p>
          </div>
        )}
        ...
        {messages.map((m, i) => {
          ...
          if (m.role === 'assistant' && m.kind === 'draft-proposal' && m.draft) {
            ...
            return (
              <div key={i} className="border border-[#2A2F39] bg-[rgba(27,109,252,0.04)] rounded-xl px-4 py-3.5">
                <p className="text-[12.5px] font-medium text-[#FAFAF8] mb-2.5">Proposed program draft</p>
                <label className="block text-[11px] font-medium text-[#8A9099] mb-1">Block name</label>
                <input
                  value={d.block_name}
                  onChange={e => updateDraft(i, { block_name: e.target.value })}
                  disabled={gen}
                  className="w-full text-sm border border-[#2A2F39] rounded-lg px-2.5 py-1.5 mb-3 bg-[#14171D] focus:outline-none focus:border-[#2A2F39] focus:ring-[3px] focus:ring-[rgba(27,109,252,0.13)] disabled:opacity-60"
                />
                <div className="space-y-2 mb-3">
                  {rows.map(([label, val, reason]) => (
                    <div key={label}>
                      <div className="flex items-baseline justify-between gap-3">
                        <span className="text-[12.5px] text-[#8A9099]">{label}</span>
                        <span className="text-[13.5px] font-semibold text-[#FAFAF8] text-right">{val}</span>
                      </div>
                      {reason && <p className="text-[11px] text-[#8A9099] leading-snug mt-0.5">{reason}</p>}
                    </div>
                  ))}
                </div>
                <p className="text-[11px] text-[#8A9099] leading-snug mb-3">Equipment defaults to barbell, dumbbell, bodyweight. Change it on the full generator if this client differs. Generating creates a <strong>draft</strong> only. Nothing reaches {clientFirstName} until you publish it.</p>
                {gen ? (
                  <div className="flex items-center gap-2 text-[13.5px] text-[#FAFAF8]">
                    <span className="w-2 h-2 rounded-full bg-[#FAFAF8] animate-bounce motion-reduce:animate-none" />
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      disabled={busy}
                      className="text-[13.5px] font-semibold px-3.5 py-1.5 text-[#FAFAF8] rounded-lg border border-[#FFFFFF] bg-[linear-gradient(180deg,#242932,#FAFAF8)] hover:bg-[linear-gradient(180deg,#242932,#FFFFFF)] shadow-[0_1px_2px_rgba(27,109,252,0.4),inset_0_1px_0_rgba(255,255,255,0.28)] transition-all active:translate-y-[0.5px] disabled:opacity-40"
                    >
                      Generate this draft
                    </button>
                    <button
                      ...
                      disabled={busy}
                      className="text-[13.5px] px-3 py-1.5 text-[#8A9099] hover:text-[#FAFAF8] transition-colors disabled:opacity-40"
                    >
                      Dismiss
                    </button>
                  </div>
          ...
          if (m.role === 'assistant' && m.kind === 'draft-done' && m.draft) {
            return (
              <div key={i} className="border border-[#1A1E26] bg-[rgba(34,160,84,0.06)] rounded-xl px-4 py-3.5">
                <p className="text-[13.5px] font-semibold text-[#FAFAF8] mb-1">Draft created: {m.draft.block_name}</p>
                <p className="text-[12.5px] text-[#8A9099] leading-relaxed mb-2.5">It’s saved as a draft (not live). Open the program page to review every session, edit if needed, and publish when you’re happy.</p>
                <a
                  href={`/dashboard/clients/${clientId}/program`}
                  className="inline-block text-[13.5px] font-semibold px-3.5 py-1.5 bg-[#FAFAF8] text-[#0B0D10] rounded-lg hover:bg-black transition-colors"
                >
                  Review the draft
                </a>
              </div>
          ...
          if (m.role === 'assistant' && m.kind === 'nutrition-proposal' && m.nutrition) {
            ...
            return (
              <div key={i} className="border border-[#2A2F39] bg-[rgba(27,109,252,0.04)] rounded-xl px-4 py-3.5">
                <p className="text-[12.5px] font-medium text-[#FAFAF8] mb-2.5">Proposed nutrition draft</p>
                <label className="block text-[11px] font-medium text-[#8A9099] mb-1">Plan name</label>
                <input
                  value={n.plan_name}
                ...
                <p className="text-[11px] text-[#8A9099] leading-snug mb-3">Generating builds the meals against the protein anchor and calorie floor, and creates a <strong>draft</strong> only. Nothing reaches {clientFirstName} until you publish it.</p>
                {gen ? (
                  <div className="flex items-center gap-2 text-[13.5px] text-[#FAFAF8]">
                    <span className="w-2 h-2 rounded-full bg-[#FAFAF8] animate-bounce motion-reduce:animate-none" />
          ...
          if (m.role === 'assistant' && m.kind === 'nutrition-done' && m.nutrition) {
            return (
              <div key={i} className="border border-[#1A1E26] bg-[rgba(34,160,84,0.06)] rounded-xl px-4 py-3.5">
                <p className="text-[13.5px] font-semibold text-[#FAFAF8] mb-1">Nutrition draft created: {m.nutrition.plan_name}</p>
                <p className="text-[12.5px] text-[#8A9099] leading-relaxed mb-2.5">It’s saved as a draft (not live). Open the nutrition page to review the meals, edit if needed, and publish when you’re happy.</p>
                <a
          ...
          if (m.role === 'assistant' && m.kind === 'edit-proposal' && m.edit) {
            ...
            return (
              <div key={i} className="border border-[#4A3A22] bg-[rgba(180,120,20,0.05)] rounded-xl px-4 py-3.5">
                <p className="text-[12.5px] font-medium text-[#E0A254] mb-2">Proposed change</p>
                <p className="text-[13.5px] text-[#FAFAF8] leading-relaxed mb-3 whitespace-pre-wrap">{m.edit.summary || 'Apply this change to the draft.'}</p>
                <p className="text-[11px] text-[#8A9099] leading-snug mb-3">Only this changes. The rest of the draft stays exactly as it is, and it stays a draft until you publish it.</p>
                {applying ? (
                  <div className="flex items-center gap-2 text-[13.5px] text-[#E0A254]">
                    <span className="w-2 h-2 rounded-full bg-[#E0A254] animate-bounce motion-reduce:animate-none" />
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => applyEdit(i)}
                      disabled={busy}
                      className="text-[13.5px] font-semibold px-3.5 py-1.5 bg-[#E0A254] text-[#FAFAF8] rounded-lg hover:bg-[#E0A254] transition-colors disabled:opacity-40"
                    >
                      Apply this change
                    </button>
                    <button
          ...
          if (m.role === 'assistant' && m.kind === 'edit-done') {
            ...
            return (
              <div key={i} className="border border-[#1A1E26] bg-[rgba(34,160,84,0.06)] rounded-xl px-4 py-3.5">
                <p className="text-[13.5px] font-semibold text-[#FAFAF8] mb-1">Change applied to the {nutritionEdit ? 'nutrition draft' : 'draft'}</p>
                {m.content && <p className="text-[12.5px] text-[#8A9099] leading-relaxed mb-2.5">{m.content}</p>}
                <a
          ...
          return (
            <div key={i} className={m.role === 'user' ? 'flex justify-end' : ''}>
              <div className={m.role === 'user'
                ? 'max-w-[85%] bg-[linear-gradient(180deg,#242932,#FAFAF8)] text-[#FAFAF8] rounded-xl rounded-br-sm px-4 py-2.5 text-sm leading-relaxed shadow-[0_1px_2px_rgba(27,109,252,0.35)]'
                : 'max-w-[92%] bg-[#14171D] border border-[#2A2F39] text-[#FAFAF8] rounded-xl rounded-bl-sm px-4 py-3 text-sm leading-relaxed'}>
                <p className="whitespace-pre-wrap">{m.content}</p>
                {m.role === 'assistant' && m.id && (
                  <button
                    onClick={() => toggleFlag(i)}
                    title={m.flagged ? 'Flagged for review — click to unflag' : 'Flag this answer for review'}
                    className={`mt-2 inline-flex items-center gap-1 text-[11px] ${m.flagged ? 'text-[#E0A254] font-semibold' : 'text-[#676D76] hover:text-[#8A9099]'} transition-colors`}
                  >
        ...
        {(loading || proposing || proposingNutrition || proposingEdit) && (
          <div className="bg-[#14171D] br-card rounded-bl-sm px-4 py-3 inline-flex items-center gap-1.5 w-fit" aria-label="Co-pilot is working">
            <span className="w-2 h-2 rounded-full bg-[#676D76] animate-bounce motion-reduce:animate-none" style={{ animationDelay: '0ms' }} />
            <span className="w-2 h-2 rounded-full bg-[#676D76] animate-bounce motion-reduce:animate-none" style={{ animationDelay: '150ms' }} />
            <span className="w-2 h-2 rounded-full bg-[#676D76] animate-bounce motion-reduce:animate-none" style={{ animationDelay: '300ms' }} />
          </div>
        )}
        ...
        {!busy && (() => {
          ...
          return (
            <div className="flex flex-col gap-1.5 pt-1">
              <p className="text-[11px] text-[#676D76]">Follow up</p>
              {last.followups.map((f, i) => (
                <button
                  key={i}
                  onClick={() => send(f)}
                  className="text-left text-[13.5px] text-[#FAFAF8] border border-[#2A2F39] bg-[rgba(27,109,252,0.05)] hover:bg-[rgba(27,109,252,0.1)] rounded-lg px-3 py-1.5 transition-colors"
                >
                  {f}
                </button>
              ))}
        ...
        {error && (
          <div className="text-[13.5px] text-[#D4817E]">{error}</div>
        )}
      </div>
      ...
      <div className="px-3 pt-2 shrink-0 space-y-2">
        <div className="flex gap-2">
          <button
            ...
            disabled={busy}
            className="flex-1 text-[13.5px] font-medium px-3 py-2 border border-[#2A2F39] text-[#FAFAF8] rounded-xl hover:bg-[rgba(27,109,252,0.05)] transition-colors disabled:opacity-40"
          >
            ＋ Draft a program
          </button>
          <button
            ...
            className={`flex-1 text-[13.5px] font-medium px-3 py-2 border rounded-xl transition-colors disabled:opacity-40 ${
              editMode === 'program'
                ? 'border-[#4A3A22] bg-[rgba(180,120,20,0.08)] text-[#E0A254]'
                : 'border-[#4A3A22] text-[#E0A254] hover:bg-[rgba(180,120,20,0.05)]'
            }`}
          >
            ✎ Refine program
          </button>
      ...
      {editMode && (
        <p className="px-4 pt-1.5 text-[11px] text-[#8A9099] leading-snug shrink-0">
      ...
      <div className="border-t border-[#2A2F39] p-3 mt-2">
        <form
        ...
        >
          <textarea
            ...
            rows={2}
            className={`flex-1 resize-none text-sm border rounded-xl px-3 py-2 focus:outline-none ${editMode ? 'border-[#4A3A22] focus:border-[#E0A254]' : 'border-[#2A2F39] focus:border-[#FAFAF8]'}`}
          />
          <button
            type="submit"
            disabled={busy || !input.trim()}
            className={`text-sm font-medium px-4 py-2 text-[#0B0D10] rounded-xl transition-colors disabled:opacity-40 ${editMode ? 'bg-[#E0A254] hover:bg-[#E0A254]' : 'bg-[#FAFAF8] hover:bg-[#FFFFFF]'}`}
          >
