...
export function BlockProgressPanel({ data }: { data: BlockProgressData | null }) {
  if (!data) {
    return (
      <div className="bg-[#111110] border border-[#1c1917] rounded-2xl px-5 py-4 mb-6">
        <p className="text-[10px] uppercase tracking-widest text-[#57534e] mb-1" style={{ fontFamily: MONO_FONT }}>
          Block progress
        </p>
        <p className="text-sm text-[#a8a29e]">No active program for this client yet.</p>
      </div>
    )
  }
  ...
  return (
    <div
      className={`bg-[#111110] border rounded-2xl px-5 py-4 mb-6 ${
        blockEnded ? 'border-amber-500/40' : blockEndingSoon ? 'border-amber-500/20' : 'border-[#1c1917]'
      }`}
    >
      ...
      {data.blockName && <p className="text-sm font-semibold text-white mb-3">{data.blockName}</p>}

      <div className="grid grid-cols-3 gap-3 mb-3">
        <div className="bg-[#0a0a0a] border border-[#1c1917] rounded-xl px-3 py-2.5">
          <p className="text-[10px] uppercase tracking-widest text-[#57534e]" style={{ fontFamily: MONO_FONT }}>Week</p>
          <p className="text-lg font-bold text-white tabular-nums" style={{ fontFamily: MONO_FONT }}>
            {data.blockWeek} <span className="text-[#57534e] text-sm font-normal">/ {data.weekDuration}</span>
          </p>
        </div>
        <div className="bg-[#0a0a0a] border border-[#1c1917] rounded-xl px-3 py-2.5">
          <p className="text-[10px] uppercase tracking-widest text-[#57534e]" style={{ fontFamily: MONO_FONT }}>This week</p>
          <p className="text-lg font-bold text-white tabular-nums" style={{ fontFamily: MONO_FONT }}>
            {data.sessionsCompletedThisWeek}
            <span className="text-[#57534e] text-sm font-normal"> / {data.sessionsPrescribedThisWeek}</span>
          </p>
          <p className="text-[10px] text-[#57534e]" style={{ fontFamily: MONO_FONT }}>
            {data.sessionsInProgressThisWeek > 0 ? `${data.sessionsInProgressThisWeek} in progress` : `${completionPct}% logged`}
          </p>
        </div>
        <div className="bg-[#0a0a0a] border border-[#1c1917] rounded-xl px-3 py-2.5">
          <p className="text-[10px] uppercase tracking-widest text-[#57534e]" style={{ fontFamily: MONO_FONT }}>Block total</p>
          <p className="text-lg font-bold text-white tabular-nums" style={{ fontFamily: MONO_FONT }}>
            {data.totalSessionsLoggedThisBlock}
          </p>
          <p className="text-[10px] text-[#57534e]" style={{ fontFamily: MONO_FONT }}>sessions logged</p>
        </div>
      </div>

      {data.latestLoggedAt && (
        <p className="text-[11px] text-[#a8a29e]" style={{ fontFamily: MONO_FONT }}>
          Last logged{' '}
      ...
      {data.totalSessionsLoggedThisBlock === 0 && (
        <p className="text-[12px] text-[#a8a29e] italic">
          ...
          <Link href="/dashboard/help#training-program" className="text-teal-400 hover:underline">
            How logging works
          </Link>
        </p>
      )}
