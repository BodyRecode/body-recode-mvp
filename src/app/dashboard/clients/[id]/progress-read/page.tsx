    ...
    <div className="max-w-[980px]">
      ...
      {!latestCheck && (
        <div className="br-card p-6 mb-6">
          <p className="text-[14px] text-[#141821] font-semibold mb-1">No new Progress Check submitted yet</p>
      ...
      {latestCheck && (
        <div className="br-card p-6 mb-6 flex items-start justify-between gap-6 flex-wrap">
          <div>
            <p className="text-[14px] text-[#141821] font-semibold mb-1">Progress Check submitted {fmt(latestCheck.submitted_at)}</p>
            <p className="text-[13px] text-[#666D7A] leading-relaxed max-w-[520px]">
      ...
      {read && (
        <>
          <div className="br-card overflow-hidden mb-6">
            <div className="px-6 pt-5 pb-5 grid grid-cols-1 sm:grid-cols-3 gap-5 border-b border-[#E8EAEE]">
              <div>
                <p className="text-[11px] font-medium text-[#98A0AD] mb-1.5">Readiness</p>
                <p className="text-[17px] font-bold text-[#141821]">
                ...
                <p className="text-[12.5px] text-[#666D7A] capitalize">{read.state_direction}{read.state_clamped ? ' · held to one step by the rules' : ''}</p>
              </div>
              <div>
                <p className="text-[11px] font-medium text-[#98A0AD] mb-1.5">Pattern</p>
                <p className="text-[17px] font-bold text-[#141821]">{readPatternLabel(read.pattern_classification) ?? 'Not named'}</p>
              ...
              <div>
                <p className="text-[11px] font-medium text-[#98A0AD] mb-1.5">Four readiness ratings</p>
          ...
          {read.pattern_changed && change.evidence && (
            <div className="br-card-flagged px-5 py-4 mb-6">
              <p className="text-[11px] font-medium text-[#1B6DFC] mb-1.5">Why the pattern changed</p>
              <p className="text-[13px] text-[#43474F] leading-relaxed">{change.evidence}</p>
            </div>
          )}

          {lint.length > 0 && (
            <div className="rounded-xl border border-[#F1DEB8] bg-[#FDF6E9] px-5 py-4 mb-6">
              <p className="text-[12px] font-medium text-[#8A5A14] mb-2">Pre-publish check on her version</p>
              <ul className="space-y-1.5">
                {lint.map((f, i) => (
                  <li key={i} className="text-[13px] text-[#43474F] leading-relaxed">
          ...
          <h2 className="text-[15px] font-semibold text-[#141821] mb-1">Her version</h2>
          <p className="text-[12.5px] text-[#666D7A] mb-3">Exactly what she will see, written to her. Nothing below this box reaches her.</p>
          <div className="br-card p-6 mb-8 space-y-5">
            {HER_SECTIONS.filter(([k]) => her[k]).map(([k, label]) => (
              <div key={k}>
                <p className="text-[11px] font-medium text-[#98A0AD] mb-1">{label}</p>
                <p className={`leading-relaxed ${k === 'headline' ? 'text-[16px] font-semibold text-[#141821]' : 'text-[14px] text-[#43474F]'}`}>{her[k]}</p>
              </div>
            ))}
          </div>

          <h2 className="text-[15px] font-semibold text-[#141821] mb-3">For you only</h2>
          {rules.length > 0 && (
            <div className="br-card px-6 py-4 mb-4">
              <p className="text-[11px] font-medium text-[#98A0AD] mb-2">Operating rules</p>
              <ul className="list-disc list-inside space-y-1 text-[13.5px] text-[#43474F]">{rules.map((r, i) => <li key={i}>{r}</li>)}</ul>
            </div>
          )}
          <div className="br-card p-6 mb-6 space-y-5">
            {typeof content.pattern_competing_read === 'string' && content.pattern_competing_read !== 'None' && (
              <div>
                <p className="text-[11px] font-medium text-[#98A0AD] mb-1">{read.pattern_classification === 'Indeterminate' ? 'Leaning toward' : 'Competing read'}</p>
                <p className="text-[14px] text-[#43474F]">{content.pattern_competing_read}</p>
              </div>
            )}
            {COACH_SECTIONS.filter(([k]) => typeof content[k] === 'string' && (content[k] as string).trim()).map(([k, label]) => (
              <div key={k}>
                <p className="text-[11px] font-medium text-[#98A0AD] mb-1">{label}</p>
                <p className="text-[14px] text-[#43474F] leading-relaxed whitespace-pre-line">{content[k] as string}</p>
              </div>
            ))}
          </div>
          ...
          {read.comparison_text && (
            <details className="br-card px-6 py-4 mb-10">
              <summary className="text-[13px] font-medium text-[#141821] cursor-pointer">The computed comparison the read was given</summary>
              <pre className="mt-3 text-[12px] text-[#43474F] whitespace-pre-wrap leading-relaxed font-sans">{read.comparison_text}</pre>
            </details>
          )}
        </>
      )}
