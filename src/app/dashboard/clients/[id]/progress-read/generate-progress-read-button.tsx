...
export default function GenerateProgressReadButton({ progressCheckId, hasRead }: { progressCheckId: string; hasRead: boolean }) {
  ...
  return (
    <div>
      ...
      {state === 'running' && (
        <p className="text-[12.5px] text-[#666D7A] mt-2 leading-relaxed">Usually 4 to 6 minutes. It reads everything since the last read. The page is not frozen, please don&apos;t refresh.</p>
      )}
      {state === 'error' && <p className="text-[12.5px] text-[#C82626] mt-2 leading-relaxed">{error}</p>}
    </div>
  )
}
