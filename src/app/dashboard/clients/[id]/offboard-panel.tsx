  ...
  if (endedAt) {
    ...
    return (
      <div className="rounded-xl border border-[#2A2F39] bg-[#14171D] p-5">
        <p className="text-sm font-semibold text-[#FAFAF8]">Engagement ended</p>
    ...
    <div className="rounded-xl border border-[#2A2F39] bg-[#14171D] p-5">
      {!open ? (
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-[#FAFAF8]">End engagement</p>
            <p className="text-[12.5px] text-[#8A9099] mt-1">
          ...
          <button onClick={() => setOpen(true)} className="flex-none px-4 py-2 rounded-lg text-[12.5px] font-medium bg-[#1A1E26] text-[#FAFAF8] hover:bg-[#2A2F39] transition-colors">
            Offboard
          </button>
        </div>
      ) : (
        <div className="space-y-4">
          <p className="text-sm font-semibold text-[#FAFAF8]">End engagement with {firstName}</p>

          <div>
            <label className="block text-[12.5px] font-medium text-[#8A9099] mb-1.5">Reason</label>
            <select
              value={reason}
            ...
            {reason && <p className="text-[12.5px] text-[#8A9099] mt-1.5">{END_REASONS.find(r => r.value === reason)?.hint}</p>}
          </div>

          <div>
            <label className="block text-[12.5px] font-medium text-[#8A9099] mb-1.5">
              What happened <span className="font-normal normal-case text-[#676D76]">(worth more than the reason code)</span>
            </label>
            <textarea
              value={notes}
          ...
          <div className="flex gap-2">
            <button onClick={submit} disabled={busy} className="px-4 py-2 rounded-lg text-[12.5px] font-medium bg-[#D4817E] text-[#FAFAF8] hover:bg-[#D4817E] disabled:opacity-40 transition-colors">
