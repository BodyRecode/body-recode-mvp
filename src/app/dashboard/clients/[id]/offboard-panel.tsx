...
export default function OffboardPanel({
  ...
  if (endedAt) {
    ...
    return (
      <div className="rounded-xl border border-[#E4E4E0] bg-[#F2F2EF] p-5">
        <p className="text-sm font-semibold text-[#0F1115]">Engagement ended</p>
    ...
    <div className="rounded-xl border border-[#E4E4E0] bg-white p-5">
      {!open ? (
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-[#0F1115]">End engagement</p>
            <p className="text-[12.5px] text-[#6E747D] mt-1">
          ...
          <button onClick={() => setOpen(true)} className="flex-none px-4 py-2 rounded-lg text-[12.5px] font-medium bg-[#EDEDEA] text-[#0F1115] hover:bg-[#E4E4E0] transition-colors">
            Offboard
          </button>
        </div>
      ) : (
        <div className="space-y-4">
          <p className="text-sm font-semibold text-[#0F1115]">End engagement with {firstName}</p>

          <div>
            <label className="block text-[12.5px] font-medium text-[#6E747D] mb-1.5">Reason</label>
            <select
              value={reason}
            ...
            {reason && <p className="text-[12.5px] text-[#6E747D] mt-1.5">{END_REASONS.find(r => r.value === reason)?.hint}</p>}
          </div>

          <div>
            <label className="block text-[12.5px] font-medium text-[#6E747D] mb-1.5">
              What happened <span className="font-normal normal-case text-[#9CA2AB]">(worth more than the reason code)</span>
            </label>
            <textarea
              value={notes}
          ...
          <div className="flex gap-2">
            <button onClick={submit} disabled={busy} className="px-4 py-2 rounded-lg text-[12.5px] font-medium bg-[#8F2D2D] text-white hover:bg-[#8F2D2D] disabled:opacity-40 transition-colors">
