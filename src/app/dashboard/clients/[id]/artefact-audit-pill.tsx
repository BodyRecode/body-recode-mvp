...
export default function ArtefactAuditPill({ audit }: { audit: ArtefactAuditResult | null }) {
  ...
  if (!audit) {
    return (
      <span className="inline-flex items-center gap-1 text-[11px] font-medium px-1.5 py-0.5 rounded bg-[#14171D] border border-[#2A2F39] text-[#676D76]">
        Not published
      </span>
    )
  }
  ...
  const statusColours: Record<typeof audit.status, { border: string; bg: string; text: string; icon: string }> = {
    green: { border: 'border-[#1A1E26]', bg: 'bg-[#14171D]', text: 'text-[#6FA98B]', icon: 'text-[#6FA98B]' },
    amber: { border: 'border-[#4A3A22]', bg: 'bg-[#1A1E26]', text: 'text-[#E0A254]', icon: 'text-[#E0A254]' },
    red:   { border: 'border-[#4A2222]',   bg: 'bg-[#1A1214]',   text: 'text-[#D4817E]',   icon: 'text-[#D4817E]' },
  }
  ...
  return (
    <div className="inline-flex flex-col gap-1.5">
      ...
      {expanded && (
        <div className={`mt-1 rounded-md border ${s.border} ${s.bg} px-3 py-2 text-[11px] text-[#C2C6CC] max-w-md space-y-2`}>
          <DetailRow
          ...
          {audit.issues.length > 0 && (
            <div>
              <p className="text-[11px] font-medium text-[#8A9099] mb-1">Validator findings</p>
              <ul className="space-y-1">
                {audit.issues.map((issue, i) => (
                  <li key={i} className="flex items-start gap-2 leading-snug">
                    <span className={`mt-0.5 inline-block w-1.5 h-1.5 rounded-full shrink-0 ${issue.severity === 'error' ? 'bg-[#D4817E]' : 'bg-[#E0A254]'}`} />
...
function DetailRow({ label, value, ok }: { label: string; value: string; ok: boolean }) {
  return (
    <div className="flex items-start gap-2">
      <span className={`mt-0.5 inline-block w-1.5 h-1.5 rounded-full shrink-0 ${ok ? 'bg-[#6FA98B]' : 'bg-[#E0A254]'}`} />
      <div className="min-w-0">
        <p className="text-[11px] font-medium text-[#8A9099]">{label}</p>
        <p className="text-[11px] text-[#FAFAF8] break-words">{value}</p>
      </div>
    </div>
  )
}
