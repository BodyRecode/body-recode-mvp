...
function Row({ label, value }: { label: string; value: unknown }) {
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-2.5 border-b border-[#2A2F39] last:border-0">
      <p className="text-[12.5px] text-[#8A9099] flex-shrink-0 w-52">{label}</p>
  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <p className="text-sm text-[#8A9099]">Intake - {client.name}</p>
      </div>
      ...
      <div className="flex items-start justify-between br-page-header sticky top-0 z-20 mb-7 pt-4 pb-3.5 border-b border-[#2A2F39] bg-[#14171D]/[0.88] backdrop-blur-md print:static print:bg-transparent">
        <div>
          ...
          {submittedDate && <p className="text-[12.5px] text-[#8A9099] mt-1">Submitted {submittedDate}</p>}
        </div>
        {intake && (
          <Link
      ...
      {!intake ? (
        <div className="bg-[#14171D] br-card p-5">
          <p className="text-[#8A9099] text-sm">Intake not yet submitted.</p>
        </div>
      ) : (
        <>
