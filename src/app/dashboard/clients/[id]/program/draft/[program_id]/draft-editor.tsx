...
export default function DraftEditor({
  ...
  const inputCls =
    'bg-[#1A1E26] border border-[#2A2F39] text-[#FAFAF8] rounded px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-[#FAFAF8] focus:border-transparent'
  ...
  return (
    <div className="max-w-[980px]">
      {/* Header */}
      <div className="mb-6">
        <div className="flex items-center gap-2 text-[#8A9099] text-sm mb-2">
          <Link href={`/dashboard/clients/${clientId}`} className="hover:text-[#FAFAF8] transition-colors">{clientName}</Link>
          <span>/</span>
          <Link href={`/dashboard/clients/${clientId}/program`} className="hover:text-[#FAFAF8] transition-colors">Training Program</Link>
          <span>/</span>
          <span className="text-[#FAFAF8]">{isActive ? 'Edit Exercises' : 'Draft Review'}</span>
        </div>
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em]">{program.block_name}</h1>
            <p className={`text-sm mt-1 ${isActive ? 'text-[#8A9099]' : 'text-[#E0A254]'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            {dirty && (
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-lg transition-colors disabled:opacity-40 ${isActive ? 'bg-[#FAFAF8] text-[#0B0D10] font-semibold hover:bg-[#FFFFFF]' : 'border border-[#2A2F39] text-[#FAFAF8] hover:border-[#8A9099]'}`}
              >
            ...
            {isActive && (
              <Link
                ...
                className="text-[12.5px] px-3 py-1.5 border border-[#2A2F39] text-[#8A9099] rounded-lg hover:border-[#2A2F39] hover:text-[#FAFAF8] transition-colors"
              >
                Done
              </Link>
            )}
      ...
      {error && (
        <p className="text-sm text-[#D4817E] bg-[#1A1214] border border-[#4A2222] rounded-md px-3 py-2 mb-4">
          {error}
        </p>
      )}

      {dirty && (
        <div className="mb-4 text-[12.5px] text-[#E0A254] bg-[linear-gradient(180deg,#1A1E26,#1A1E26)] border border-[#4A3A22]/50 rounded-lg px-3 py-2">
          You have unsaved changes. Save before promoting or your edits will be lost.
        </div>
      )}

      {/* Program identity */}
      <div className="bg-[#14171D] border border-[#4A3A22]/40 rounded-xl p-5 mb-4">
        <div className="flex items-start justify-between mb-2">
          <div>
            <p className="text-[12.5px] text-[#8A9099] mb-1 capitalize">
      ...
      {/* Editing hint */}
      <p className="text-[12.5px] text-[#676D76] mb-4 px-1">
        ...
        {sessions.map((session, sIdx) => (
          <div key={sIdx} className="bg-[#14171D] br-card overflow-hidden">
            ...
            <div className="px-5 py-3 border-b border-[#2A2F39] flex items-center justify-between">
              <h3 className="font-semibold text-[#FAFAF8] text-sm">{session.day_label}</h3>
            ...
            <div className="divide-y divide-[#1A1E26]/60">
              ...
              {session.movement_prep?.length > 0 && (
                <div className="px-5 py-4 bg-[#1A1E26]/30">
                  ...
                  <p className="text-[10px] text-[#676D76] mb-3">Non-Slot · Prepare joints, tissues, and coordination</p>
                    ...
                    {session.movement_prep.map((item, i) => (
                      <div key={i} className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          className="flex-1 bg-transparent border-b border-[#2A2F39] text-sm text-[#FAFAF8] py-0.5 focus:outline-none focus:border-[#FAFAF8] transition-colors"
                        />
                      </div>
                    ))}
                  </div>
                ...
                <div key={bIdx} className="px-5 py-4">
                  ...
                  <div className="space-y-2">
                    {block.exercises.map((ex, eIdx) => {
                      ...
                      return (
                        <div key={eIdx} className={`rounded-lg border transition-colors ${isEditing ? 'border-[#2A2F39] bg-[#1A1E26]/50' : 'border-transparent hover:border-[#2A2F39] cursor-pointer'}`}>
                          {/* Collapsed row */}
                          {!isEditing ? (
                            <div
                            ...
                            <div className="px-3 py-3 space-y-3">
                              {/* Exercise name + swap */}
                              <div className="flex items-center gap-2">
                                <span className="text-sm font-semibold text-[#FAFAF8] flex-1">{ex.exercise_name}</span>
                                <button
                                  onClick={() => isSwapping ? closeSwap() : openSwap(path)}
                                  className="text-[12.5px] px-2.5 py-1 border border-[#2A2F39] text-[#8A9099] rounded hover:border-[#FAFAF8] hover:text-[#FAFAF8] transition-colors"
                                >
                              ...
                              {/* Swap panel */}
                              {isSwapping && (
                                <div className="border border-[#2A2F39] rounded-lg bg-[#14171D] overflow-hidden">
                                  <div className="p-2 border-b border-[#2A2F39]">
                                    <input
                                      type="text"
                                      placeholder="Search exercises…"
                                      value={swapSearch}
                                      onChange={e => setSwapSearch(e.target.value)}
                                      className="w-full bg-[#1A1E26] border border-[#2A2F39] text-[#FAFAF8] rounded px-2 py-1.5 text-[12.5px] focus:outline-none focus:ring-1 focus:ring-[#FAFAF8]"
                                      autoFocus
                                    />
                                    {!libraryLoaded && <p className="text-[12.5px] text-[#676D76] mt-1 px-1">Loading library…</p>}
                                    {libraryLoaded && !swapSearch && currentPatterns.length > 0 && (
                                      <p className="text-[10px] text-[#676D76] mt-1 px-1">Showing same pattern. Type to search all.</p>
                                    )}
                                  </div>
                                  <div className="max-h-48 overflow-y-auto divide-y divide-[#1A1E26]">
                                    {swapResults.length === 0 && (
                                      <p className="text-[12.5px] text-[#676D76] px-3 py-2">No matches found.</p>
                                    )}
                                    {swapResults.map(lib => (
                                      <button
                                        key={lib.name}
                                        onClick={() => selectSwapExercise(lib, path)}
                                        className="w-full text-left px-3 py-2 hover:bg-[#1A1E26] transition-colors"
                                      >
                                        <span className="text-sm text-[#FAFAF8] block">{lib.name}</span>
                              ...
                              <div className="grid grid-cols-4 gap-2">
                                <div>
                                  <label className="block text-[10px] text-[#8A9099] mb-1">Sets</label>
                                  <input
                                    type="number"
                                    min={1}
                                    max={10}
                                ...
                                <div>
                                  <label className="block text-[10px] text-[#8A9099] mb-1">Reps</label>
                                  <input
                                    type="text"
                                    value={ex.reps}
                                ...
                                <div>
                                  <label className="block text-[10px] text-[#8A9099] mb-1">RPE</label>
                                  <input
                                    type="number"
                                    min={5}
                                    max={10}
                                ...
                                <div>
                                  <label className="block text-[10px] text-[#8A9099] mb-1">Rest</label>
                                  <input
                                    type="text"
                                    value={ex.rest}
                              ...
                              <div>
                                <label className="block text-[10px] text-[#8A9099] mb-1">Notes</label>
                                <input
                                  type="text"
                                  value={ex.notes}
