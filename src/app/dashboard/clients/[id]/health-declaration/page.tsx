...
function Row({ label, value }: { label: string; value: string | number | boolean | null | undefined }) {
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-2.5 border-b border-[#2A2F39] last:border-0">
      <p className="text-[12.5px] text-[#8A9099] flex-shrink-0 w-44">{label}</p>
      <p className="text-sm text-[#FAFAF8] text-right">{display}</p>
    </div>
  )
}
    ...
    return (
      <div className="max-w-2xl">
        <div className="flex items-center gap-3 mb-8">
          ...
          <p className="text-sm text-[#8A9099]">Health Declaration - {client.name}</p>
        </div>
        <p className="text-[#8A9099] text-sm">Health declaration not yet submitted.</p>
      </div>
    )
  }
    ...
    <div className="max-w-2xl">
      ...
      <div className="flex items-center justify-between br-page-header sticky top-0 z-20 mb-7 pt-4 pb-3.5 border-b border-[#2A2F39] bg-[#14171D]/[0.88] backdrop-blur-md print:static print:bg-transparent">
        <div>
          ...
          <p className="text-[12.5px] text-[#8A9099] mt-1">Submitted {submittedDate}</p>
        </div>
        <div className="flex items-center gap-2">
          {client.medical_clearance_required && (
            <span className="text-[12.5px] font-semibold text-[#E0A254] bg-[linear-gradient(180deg,#1A1E26,#1A1E26)] border border-[#4A3A22] px-3 py-1.5 rounded-lg">
              Clearance required
            </span>
          )}
          <Link
      ...
      {!d ? (
        <div className="bg-[#14171D] br-card p-5">
          <p className="text-[#8A9099] text-sm">Form was submitted but detailed answers were not saved (submitted before this feature was added).</p>
        </div>
      ) : (
        <>
          {/* Personal Details */}
            ...
            {d.cardiovascularScreening?.symptoms?.length > 0 ? (
              <div className="space-y-1.5">
                {d.cardiovascularScreening.symptoms.map((s: string) => (
                  <div key={s} className={`text-sm px-3 py-2 rounded-lg ${s === 'None of the above' ? 'text-[#FAFAF8] bg-[rgba(27,109,252,0.08)]' : 'text-[#E0A254] bg-[#1A1E26]'}`}>
                    {s}
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-[#8A9099]">No symptoms reported</p>
            )}
          </Section>
          ...
          <Section title="Barriers & Goals">
            {d.barriersAndGoals?.barriers?.length > 0 && (
              <div className="mb-3">
                <p className="text-[12.5px] text-[#8A9099] mb-2">Barriers</p>
                <div className="flex flex-wrap gap-2">
                  {d.barriersAndGoals.barriers.map((b: string) => (
                    <span key={b} className="text-[12.5px] bg-[#1A1E26] text-[#FAFAF8] px-2.5 py-1 rounded-lg border border-[#2A2F39]">{b}</span>
                  ))}
                </div>
              </div>
            )}
