...
export default function CheckinFeedbackForm({
  ...
  // pipeline and is scheduled to send. Coach gets a 4h intervention window
  // (Edit, Send now, Skip). After scheduled time passes, Inngest sends it.
  const isAutoDraftPending =
    !!existing &&
    !existing.email_sent_at &&
    !!autoGeneratedAt &&
    !!autoSendScheduledAt &&
    new Date(autoSendScheduledAt).getTime() > Date.now()

  // Local clock for the countdown display. Re-renders every 30s.
  const [nowMs, setNowMs] = useState(Date.now())
  useEffect(() => {
    if (!isAutoDraftPending) return
  ...
  // Renders above the response card when an auto-generated draft is queued
  // for send within the intervention window. Shows the countdown + lets the
  // coach Send now to short-circuit the wait.
  function AutoDraftBanner() {
    if (!isAutoDraftPending || !autoSendScheduledAt) return null
    const target = new Date(autoSendScheduledAt).getTime()
    return (
      <div className="mb-3 rounded-lg border border-[#1B6DFC]/30 bg-[#1B6DFC]/5 px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          <p className="text-[11px] font-bold uppercase tracking-widest text-[#1B6DFC]">AI draft, auto-sending in {formatCountdown(target, nowMs)}</p>
          <p className="text-xs text-[#3A3A3A] mt-1 leading-relaxed">
            This response was AI-generated when {clientFirstName} submitted the check-in. It will email {clientFirstName} automatically at {new Date(autoSendScheduledAt).toLocaleString('en-AU', { timeZone: 'Australia/Brisbane', weekday: 'short', day: 'numeric', month: 'short', hour: 'numeric', minute: '2-digit', hour12: true })} unless you edit, send now, or skip.
          </p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            onClick={() => submit(true)}
            disabled={pending}
            className="text-xs font-bold px-3 py-1.5 bg-[#1B6DFC] text-white rounded-lg hover:bg-[#1057CC] transition-colors disabled:opacity-50"
          >
            Send now
          </button>
          <button
            ...
            onClick={skip}
            disabled={skipping}
            className="text-xs font-bold px-3 py-1.5 border border-[#E5E5E5] text-[#6B6B6B] rounded-lg hover:border-[#1B6DFC] hover:text-[#1B6DFC] transition-colors disabled:opacity-50"
          >
            Skip instead
          </button>
        </div>
  ...
  function AiFailureBanner() {
    ...
    return (
      <div className="mb-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3">
        <p className="text-[11px] font-bold uppercase tracking-widest text-amber-700">AI auto-response failed</p>
        <p className="text-xs text-[#3A3A3A] mt-1 leading-relaxed">
