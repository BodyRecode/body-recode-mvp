  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <p className="text-sm text-[#8A9099]">Medical Clearance - {client.name}</p>
      </div>

      {approved ? (
        <div className="bg-[rgba(27,109,252,0.08)] border border-[#FAFAF8]/20 rounded-xl p-5 mb-6">
          <p className="text-sm font-semibold text-[#FAFAF8] mb-1">Clearance approved</p>
          <p className="text-[12.5px] text-[#8A9099]">Approved on {new Date(client.medical_clearance_received_at!).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })}. Intake and baseline are unlocked.</p>
        </div>
      ) : submitted ? (
        <div className="bg-[linear-gradient(180deg,#1A1E26,#1A1E26)] border border-[#4A3A22] rounded-xl p-5 mb-6 flex items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold text-[#E0A254] mb-1">Form uploaded - awaiting your approval</p>
        ...
        <div className="bg-[#14171D] br-card p-5 mb-6">
          <p className="text-sm font-semibold text-[#E0A254] mb-1">Waiting for client upload</p>
          <p className="text-[12.5px] text-[#8A9099]">The client has been notified. This page will update once they upload their completed form.</p>
        </div>
      )}
      ...
      {submitted && docSignedUrl && (
        <div className="bg-[#14171D] br-card p-5 mb-6">
          <p className="text-[12.5px] font-medium text-[#8A9099] mb-3">Uploaded Document</p>
          {docSignedUrl.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
            <img src={docSignedUrl} alt="Medical clearance form" className="w-full rounded-lg border border-[#2A2F39]" />
          ) : (
            <a
              href={docSignedUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-3 text-sm text-[#FAFAF8] hover:text-[#FFFFFF] transition-colors"
            >
      ...
      {/* WhatsApp message */}
      <div className="bg-[#14171D] br-card p-5 mb-6">
        <p className="text-[12.5px] font-medium text-[#8A9099] mb-3">WhatsApp Message Template</p>
        <pre className="text-sm text-[#FAFAF8] whitespace-pre-wrap leading-relaxed font-sans">{whatsappMessage}</pre>
      </div>

      {/* Remove clearance requirement */}
      {!approved && (
        <div className="bg-[#14171D] br-card p-5 mb-6 flex items-center justify-between gap-4">
          <div>
            <p className="text-[12.5px] font-medium text-[#8A9099] mb-1">Remove Requirement</p>
            <p className="text-[12.5px] text-[#8A9099]">If the clearance was flagged in error, remove it to clear this client's file.</p>
          </div>
      ...
      <div className="bg-[#14171D] br-card p-5">
        <div className="flex items-center justify-between mb-2">
          <p className="text-[12.5px] font-medium text-[#8A9099]">Blank Form (Coach Copy)</p>
          <Link
            href={`/dashboard/clients/${id}/medical-clearance/print`}
            target="_blank"
            className="text-[12.5px] bg-[#1A1E26] text-[#FAFAF8] hover:bg-[#2A2F39] hover:text-[#FAFAF8] px-3 py-1.5 rounded-lg transition-colors"
          >
            Open printable form ↗
          </Link>
        </div>
        <p className="text-[12.5px] text-[#8A9099]">Blank form pre-filled with client details, for your records.</p>
      </div>
    </div>
  )
}
