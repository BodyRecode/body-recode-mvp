  ...
  return (
    <div id={id} className="mt-6 scroll-mt-8">
      <div className="flex items-center justify-between mb-3 gap-3 flex-wrap">
        ...
        >
          <span
            className="w-[22px] h-[22px] rounded-md shrink-0 flex items-center justify-center text-[#FAFAF8] transition-colors group-hover:text-[#FFFFFF]"
            style={{
              background: 'rgba(27,109,252,0.08)',
              boxShadow: 'inset 0 0 0 1px #2A2F39',
            }}
            aria-hidden
          >
            <ChevronDown
          ...
          <h2 className="text-[13.5px] font-semibold text-[#FAFAF8] tracking-[-0.015em] truncate">
            {title}
            {subtitle && <span className="text-[#676D76] font-normal"> {subtitle}</span>}
          </h2>
          {attentionLabel && !open && (
            <span className="shrink-0 inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-[3px] rounded-full border border-[#4A3A22] text-[#E0A254] bg-[linear-gradient(180deg,#1A1E26,#1A1E26)] shadow-[0_1px_2px_rgba(16,24,40,0.05)]">
