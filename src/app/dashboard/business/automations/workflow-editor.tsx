...
function SortableStep({
  ...
  return (
    <div ref={setNodeRef} style={style} className="relative">
      {/* Connector line */}
      <div className="absolute left-7 -top-4 w-px h-4 bg-[#1F242C]" />
      ...
      <div className="bg-[#1A1E26] br-card overflow-hidden">
        ...
        <div className="flex items-center gap-3 p-3">
          <button
            {...attributes}
            {...listeners}
            className="text-[#676D76] hover:text-[#8A9099] cursor-grab active:cursor-grabbing p-1"
          >
            <GripVertical size={14} />
          </button>

          {step.type === 'wait' && <Clock size={14} className="text-[#A96A12] shrink-0" />}
          {step.type === 'condition' && <GitBranch size={14} className="text-[#C2C6CC] shrink-0" />}
          {step.type === 'action' && ActionIcon && <ActionIcon size={14} className="text-[#FAFAF8] shrink-0" />}

          <span className="text-sm font-medium text-[#FAFAF8] flex-1">
            {step.type === 'wait'
          ...
          <button onClick={() => setExpanded(e => !e)} className="text-[#8A9099] hover:text-[#FAFAF8] p-1">
            {expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
          </button>
          <button onClick={() => onDelete(step.id)} className="text-[#676D76] hover:text-[#D4817E] transition-colors p-1">
            <Trash2 size={13} />
          </button>
        </div>
        ...
        {/* Config */}
        {expanded && (
          <div className="px-4 pb-4 pt-1 border-t border-[#2A2F39] space-y-3">
            {step.type === 'action' && (
              <>
                {/* Action type picker */}
                <div>
                  <label className="block text-[12.5px] text-[#8A9099] mb-1.5">Action</label>
                ...
                {actionDef?.fields.map(field => (
                  <div key={field.key}>
                    <label className="block text-[12.5px] text-[#8A9099] mb-1.5">{field.label}</label>
                    {field.type === 'textarea' ? (
                      <textarea
                        ...
                        className="w-full bg-[#1F242C] border border-[#2A2F39] rounded-lg px-3 py-2 text-[12.5px] text-[#FAFAF8] placeholder-[#676D76] resize-none focus:outline-none focus:border-[#2A2F39]"
                      />
                    ) : field.type === 'select' ? (
                      <select
                        value={(step.config[field.key] as string) || ''}
                        ...
                        className="w-full bg-[#1F242C] border border-[#2A2F39] rounded-lg px-3 py-2 text-[12.5px] text-[#FAFAF8] focus:outline-none focus:border-[#2A2F39]"
                      >
                      ...
                      <input
                        ...
                        placeholder={`Enter ${field.label.toLowerCase()}...`}
                        className="w-full bg-[#1F242C] border border-[#2A2F39] rounded-lg px-3 py-2 text-[12.5px] text-[#FAFAF8] placeholder-[#676D76] focus:outline-none focus:border-[#2A2F39]"
                      />
                    )}
                  </div>
                ))}
            ...
            {step.type === 'wait' && (
              <div className="flex gap-2">
                <div className="flex-1">
                  <label className="block text-[12.5px] text-[#8A9099] mb-1.5">Amount</label>
                  <input
                    type="number"
                    min="1"
                ...
                <div className="flex-1">
                  <label className="block text-[12.5px] text-[#8A9099] mb-1.5">Unit</label>
                  <select
...
function AddStepButton({ onAdd }: { onAdd: (type: StepType) => void }) {
  ...
  return (
    <div className="relative flex flex-col items-center">
      <div className="w-px h-4 bg-[#1F242C]" />
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1.5 text-[12.5px] text-[#8A9099] hover:text-[#FAFAF8] border border-[#2A2F39] hover:border-[#2A2F39] px-3 py-1.5 rounded-lg transition-colors bg-[#14171D]"
      >
        <Plus size={12} />
        Add Step
      </button>
      {open && (
        <>
          ...
          <div className="absolute top-8 z-20 bg-[#1A1E26] br-card shadow-2xl overflow-hidden w-44">
            {[
              { type: 'action' as StepType, label: 'Action', icon: Zap, colour: 'text-[#FAFAF8]' },
              { type: 'wait' as StepType, label: 'Wait / Delay', icon: Clock, colour: 'text-[#A96A12]' },
              { type: 'condition' as StepType, label: 'Condition', icon: GitBranch, colour: 'text-[#C2C6CC]' },
            ].map(opt => {
              const Icon = opt.icon
              return (
                <button
                  ...
                  className="w-full flex items-center gap-2.5 px-4 py-2.5 text-[12.5px] text-[#FAFAF8] hover:bg-[#1F242C] transition-colors"
                >
...
export default function WorkflowEditor({ initial }: Props) {
  ...
  return (
    <div className="max-w-2xl">
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex-1 mr-4">
          <input
            ...
            className="w-full bg-transparent text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em] placeholder-[#676D76] focus:outline-none border-b border-transparent focus:border-[#2A2F39] pb-1 transition-colors"
          />
        </div>
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 text-xs font-medium px-3 py-2 rounded-lg border transition-colors ${
              isActive
                ...
                : 'bg-[#1F242C] border-[#2A2F39] text-[#8A9099]'
            }`}
          >
          ...
          <button
            ...
            className="flex items-center gap-2 bg-[#FAFAF8] hover:bg-[#E4E4E0] text-[#14171D] text-sm font-semibold px-4 py-2 rounded-lg disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
      ...
      {/* Trigger block */}
      <div className="mb-2">
        <p className="text-[10px] font-semibold text-[#676D76] mb-2 ml-1">Trigger</p>
        {selectedTrigger && !showTriggerPicker ? (
          <div
            className="bg-[#1A1E26] border border-[#2A2F39] rounded-xl p-4 flex items-center justify-between cursor-pointer hover:border-[#FAFAF8]/60 transition-colors"
            onClick={() => setShowTriggerPicker(true)}
          >
            <div>
              <p className="text-sm font-semibold text-[#FAFAF8]">{selectedTrigger.label}</p>
              <p className="text-[12.5px] text-[#8A9099] mt-0.5">{selectedTrigger.description}</p>
            </div>
            <button className="text-[#676D76] hover:text-[#8A9099]">
              <X size={14} />
            </button>
          </div>
        ) : (
          <div className="bg-[#1A1E26] br-card p-4">
            <p className="text-[12.5px] text-[#8A9099] mb-3">Choose what starts this workflow:</p>
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 rounded-lg border transition-colors ${
                    triggerType === trigger.value
                      ...
                      : 'border-[#2A2F39] hover:border-[#2A2F39] hover:bg-[#1F242C]/50'
                  }`}
                >
                  <p className={`text-sm font-medium ${triggerType === trigger.value ? 'text-[#FAFAF8]' : 'text-[#FAFAF8]'}`}>
                    {trigger.label}
                  </p>
                  <p className="text-[12.5px] text-[#8A9099] mt-0.5">{trigger.description}</p>
                </button>
              ))}
            </div>
          </div>
      ...
      {triggerType && (
        <div className="mt-2">
          <p className="text-[10px] font-semibold text-[#676D76] mb-2 ml-1 mt-4">Steps</p>
          ...
          {steps.length === 0 && (
            <p className="text-center text-[#676D76] text-[12.5px] mt-2">
