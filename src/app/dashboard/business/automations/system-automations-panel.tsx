...
const AUTOMATIC_AUTOMATIONS = [
  ...
  },
  {
    ...
    trigger: 'POST /api/submit-progress-check (client submits). For the near-full check the email also carries what has changed in her words, which sections moved compared with last time, disputed answers, injury items to watch and a medications difference (never auto-applied), and turns red with a Needs attention subject for pregnant now or non-prescribed androgen use.',
    steps: 1,
  },
  ...
  // Weekly + session automations
  {
    ...
    description: 'Coach notification + client confirmation when a weekly check-in (Form A or Form B) is submitted. Triggers CFWS generation on EVERY submission, pairing the form just submitted with the most recent opposite form on file - normally last week\'s, because a client fills one form a week and they alternate. (Corrected 2026-09-07 from "when both forms for the week are in", which never happens.) The prompt is told the true week of each form, so a week-on-week change reads as movement rather than the two forms contradicting each other, and the active CFFS is passed through as the anchor for the four readiness ratings - which this path had never done since the rubric landed 2026-05-18, so every automatically generated CFWS before 2026-09-07 was rated cold. The check-in now also carries the training + nutrition review blocks (folded in 2026-07-23), so on submit it files the same program_reviews / nutrition_reviews rows and updates each plan direction.',
  ...
  // the gate, NOT clients.active, which does not stop contact.
  {
    ...
    trigger: 'Coach clicks Notify Client on the active nutrition plan view (gated on plan is_active + Nutrition Read published)',
    steps: 1,
  },
  {
    ...
    trigger: 'Coach clicks Notify Client on the active program view (gated on program status=active + Program Read published)',
    steps: 1,
  },
  {
    id: 'trajectory-reading-published-to-client',
    name: 'Progress Read Published to Client',
    description: 'Client-facing notification that a Progress Read is in the portal. Mirror of the Program / Nutrition Notify Client pattern. Restored 2026-06-22 after 2026-06-09 scrap proved confusing — block-end readings are their own reflection moment and deserve their own coach-gated notification.',
    trigger: 'Coach clicks Notify Client on the Progress Read panel (gated on trajectory_reading_published_at present)',
    steps: 1,
  },
  {
