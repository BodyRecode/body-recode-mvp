...
const statusConfig: Record<string, { label: string; colour: string; icon: typeof Clock }> = {
  ...
  active: { label: 'Sending', colour: 'text-teal-400', icon: Send },
  completed: { label: 'Sent', colour: 'text-teal-400', icon: CheckCircle2 },
    ...
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-stone-400 text-sm">{drafts} draft · {sent} sent</p>
        </div>
        <Link
      ...
      {campaigns && campaigns.length > 0 ? (
        ...
        <div className="bg-stone-900 border border-dashed border-stone-800 rounded-xl p-12 text-center">
          ...
          <p className="text-stone-600 text-xs mb-6">Send email or SMS broadcasts to your leads and clients</p>
          <Link
            href="/dashboard/business/campaigns/new"
