...
export default async function ReconcilePage() {
  ...
  return (
    <div className="max-w-4xl">
      ...
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-[#FAFAF8] mb-1">Reconcile</h2>
        <p className="text-[#8A9099] text-sm">Match Stripe customers to client records and tag products by business stream.</p>
      </div>

      {/* Backfill */}
      <section className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          <h3 className="text-[12.5px] font-semibold text-[#8A9099]">Stripe Backfill</h3>
        </div>
        <div className="bg-[#1A1E26] br-card p-5">
          <p className="text-sm text-[#FAFAF8] mb-1">Pull every Stripe customer and subscription into the cache.</p>
          <p className="text-[12.5px] text-[#8A9099] mb-4">Safe to re-run. Matches Stripe customers to clients by email, populates <code className="text-[#8A9099]">stripe_customer_id</code>, and refreshes <code className="text-[#8A9099]">client_subscriptions</code> from live Stripe state. Run this once after the Phase 1 schema migration, then anytime you suspect drift.</p>
          <RunBackfillButton />
        </div>
      </section>
      ...
      <section className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          <h3 className="text-[12.5px] font-semibold text-[#8A9099]">Commencement Fee Backfill</h3>
        </div>
        <div className="bg-[#1A1E26] br-card p-5">
          <p className="text-sm text-[#FAFAF8] mb-1">Patch historical clients whose $240 commencement payment landed before the tracker was wired.</p>
          <p className="text-[12.5px] text-[#8A9099] mb-4">Until recently, the lead-stage commencement webhook recorded payments in <code className="text-[#8A9099]">be_payments</code> only — it never wrote to <code className="text-[#8A9099]">client_payment_plan</code>. That left clients like Luke showing &quot;Not tracked for payments&quot; even though the fee landed. This walks every paid $240 no-subscription payment and marks the matching plan row as paid. Idempotent.</p>
      ...
      <section className="mb-8">
        <div className="flex items-center gap-2 mb-3">
          <h3 className="text-[12.5px] font-semibold text-[#8A9099]">Clients with no Stripe link</h3>
          <span className="text-[12.5px] text-[#676D76]">{clientsNoStripe?.length ?? 0}</span>
        </div>
          ...
          <div className="bg-[#1A1E26] br-card overflow-hidden">
            {clientsNoStripe.map(c => (
              <Link
                ...
                className="flex items-center justify-between px-4 py-3 border-b border-[#2A2F39] last:border-b-0 hover:bg-[#1F242C]/40 transition-colors"
              >
                <div className="min-w-0">
                  <p className="text-sm font-medium text-[#FAFAF8] truncate">{c.name ?? c.email ?? 'Unnamed'}</p>
                  <p className="text-[12.5px] text-[#8A9099] truncate">{c.email ?? 'No email'}</p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-[10px] text-[#A96A12] font-semibold">No link</span>
                  <ExternalLink size={12} className="text-[#676D76]" />
                </div>
              </Link>
            ))}
          </div>
        ) : (
          <div className="bg-[#1A1E26] border border-dashed border-[#2A2F39] rounded-xl p-6 text-center">
            <p className="text-[#8A9099] text-sm">All active clients are linked to a Stripe customer.</p>
          </div>
        )}
        <p className="text-[12.5px] text-[#676D76] mt-2">
      ...
      <section className="mb-8">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <h3 className="text-[12.5px] font-semibold text-[#8A9099]">Product Categories</h3>
            <span className="text-[12.5px] text-[#676D76]">{products?.length ?? 0}</span>
          </div>
        ...
        <p className="text-[12.5px] text-[#676D76] mb-3">
          Tag every product by which business stream it belongs to. <strong className="text-[#8A9099]">Performance Coaching</strong> products count toward client LTV; the others don&apos;t. Until products are tagged, untagged paid payments are treated as Performance Coaching by default.
        </p>
        {products && products.length > 0 ? (
          <div className="bg-[#1A1E26] br-card overflow-hidden">
            {products.map(p => (
              <div
                key={p.id}
                className="flex items-center justify-between px-4 py-3 border-b border-[#2A2F39] last:border-b-0"
              >
                <div className="min-w-0 flex items-center gap-3">
                  <Tag size={13} className="text-[#676D76] shrink-0" />
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-[#FAFAF8] truncate">{p.name}</p>
                    <p className="text-[12.5px] text-[#8A9099]">
                      ${p.price?.toLocaleString('en-AU')}
                      {p.type === 'subscription' && p.billing_interval && (
                        <span className="text-[#676D76]"> / {p.billing_interval.replace('ly', '')}</span>
                      )}
                      {!p.is_active && <span className="text-[#FAFAF8] ml-2">(inactive)</span>}
                    </p>
                  </div>
                </div>
          ...
          <div className="bg-[#1A1E26] border border-dashed border-[#2A2F39] rounded-xl p-6 text-center">
            <p className="text-[#8A9099] text-sm">No products yet.</p>
          </div>
        )}
      </section>
    </div>
