...
export default function CoachesClient() {
  ...
  const label = 'block text-[12.5px] font-semibold text-[#1A1A1A] mb-1.5'

  return (
    <div className="space-y-8">
      <form onSubmit={invite} className="bg-white border border-[#E8EAEE] rounded-xl p-5 space-y-4">
        <p className="text-[13.5px] text-[#43474F] leading-relaxed">
      ...
      <div>
        ...
        {loading ? (
          <p className="text-[13.5px] text-[#98A0AD]">Loading…</p>
        ) : invitations.length === 0 ? (
          <p className="text-[13.5px] text-[#98A0AD]">None yet.</p>
        ) : (
            ...
            {invitations.map(i => (
              <div key={i.id} className="bg-white border border-[#E8EAEE] rounded-xl p-4 flex items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  {i.note && <p className="text-[12.5px] text-[#98A0AD] mt-1">{i.note}</p>}
                  <p className="text-[11px] text-[#98A0AD] mt-1">
