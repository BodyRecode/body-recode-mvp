...
function Bullets({ items, tone = 'stone' }: { items: string[]; tone?: 'stone' | 'plus' | 'minus' }) {
  ...
  return (
    <div className="space-y-1.5">
      {items.map(s => (
        <div key={s} className="flex items-start gap-2 text-sm text-[#141821] leading-relaxed">
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <p className="text-lg font-semibold text-[#141821] leading-snug mb-2">A collective of coaches practising to one standard.</p>
      ...
      <Card>
        ...
        <p className="text-[12.5px] font-semibold text-[#666D7A] mt-4 mb-2">Not for</p>
      ...
      <Card>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={r.k} className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-3 py-2 border-b border-[#E8EAEE]/60 last:border-0">
              <p className="text-sm font-medium text-[#141821] w-32 shrink-0">{r.k}</p>
              <p className="text-sm text-[#666D7A]">{r.v}</p>
            </div>
          ))}
        </div>
        <p className="text-[12.5px] text-[#666D7A] mt-3">
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={r.k} className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-3 py-2 border-b border-[#E8EAEE]/60 last:border-0">
              <p className="text-sm font-medium text-[#141821] w-44 shrink-0">{r.k}</p>
              <p className="text-sm text-[#666D7A]">{r.v}</p>
            </div>
          ))}
        </div>
      ...
      <Card>
        ...
        <p className="text-[12.5px] text-[#98A0AD] mt-3">Sold after a coach is live, never at the front. Exact figures stay a verbal + term-sheet conversation - the public site shows an ROI illustration, not a rate card.</p>
      </Card>
    </div>
  )
}
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <p className="text-sm text-[#666D7A] leading-relaxed mb-4">
      ...
      {pillars.map(p => (
        <Card key={p.n}>
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-base font-semibold text-[#141821] mb-1">{p.t}</p>
              <p className="text-sm text-[#666D7A] leading-relaxed">{p.d}</p>
            </div>
          </div>
        </Card>
      ))}
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <p className="text-sm text-[#666D7A] leading-relaxed">Where coaches, studio owners and founders are. The Collective is a business offer; LinkedIn is the business feed. Posted from Kade's profile alongside the Body Recode executive and personal-brand posts (total LinkedIn feed ~4-6/week).</p>
      </Card>
      <Card>
        <SectionLabel>Personal brand - Pillar 3 (AI & Leverage)</SectionLabel>
        <div className="mb-3"><Tag color="violet">~1x per week</Tag></div>
        <p className="text-sm text-[#666D7A] leading-relaxed">The building-in-public story on @kade_dunstone_ - "I built the engine to run my own coaching; now other coaches run their practice on it." Seeds interest and feeds the Fit Scorecard. Does not replace the LinkedIn track.</p>
      </Card>
      <Card>
        <SectionLabel>Email</SectionLabel>
        <p className="text-sm text-[#666D7A] leading-relaxed">Founding-story email on application; a 3-4 email nurture sequence for "building" applicants (the warmest future pipeline). Consumer Instagram (@body_recode_) is not a coach-recruitment channel - wrong audience.</p>
      </Card>
    </div>
  )
}
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <div className="p-3 rounded-lg border border-[#E8EAEE] bg-[#FBFCFD]/60">
          <div className="flex items-center gap-2 mb-1"><span className="text-sm font-semibold text-[#141821]">Melisa - Harmony Yoga</span><Tag color="amber">Verbal yes, not signed</Tag></div>
          <p className="text-[12.5px] text-[#666D7A] leading-relaxed">Collective Partner #1. Yoga modality, Brisbane. Terms locked with her 2026-07-15. Preview mocks built (/dashboard/preview/melisa + /harmony). Blocked on: signed agreement (lawyer), her brand pack + domain, $2,500 setup cleared, yoga branch merge.</p>
        </div>
        <p className="text-[12.5px] text-[#98A0AD] mt-3">9 remaining founding seats fed by the Fit Scorecard waitlist.</p>
      </Card>
      <Card>
        ...
        <p className="text-[12.5px] text-[#98A0AD] mt-3">When these land, the content posture flips from "apply to the waitlist" to "founding seats open" - same funnel, harder CTA.</p>
      </Card>
    </div>
  )
}
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-3.5 py-2 text-sm font-medium rounded-t-lg whitespace-nowrap transition-colors ${
                tab === t.id ? 'bg-white text-[#141821] border-t border-l border-r border-[#E8EAEE]' : 'text-[#666D7A] hover:text-[#141821]'
              }`}>{t.label}</button>
          ))}
        </div>
      </div>
