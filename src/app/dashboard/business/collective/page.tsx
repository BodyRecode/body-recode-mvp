...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="text-[11px] font-medium text-[#8A9099] mb-3">{children}</p>
}

function Tag({ children, color = 'sky' }: { children: React.ReactNode; color?: 'sky' | 'amber' | 'violet' | 'stone' | 'green' | 'red' }) {
  const styles: Record<string, string> = {
    sky:    'bg-[#FAFAF8]/10 text-[#1056D6] border-[#FAFAF8]/25',
...
function Bullets({ items, tone = 'stone' }: { items: string[]; tone?: 'stone' | 'plus' | 'minus' }) {
  ...
  const cls = tone === 'plus' ? 'text-[#FAFAF8]' : tone === 'minus' ? 'text-[#D4817E]' : 'text-[#676D76]'
  return (
    <div className="space-y-1.5">
      {items.map(s => (
        <div key={s} className="flex items-start gap-2 text-sm text-[#FAFAF8] leading-relaxed">
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <p className="text-lg font-semibold text-[#FAFAF8] leading-snug mb-2">A collective of coaches practising to one standard.</p>
        ...
        <div className="mt-4">
          <a href="/docs/strategy/collective/collective-gtm-content-strategy-v1.pdf" target="_blank" rel="noopener noreferrer"
            className="text-[12.5px] font-medium px-2 py-1 rounded bg-[#FAFAF8] text-[#0B0D10] hover:bg-[#1056D6]">View full strategy .pdf</a>
        </div>
      </Card>
      ...
      <Card>
        ...
        <p className="text-[12.5px] font-semibold text-[#8A9099] mt-4 mb-2">Not for</p>
      ...
      <Card>
        ...
        <div className="space-y-3">
          {[
            { k: 'Body Recode', v: 'Pure white · Signal Blue #FAFAF8 · sans-bold · BR logo' },
            ...
            <div key={r.k} className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-3 py-2 border-b border-[#2A2F39]/60 last:border-0">
              <p className="text-sm font-medium text-[#FAFAF8] w-32 shrink-0">{r.k}</p>
              <p className="text-sm text-[#8A9099]">{r.v}</p>
            </div>
          ))}
        </div>
        <p className="text-[12.5px] text-[#8A9099] mt-3">
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={r.k} className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-3 py-2 border-b border-[#2A2F39]/60 last:border-0">
              <p className="text-sm font-medium text-[#FAFAF8] w-44 shrink-0">{r.k}</p>
              <p className="text-sm text-[#8A9099]">{r.v}</p>
            </div>
          ))}
        </div>
      ...
      <Card>
        ...
        <p className="text-[12.5px] text-[#676D76] mt-3">Sold after a coach is live, never at the front. Exact figures stay a verbal + term-sheet conversation - the public site shows an ROI illustration, not a rate card.</p>
      </Card>
    </div>
  )
}
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <p className="text-sm text-[#8A9099] leading-relaxed mb-4">
          Content → <code className="text-[12.5px] bg-[#1A1E26] px-1 rounded">/collective</code> → <code className="text-[12.5px] bg-[#1A1E26] px-1 rounded">/collective/apply</code> (Fit Scorecard, ~9 questions) → scored across method / audience / modality / readiness → one of three tiers. Every CTA points to the Fit Scorecard, never a checkout.
        </p>
        <div className="space-y-3">
          <div className="p-3 rounded-lg border border-[#22A05A]/20 bg-[#EDF8F1]/40">
            <div className="flex items-center gap-2 mb-1"><Tag color="green">Ready</Tag><span className="text-sm font-semibold text-[#FAFAF8]">Book a call + instant SMS to Kade</span></div>
            <p className="text-[12.5px] text-[#8A9099]">Defined method + audience, supported modality. Goes on the founding waitlist.</p>
          </div>
          <div className="p-3 rounded-lg border border-[#B7791F]/20 bg-[#FDF6E9]/40">
            <div className="flex items-center gap-2 mb-1"><Tag color="amber">Building</Tag><span className="text-sm font-semibold text-[#FAFAF8]">Tailored "fix this first" + nurture</span></div>
            <p className="text-[12.5px] text-[#8A9099]">Method or audience but not both. This tier is also the Emerging Coach farm system - nurtured toward a future cohort.</p>
          </div>
          <div className="p-3 rounded-lg border border-[#D4817E]/20 bg-[#FDEDED]/40">
            <div className="flex items-center gap-2 mb-1"><Tag color="red">Not yet</Tag><span className="text-sm font-semibold text-[#FAFAF8]">Gentle decline + a resource</span></div>
            <p className="text-[12.5px] text-[#8A9099]">Unsupported modality, cheap-tool mindset, or too many reds.</p>
          </div>
        </div>
      </Card>
  ...
  return (
    <div className="space-y-4">
      {pillars.map(p => (
        <Card key={p.n}>
          <div className="flex items-start gap-4">
            <span className="text-2xl font-bold font-mono text-[#FAFAF8]/40">{p.n}</span>
            <div>
              <p className="text-base font-semibold text-[#FAFAF8] mb-1">{p.t}</p>
              <p className="text-sm text-[#8A9099] leading-relaxed">{p.d}</p>
            </div>
          </div>
        </Card>
      ))}
      <Card>
        ...
        <p className="text-sm text-[#8A9099] leading-relaxed">
          Established coaches hear <em className="text-[#FAFAF8] not-italic font-medium">"own a business, not rent a tool"</em> (they already have a business). Emerging coaches hear <em className="text-[#FAFAF8] not-italic font-medium">"learn to practise to one standard, and grow into the platform"</em> (aspirational / educational). Never mix the two in one post - different audience, different stage.
        </p>
      </Card>
    </div>
  )
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <p className="text-sm text-[#8A9099] leading-relaxed">Where coaches, studio owners and founders are. The Collective is a business offer; LinkedIn is the business feed. Posted from Kade's profile alongside the Body Recode executive and personal-brand posts (total LinkedIn feed ~4-6/week).</p>
      </Card>
      <Card>
        <SectionLabel>Personal brand - Pillar 3 (AI & Leverage)</SectionLabel>
        <div className="mb-3"><Tag color="violet">~1x per week</Tag></div>
        <p className="text-sm text-[#8A9099] leading-relaxed">The building-in-public story on @kade_dunstone_ - "I built the engine to run my own coaching; now other coaches run their practice on it." Seeds interest and feeds the Fit Scorecard. Does not replace the LinkedIn track.</p>
      </Card>
      <Card>
        <SectionLabel>Email</SectionLabel>
        <p className="text-sm text-[#8A9099] leading-relaxed">Founding-story email on application; a 3-4 email nurture sequence for "building" applicants (the warmest future pipeline). Consumer Instagram (@body_recode_) is not a coach-recruitment channel - wrong audience.</p>
      </Card>
    </div>
  )
}
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <div className="p-3 rounded-lg border border-[#2A2F39] bg-[#14171D]/60">
          <div className="flex items-center gap-2 mb-1"><span className="text-sm font-semibold text-[#FAFAF8]">Melisa - Harmony Yoga</span><Tag color="amber">Verbal yes, not signed</Tag></div>
          <p className="text-[12.5px] text-[#8A9099] leading-relaxed">Collective Partner #1. Yoga modality, Brisbane. Terms locked with her 2026-07-15. Preview mocks built (/dashboard/preview/melisa + /harmony). Blocked on: signed agreement (lawyer), her brand pack + domain, $2,500 setup cleared, yoga branch merge.</p>
        </div>
        <p className="text-[12.5px] text-[#676D76] mt-3">9 remaining founding seats fed by the Fit Scorecard waitlist.</p>
      </Card>
      <Card>
        ...
        <p className="text-[12.5px] text-[#676D76] mt-3">When these land, the content posture flips from "apply to the waitlist" to "founding seats open" - same funnel, harder CTA.</p>
      </Card>
    </div>
  )
}
    ...
    <div className="flex flex-col h-full">
      <div className="px-6 pt-6 pb-0 border-b border-[#2A2F39]">
        <div className="flex items-center gap-3 mb-1">
          <h1 className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em]">The Body Recode Collective</h1>
          <span className="text-[12.5px] text-[#8A9099] bg-[#1F242C] px-2 py-0.5 rounded font-mono">bodyrecode.au/collective</span>
        </div>
        <p className="text-sm text-[#8A9099] mb-4">The B2B licensing arm - a collective of coaches practising to one standard</p>

        <div className="flex gap-1 overflow-x-auto pb-px">
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-3.5 py-2 text-sm font-medium rounded-t-lg whitespace-nowrap transition-colors ${
                tab === t.id ? 'bg-[#14171D] text-[#FAFAF8] border-t border-l border-r border-[#2A2F39]' : 'text-[#8A9099] hover:text-[#FAFAF8]'
              }`}>{t.label}</button>
          ))}
        </div>
      </div>
