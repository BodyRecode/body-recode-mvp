    ...
    <div className="max-w-2xl">
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-medium text-stone-400 mb-2">Funnel Name</label>
            <input
              type="text"
              value={name}
              onChange={e => {
              ...
              className="w-full bg-stone-900 border border-stone-800 rounded-lg px-4 py-2.5 text-sm text-white placeholder-stone-600 focus:outline-none focus:border-teal-500"
            />
          </div>
          <div>
            ...
            <div className="flex items-center bg-stone-900 border border-stone-800 rounded-lg overflow-hidden focus-within:border-teal-500">
              <span className="pl-3 text-stone-600 text-xs shrink-0">/f/</span>
              <input
                type="text"
                value={slug}
                onChange={e => setSlug(autoSlug(e.target.value))}
                placeholder="free-consultation"
                className="flex-1 bg-transparent px-2 py-2.5 text-sm text-white placeholder-stone-600 focus:outline-none"
              />
            </div>
          </div>
        </div>
        ...
        {/* Active toggle */}
        <div className="flex items-center justify-between bg-stone-900 border border-stone-800 rounded-lg px-4 py-3">
          <div>
          ...
          <button
            ...
            className={`relative w-10 h-6 rounded-full transition-colors ${isActive ? 'bg-teal-500' : 'bg-stone-700'}`}
          >
        ...
        {/* Headline */}
        <div>
          <label className="block text-xs font-medium text-stone-400 mb-2">Headline</label>
          <input
            type="text"
            value={headline}
        ...
        {/* Subheadline */}
        <div>
          <label className="block text-xs font-medium text-stone-400 mb-2">Subheadline</label>
          <input
            type="text"
            value={subheadline}
        ...
        {/* Body */}
        <div>
          <label className="block text-xs font-medium text-stone-400 mb-2">Body Copy</label>
          <textarea
            value={body}
            onChange={e => setBody(e.target.value)}
            placeholder="Describe what they'll get, who it's for, what happens next..."
            rows={6}
            className="w-full bg-stone-900 border border-stone-800 rounded-lg px-4 py-3 text-sm text-white placeholder-stone-600 focus:outline-none focus:border-teal-500 resize-none"
          />
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-medium text-stone-400 mb-2">Button Label</label>
            <input
              type="text"
              value={ctaLabel}
          ...
          <div>
            <label className="block text-xs font-medium text-stone-400 mb-2">After Submit</label>
            <div className="relative">
              <select
                value={redirectTo}
                onChange={e => setRedirectTo(e.target.value)}
                className="w-full appearance-none bg-stone-900 border border-stone-800 rounded-lg px-4 py-2.5 pr-8 text-sm text-white focus:outline-none focus:border-teal-500"
              >
        ...
        <div className="pt-2">
          <button
            onClick={save}
            disabled={saving}
            className="bg-teal-500 hover:bg-teal-400 disabled:opacity-50 text-stone-950 text-sm font-semibold px-6 py-2.5 rounded-lg transition-colors"
          >
