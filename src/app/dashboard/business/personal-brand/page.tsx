...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-[11px] font-medium text-[#8A9099] mb-3">
      {children}
    </p>
  )
}
...
function Tag({ children, color = 'teal' }: { children: React.ReactNode; color?: 'teal' | 'violet' | 'amber' | 'orange' | 'stone' }) {
  const styles: Record<string, string> = {
    ...
    orange: 'bg-[#1A1E26]/10 text-[#C2C6CC] border-[#2A2F39]/20',
    stone:  'bg-[#8A9099]/10 text-[#8A9099] border-[#2A2F39]/20',
  }
  return (
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <div className="mt-4 flex flex-wrap gap-3">
          {[
            ...
            <div key={s.name} className="flex items-center gap-2 border border-[#2A2F39] rounded-lg px-3 py-2">
              <span className="w-6 h-6 rounded" style={{ background: s.hex, border: '1px solid rgba(0,0,0,0.08)' }} />
              <div>
                <p className="text-[12.5px] font-medium text-[#FAFAF8]">{s.name}</p>
                <p className="text-[10px] text-[#8A9099] font-mono">{s.hex}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <Card>
        ...
        <p className="text-[12.5px] text-[#8A9099] mb-4">Every post is one of these, always in clay. Live previews:</p>
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            <div key={p.label}>
              ...
              <img src={p.url} alt={p.label} className="w-full rounded-lg border border-[#2A2F39]" />
              <p className="text-[10px] text-[#8A9099] mt-1 text-center">{p.label}</p>
            </div>
          ))}
        </div>
      ...
      <Card>
        ...
        <p className="text-[12.5px] text-[#8A9099] mb-4">
          The launch month <strong className="text-[#8A9099]">leads with the human</strong> &mdash; story + photo posts carry it, two framework carousels teach, and sharp cards punctuate. Story hooks are specific scenes, not maxims; you write the detail in the caption. <strong className="text-[#8A9099]">Reels start as a Phase-2 block from w/c 6 Jul</strong>, once the HeyGen avatar is ready (talking-head is where the relationship really forms). All <strong className="text-[#8A9099]">scheduled in the one Content Calendar</strong> (Marketing Strategy → Content Calendar, filter Personal Brand) &mdash; this is the plan reference.
        </p>
        <div className="space-y-4">
          {KIT_PLAN.map(wk => (
            <div key={wk.week}>
              <div className="flex items-baseline justify-between mb-2">
                <p className="text-sm font-semibold text-[#FAFAF8]">{wk.week}</p>
                <p className="text-[12.5px] text-[#8A9099]">{wk.range}</p>
              </div>
              <div className="space-y-1.5">
                {wk.posts.map(p => (
                  <div key={p.date} className="flex items-start gap-3 py-1.5 border-b border-[#2A2F39]/60 last:border-0">
                    <span className="text-[12.5px] font-mono text-[#8A9099] w-12 shrink-0">{p.date}</span>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                        <Tag color={KIT_PILLAR_COLOR[p.pillar]}>{p.pillar}</Tag>
                        <Tag color={formatColor[p.format]}>{p.format}</Tag>
                        <span className="text-[10px] text-[#676D76]">{p.treatment}</span>
                      </div>
                      <p className="text-sm text-[#FAFAF8] leading-snug">{p.hook}</p>
                    </div>
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <p className="text-[#FAFAF8] text-sm leading-relaxed">
        ...
        <div className="mt-4 space-y-2">
          {[
            ...
            <div key={s} className="flex items-center gap-2 text-sm text-[#8A9099]">
              <span className="text-[#D4817E]">-</span> {s}
            </div>
          ))}
          <div className="flex items-start gap-2 text-sm text-[#FAFAF8] mt-3">
      ...
      <Card>
        ...
        <p className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em] mb-3">Interpret before you act.</p>
        <p className="text-[#8A9099] text-sm leading-relaxed">
      ...
      <Card>
        ...
        <div className="bg-[#1A1E26] rounded-lg p-4 text-sm text-[#FAFAF8] leading-relaxed font-mono">
      ...
      <Card>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={r.platform} className="flex items-center justify-between py-2 border-b border-[#2A2F39] last:border-0">
              <div>
                <p className="text-sm font-medium text-[#FAFAF8]">{r.platform}</p>
                <p className="text-[12.5px] text-[#8A9099]">{r.handle}</p>
              </div>
              <code className="text-[12.5px] text-[#FAFAF8] bg-[rgba(27,109,252,0.08)] px-2 py-1 rounded">{r.link}</code>
            </div>
          ))}
        </div>
        <p className="mt-3 text-[12.5px] text-[#8A9099]">Profile photo: clean headshot, dark background, on-brand</p>
      </Card>
      ...
      <Card>
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="flex items-start gap-3 py-2 border-b border-[#2A2F39]/60 last:border-0">
              <span className="text-[12.5px] text-[#676D76] font-mono mt-0.5 w-4 shrink-0">{i + 1}</span>
              <p className="text-sm text-[#FAFAF8]">{p}</p>
            </div>
          ))}
        </div>
      </Card>
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <div className="space-y-4">
          {rebuilds.map(r => (
            <div key={r.title} className="border border-[#2A2F39] rounded-lg overflow-hidden">
              <div className={`px-4 py-2.5 border-b border-[#2A2F39] ${
                r.color === 'teal' ? 'bg-[#FAFAF8]/5' :
                r.color === 'violet' ? 'bg-[#1A1E26]/5' : 'bg-[#B7791F]/5'
              }`}>
                <p className="text-sm font-semibold text-[#FAFAF8]">{r.title}</p>
              </div>
              <div className="px-4 py-3 space-y-2">
                <div>
                  <p className="text-[10px] text-[#676D76] mb-1">Context</p>
                  <p className="text-sm text-[#8A9099]">{r.context}</p>
                </div>
                <div>
                  <p className="text-[10px] text-[#676D76] mb-1">What it produced</p>
                  <p className="text-sm text-[#FAFAF8]">{r.lesson}</p>
                </div>
              </div>
            </div>
          ))}
        ...
        <Card key={p.num}>
          <div className="flex items-start gap-4">
            <span className={`text-2xl font-bold font-mono ${
              p.color === 'teal' ? 'text-[#FAFAF8]/40' :
              p.color === 'violet' ? 'text-[#C2C6CC]/40' :
              p.color === 'amber' ? 'text-[#B7791F]/40' : 'text-[#C2C6CC]/40'
            }`}>{p.num}</span>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <p className="text-base font-semibold text-[#FAFAF8]">Pillar {p.num} - {p.title}</p>
                <Tag color={p.color}>{p.color === 'teal' ? 'Body' : p.color === 'violet' ? 'Mind' : p.color === 'amber' ? 'AI' : 'Story'}</Tag>
              </div>
              <p className="text-sm text-[#8A9099] mb-3">{p.description}</p>
              <div className="space-y-1">
                {p.topics.map(t => (
                  <div key={t} className="flex items-start gap-2 text-sm text-[#FAFAF8]">
                    <span className="text-[#676D76] mt-1 shrink-0">-</span>
                    {t}
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="space-y-4">
        ...
        {(Object.entries(scriptSections) as [typeof scriptTab, { label: string }][]).map(([k, v]) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
              scriptTab === k
                ? 'bg-[rgba(27,109,252,0.08)] text-[#FAFAF8] border-[#2A2F39]'
                : 'text-[#8A9099] border-[#2A2F39] hover:text-[#FAFAF8]'
            }`}
          >{v.label}</button>
        ))}
      </div>
      ...
      <Card>
        ...
        {scriptTab === 'launch' && <p className="text-[12.5px] text-[#8A9099] mb-4">Post these in order before any outreach or promotion begins.</p>}
        {scriptTab === 'video' && <p className="text-[12.5px] text-[#8A9099] mb-4">Record using HeyGen avatar. Calm delivery, clean background. 60-90 seconds.</p>}
        <div className="space-y-2">
          {current.scripts.map((s, i) => (
            <div key={s.id} className="border border-[#2A2F39] rounded-lg overflow-hidden">
              <button
                onClick={() => setExpanded(expanded === s.id ? null : s.id)}
                className="w-full flex items-center justify-between px-4 py-3 hover:bg-[#1A1E26]/50 transition-colors text-left"
              >
                <div className="flex items-center gap-3 min-w-0">
                  <span className="text-[12.5px] text-[#676D76] font-mono shrink-0">{String(i + 1).padStart(2, '0')}</span>
                  <span className="text-sm font-medium text-[#FAFAF8] truncate">{s.hook}</span>
                  <Tag color={s.pillarColor}>{s.type}</Tag>
                </div>
                <span className="text-[#676D76] text-[12.5px] shrink-0 ml-2">{expanded === s.id ? 'hide' : 'view'}</span>
              </button>
              {expanded === s.id && (
                <div className="px-4 pb-4 space-y-3 border-t border-[#2A2F39]">
                  <div className="flex items-center gap-3 mt-3">
                    <Tag color={s.pillarColor}>{s.pillar}</Tag>
                    <span className="text-[12.5px] text-[#8A9099]">{s.platform}</span>
                  </div>
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-[10px] text-[#676D76]">Copy</p>
                      <button
                    ...
                    <div className="bg-[#1A1E26] rounded-lg p-3 text-sm text-[#FAFAF8] leading-relaxed whitespace-pre-line">
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={r.type} className="flex items-center gap-3 py-2 border-b border-[#2A2F39]/60 last:border-0">
              <p className="text-sm font-medium text-[#FAFAF8] w-28 shrink-0">{r.type}</p>
              <p className="text-sm text-[#8A9099]">{r.desc}</p>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card>
        ...
        <p className="text-sm text-[#8A9099] leading-relaxed mb-3">
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={r.channel} className="flex items-center gap-3 text-[12.5px] py-1.5 border-b border-[#2A2F39]/60 last:border-0">
              <p className="text-[#FAFAF8] w-52 shrink-0">{r.channel}</p>
              <p className="text-[#8A9099]">{r.freq}</p>
            </div>
          ))}
        </div>
        <p className="text-[12.5px] text-[#676D76] mt-3">See <strong className="text-[#8A9099]">Marketing Strategy → LinkedIn tab</strong> for the Body Recode LinkedIn pillars, tone rules, and 12-week pipeline.</p>
      </Card>

      <Card>
        <SectionLabel>Tone</SectionLabel>
        <p className="text-sm text-[#FAFAF8] mb-4">Direct. Structured. No filler. Reads like someone who has thought about this for years and is now documenting it.</p>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-[10px] text-[#D4817E]/60 mb-2">Never</p>
            <div className="space-y-1">
              {['Motivational fluff', '"Here\'s my morning routine"', 'Gym selfies', 'Vague inspiration', 'Hype'].map(t => (
                <div key={t} className="flex items-center gap-2 text-[12.5px] text-[#8A9099]">
                  <span className="text-[#D4817E]/60">-</span> {t}
                </div>
              ))}
            </div>
          </div>
          <div>
            <p className="text-[10px] text-[#FAFAF8]/60 mb-2">Always</p>
            <div className="space-y-1">
              {['A specific observation', 'A named pattern', 'A reframe', 'A real moment from the journey'].map(t => (
                <div key={t} className="flex items-center gap-2 text-[12.5px] text-[#FAFAF8]">
                  <span className="text-[#FAFAF8]/60">+</span> {t}
                </div>
              ))}
            </div>
          </div>
      ...
      <Card>
        ...
        <p className="text-sm text-[#8A9099] mb-4">The personal brand does not sell Body Recode directly. It builds the trust that makes Body Recode the obvious next step.</p>
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="flex items-start gap-3 text-sm text-[#FAFAF8]">
              <span className="text-[#676D76] font-mono text-[12.5px] mt-0.5 w-4 shrink-0">{i + 1}</span>
              {s}
            </div>
          ))}
        </div>
        <div className="mt-4 p-3 bg-[#1A1E26] rounded-lg border border-[#2A2F39]">
          <p className="text-[12.5px] text-[#8A9099]">Never pitch Body Recode directly from the personal account. Reference it. Link it occasionally. Let the trust do the selling.</p>
        </div>
      </Card>
    </div>
  )
  ...
  return (
    <div className="space-y-5">
      <Card>
        ...
        <div className="space-y-2">
          {steps.map((s, i) => (
            <div key={i} className="flex items-center gap-3 py-2 border-b border-[#2A2F39]/60 last:border-0">
              <div className="w-4 h-4 rounded border border-[#2A2F39] shrink-0" />
              <p className="text-sm text-[#FAFAF8]">{s.step}</p>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={r.platform} className="flex items-center justify-between py-2 border-b border-[#2A2F39]/60 last:border-0">
              <p className="text-sm text-[#8A9099] w-36 shrink-0">{r.platform}</p>
              <code className="text-[12.5px] text-[#FAFAF8] bg-[rgba(27,109,252,0.08)] px-2 py-1 rounded">{r.url}</code>
            </div>
          ))}
        </div>
      </Card>
      ...
      <Card>
        ...
        <div className="space-y-3 text-sm text-[#8A9099]">
          ...
          <div className="mt-4 space-y-2">
            {[
              ...
              <div key={i} className="flex items-start gap-3">
                <span className="text-[#676D76] font-mono text-[12.5px] mt-0.5 shrink-0">{i + 1}</span>
                <span className="text-[#FAFAF8]">{s}</span>
              </div>
            ))}
          </div>
        </div>
...
export default function PersonalBrandPage() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="px-6 pt-6 pb-0 border-b border-[#2A2F39]">
        <div className="flex items-center gap-3 mb-1">
          <h1 className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em]">Personal Brand</h1>
          <span className="text-[12.5px] text-[#8A9099] bg-[#1F242C] px-2 py-0.5 rounded font-mono">@kade_dunstone_</span>
        </div>
        <p className="text-sm text-[#8A9099] mb-4">Instagram - the thinking, the journey, the builder</p>
          ...
          {TABS.map(t => (
            <button
              ...
              className={`px-3.5 py-2 text-sm font-medium rounded-t-lg whitespace-nowrap transition-colors ${
                tab === t.id
                  ? 'bg-[#14171D] text-[#FAFAF8] border-t border-l border-r border-[#2A2F39]'
                  : 'text-[#8A9099] hover:text-[#FAFAF8]'
              }`}
            >
              {t.label}
            </button>
