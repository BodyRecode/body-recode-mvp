          ...
          <Card className="border-blue-500/30 bg-blue-500/5">
            ...
            <div className="mt-3 space-y-2 text-xs">
              ...
              <div className="bg-stone-50 border border-stone-200 rounded-lg p-3">
                ...
                <p className="text-stone-700 leading-relaxed mb-2"><strong>If it misses:</strong> DO NOT narrow the targeting. Narrowing is what caused the July exhaustion. Debug in order: (1) creative - swap in a format-breaker, one at a time, same ad set, (2) in-portal intake friction, (3) /decode LP conversion, (4) the offer itself.</p>
                ...
                <p className="text-stone-700 leading-relaxed mt-2"><strong className="text-amber-700">Broad is all genders, and The Body Decode is not.</strong> The page opens &quot;for women whose bodies have stopped responding&quot;, the signup form turns men away, and the scorecard routes them to a strategy call instead (25 Aug). Men were <strong>3 of the 29 typed leads, about 10%</strong>. <strong>Do NOT add a gender filter to fix it</strong> - the optimisation event below is the actual lever.</p>
              </div>

              <div className="bg-white/70 border border-violet-500/20 rounded-lg p-3">
                <p className="text-[10px] font-bold text-violet-700 uppercase tracking-widest mb-1">Pre-launch · check the optimisation event</p>
                <p className="text-stone-700 leading-relaxed"><strong>Which event the campaign optimises for decides whether broad self-corrects.</strong> Checked 25 Aug 2026.</p>
                <p className="text-stone-700 leading-relaxed mt-2"><strong><code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-[11px]">Lead</code> fires on ONE thing</strong> - a Body Decode enrolment, in <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-[11px]">/api/challenge/enroll</code>. Men are blocked from that route, so a man can never fire it. Optimise on <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-[11px]">Lead</code> and the algorithm stops serving men within days on its own - <strong>a gender filter buys nothing the optimiser was not going to do anyway</strong>, and costs the freedom that made broad work after July.</p>
                <p className="text-stone-700 leading-relaxed mt-2"><strong className="text-amber-700">The trap:</strong> a Body Decode enrolment fires <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-[11px]">CompleteRegistration</code> as well, deliberately - Meta locks an ad set&apos;s conversion event after publish, so both are fired to keep the swap open without rebuilding the ad set. But <strong>the scorecard fires <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-[11px]">CompleteRegistration</code> too, and men CAN complete the scorecard.</strong> So on that event a man counts as a conversion while never being able to enrol, and Meta keeps buying traffic that cannot convert downstream. On <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-[11px]">Lead</code> he cannot. <strong>Fix the event, not the audience.</strong></p>
                <p className="text-stone-700 leading-relaxed mt-2"><strong>/book now fires <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-[11px]">Schedule</code></strong> (25 Aug). It fired nothing before, so every strategy call the funnel produced was invisible - and that was never a male-traffic issue, which is how it was first framed: <strong>emails 3, 4 and 5 of the female sequence push /book, and so does the scorecard result for Transitioning and Ready.</strong> Meta was blind to all of it. <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-[11px]">Schedule</code> is a Meta standard event, so it reports without defining a custom conversion.</p>
                <p className="text-stone-700 leading-relaxed mt-2"><strong className="text-amber-700">MEASUREMENT ONLY. Do not make Schedule an ad set&apos;s conversion event.</strong> Bookings are rare next to Decode signups, and Meta needs roughly 50 conversions a week per ad set to leave the learning phase. At $25/day, optimising on the rare event starves it. Keep the ad sets on <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-[11px]">Lead</code>; in a month the Schedule count answers whether calls are worth chasing, on evidence rather than argument.</p>
                <p className="text-stone-700 leading-relaxed mt-2"><strong>Revisit gender targeting after Round 2 has run</strong>, on real numbers rather than three. Note the July exhaustion was <em>interest</em> narrowing - lookalikes and interest stacks shrink the deliverable pool, a demographic filter does not. The reason to hold off is that changing targeting and creative in the same round confounds the read.</p>
              </div>
          ...
          <Card className="border-violet-500/30 bg-violet-500/5">
            ...
            <div className="mt-3 space-y-2 text-xs">
              ...
              <div className="bg-white/70 border border-violet-500/20 rounded-lg p-3">
                ...
                <p className="text-stone-700 leading-relaxed">Organic doctrine posts point at <strong>/decode</strong>, not the scorecard, so the two do not compete for the same reader. The read is not skipped: <strong>the questions run inside the portal</strong>, and anyone arriving from the scorecard has them prefilled rather than being asked twice. When paid goes dark, routing <strong>reverts to scorecard-first</strong>. A scoped exception to the locked rule, not a repeal.</p>
              </div>
              <div className="bg-white/70 border border-violet-500/20 rounded-lg p-3">
