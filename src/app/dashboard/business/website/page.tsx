...
function StatCard({ label, value, sub, highlight }: {
  ...
  return (
    <div className={`bg-[#14171D] border rounded-xl p-5 ${highlight ? 'border-[#9CC0FB]' : 'border-[#2A2F39]'}`}>
      <p className="text-[12.5px] font-medium text-[#8A9099] mb-2">{label}</p>
      <p className={`text-3xl font-black ${highlight ? 'text-[#FAFAF8]' : 'text-[#FAFAF8]'}`}>{value}</p>
      {sub && <p className="text-[12.5px] mt-1 font-medium text-[#8A9099]">{sub}</p>}
    </div>
  )
}
...
function Insight({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="br-card p-5">
      <p className="text-[12.5px] font-medium text-[#8A9099] mb-3">{title}</p>
      <div className="space-y-2 text-sm text-[#8A9099] leading-relaxed">
        {children}
      </div>
    </div>
  )
...
function DailyChart({ data }: { data: DayData[] }) {
  if (!data || data.length === 0) return <div className="br-card p-5"><p className="text-sm text-[#676D76]">No data yet.</p></div>
  const max = data.reduce((m, d) => Math.max(m, d.views), 1)
  const hasData = data.some(d => d.views > 0)

  return (
    <div className="br-card p-5">
      <p className="text-[12.5px] font-medium text-[#8A9099] mb-5">Daily Page Views</p>
      {!hasData ? (
        <p className="text-sm text-[#676D76] py-4">No data yet for this period.</p>
      ) : (
          ...
          {data.map((d) => {
            ...
            return (
              <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group relative">
                {d.views > 0 && (
                  <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-[#1F242C] border border-[#2A2F39] rounded px-1.5 py-0.5 text-[10px] text-[#FAFAF8] whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                    {d.views} views · {d.visitors} visitors
                  </div>
                )}
                <div
                  className={`w-full rounded-sm transition-all ${isToday ? 'bg-[#FAFAF8]' : d.views > 0 ? 'bg-[#8A9099] group-hover:bg-[#8A9099]' : 'bg-[#1A1E26]'}`}
                  style={{ height: `${height}%`, minHeight: d.views > 0 ? '4px' : '2px' }}
                />
                {data.length <= 14 && (
                  <span className="text-[10px] text-[#676D76] rotate-0 truncate w-full text-center">{label}</span>
                )}
              </div>
            )
          })}
      ...
      {data.length > 14 && (
        <div className="flex justify-between mt-2">
          <span className="text-[10px] text-[#676D76]">{data[0]?.date}</span>
          <span className="text-[10px] text-[#676D76]">{data[data.length - 1]?.date}</span>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex items-center justify-between br-page-header sticky top-0 z-20 mb-7 pt-4 pb-3.5 border-b border-[#2A2F39] bg-[#14171D]/[0.88] backdrop-blur-md print:static print:bg-transparent">
        <div>
          <h1 className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em]">Website</h1>
          <p className="text-sm text-[#8A9099] mt-0.5">performance.bodyrecode.au</p>
        </div>
        <div className="flex items-center gap-2">
          {[7, 30, 90].map(d => (
            <button
              key={d}
              onClick={() => setDays(d)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors ${days === d ? 'bg-[#DDE9FD] text-[#FAFAF8] border border-[#2A2F39]' : 'bg-[#1F242C] text-[#8A9099] border border-[#2A2F39] hover:text-[#FAFAF8]'}`}
            >
              {d}d
            </button>
          ))}
          <a
            ...
            className="ml-2 px-3 py-1.5 rounded-lg text-[12.5px] font-semibold bg-[#1F242C] border border-[#2A2F39] text-[#8A9099] hover:text-[#FAFAF8] transition-colors"
          >
            View site ↗
          </a>
        </div>
      ...
      {loading && <p className="text-sm text-[#8A9099] py-8 text-center">Loading analytics...</p>}

      {error && (
        <div className="bg-[#FDEDED] border border-[#F5C9C9] rounded-xl p-5">
          <p className="text-sm font-semibold text-[#D4817E] mb-1">Analytics unavailable</p>
          <p className="text-[12.5px] text-[#8A9099]">{error}</p>
        </div>
      )}
      ...
      {!loading && data && (
        <>
          ...
          <Insight title="What these numbers mean">
            ...
            <p>
              <span className="text-[#FAFAF8] font-medium">Bounce rate</span> measures sessions where the visitor left without going to a second page. A high bounce rate on a scorecard-focused site isn&apos;t necessarily bad - if someone lands directly on /scorecard and completes it, that counts as a bounce even though it was a conversion. Context matters more than the raw number.
            </p>
          </Insight>
          ...
          <Insight title="How to read the traffic pattern">
            ...
            <p>
              <span className="text-[#FAFAF8] font-medium">What to look for:</span> spikes that line up with posts or DMs you sent that day. If a post goes out on Monday and traffic jumps Tuesday, that&apos;s the post working. If traffic is flat across the week with no spikes, content is either not reaching people or not compelling them to click through.
            </p>
            <p>
              <span className="text-[#FAFAF8] font-medium">What needs to keep happening:</span> daily Instagram activity - posts, stories, and direct outreach - is what drives consistent traffic. The chart should start showing a rhythm that maps to your posting schedule. Weeks without posts will show up as flatlines.
            </p>
            {isSparse && (
      ...
      {/* Pages */}
      <div className="br-card p-5">
        <p className="text-[12.5px] font-medium text-[#8A9099] mb-2">Live Pages</p>
        <p className="text-sm text-[#8A9099] mb-4">All six pages are live. The scorecard is the primary conversion point - every other page should funnel toward it. The Founder Program page is the current active offer.</p>
        <div className="grid sm:grid-cols-2 gap-2">
          {[
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-start justify-between px-3 py-3 bg-[#1A1E26] hover:bg-[#1F242C] border border-[#2A2F39] rounded-lg transition-colors group"
            >
              <div>
                <span className="text-sm text-[#FAFAF8] group-hover:text-[#FAFAF8] transition-colors block">{p.label}</span>
                <span className="text-[12.5px] text-[#676D76] mt-0.5 block">{p.note}</span>
              </div>
              <span className="text-[12.5px] text-[#676D76] group-hover:text-[#8A9099] transition-colors shrink-0 ml-3 mt-0.5">{p.path} ↗</span>
            </a>
          ))}
        </div>
      </div>
      ...
      <Insight title="What needs to keep happening">
        <p>
          <span className="text-[#FAFAF8] font-medium">Content drives traffic.</span> The website does not rank organically yet and there are no paid ads running. Every visitor right now is coming from Instagram - either from posts, stories, or direct links you send in DMs. The number one lever is posting consistently and directing people to the scorecard.
        </p>
        <p>
          <span className="text-[#FAFAF8] font-medium">The scorecard is the only metric that matters right now.</span> Visitors are a means to an end. A visitor who doesn&apos;t submit the scorecard doesn&apos;t enter the system. Focus content and outreach on getting people to /scorecard - that is where traffic becomes pipeline.
        </p>
        <p>
          <span className="text-[#FAFAF8] font-medium">As traffic grows, watch the chart for content-traffic correlation.</span> Once there are 30+ days of data, patterns will become clear. Posts that spike traffic are your highest-performing content - double down on those formats and topics. Posts with no visible traffic effect need to be reconsidered.
        </p>
        <p>
          <span className="text-[#FAFAF8] font-medium">SEO is a longer-term play.</span> The site copy is strong and structured well. Over 3-6 months, search visibility for relevant terms (online fitness coaching Brisbane, body composition coaching, etc.) will build. This will layer organic traffic on top of the social-driven traffic you are building now.
        </p>
      </Insight>
