    ...
    <div className="mb-4 br-card overflow-hidden">
      ...
      <div className="p-5">
        <p className="text-[13px] text-[#6E747D] leading-relaxed mb-4">
          Route additional domains to this tenant. After adding a domain, point it via CNAME to <code className="bg-[#F2F2EF] px-1 py-0.5 rounded text-[12px]">cname.vercel-dns.com</code> in your DNS registrar, then copy the env var line below into Vercel and redeploy.
        </p>
        ...
        {loading ? (
          <div className="text-[13px] text-[#6E747D]">Loading domains…</div>
        ) : domains.length === 0 ? (
          <div className="text-[13px] text-[#6E747D] italic mb-4">No custom domains yet.</div>
        ) : (
                ...
                {domains.map((d) => (
                  <tr key={d.id}>
                    ...
                    <td className="px-3 py-2">
                      {d.is_primary ? (
                        <span className="text-[11.5px] font-medium bg-[#F2F2EF] text-[#000000] px-1.5 py-0.5 rounded">Primary</span>
                      ) : (
                    ...
                    <td className="px-3 py-2 text-right">
                      <button
                        ...
                        disabled={pending}
                        className="text-[12px] text-[#8F2D2D] hover:text-[#8F2D2D] underline disabled:opacity-40"
                      >
                        Remove
                      </button>
                    </td>
        ...
        <form onSubmit={handleAdd} className="mb-4 p-3 rounded-lg border border-[#E4E4E0] bg-[#FAFAF8]">
          ...
          <label className="flex items-center gap-2 mt-3">
            <input
              ...
              disabled={pending}
            />
            <span className="text-[12px] text-[#0F1115]">Mark as primary (used for absolute URLs in emails, deposes any existing primary)</span>
          </label>
        </form>
        ...
        {envVarLine && (
          <div className="p-3 rounded-lg border border-[#DCDCD7] bg-[rgba(27,109,252,0.08)]">
            ...
            <p className="text-[12px] text-[#000000] leading-relaxed mb-2">
            ...
            <div className="p-2 rounded bg-white border border-[#F2F2EF] font-mono text-[12px] text-[#0F1115] break-all select-all">{envVarLine}</div>
          </div>
        )}
      </div>
    </div>
