    ...
    <div className="max-w-[1100px]">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] font-medium text-[#676D76]">Window</span>
          {WINDOWS.map(w => (
            <Link
              key={w.value}
              ...
              className={`px-2.5 py-1 text-xs rounded-md border transition-colors ${
                w.value === days
                  ? 'bg-[#FAFAF8] border-[#FAFAF8] text-[#0B0D10]'
        ...
        {doctrineVersions.length > 0 && (
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[10px] font-medium text-[#676D76]">Doctrine</span>
            <Link
      ...
      {totalRequests === 0 ? (
        ...
        <>
          {/* Headline numbers */}
          <Card>
            <p className="text-[10px] font-medium text-[#676D76] mb-4">
              Last {days} days · {doctrineParam ? `doctrine v${doctrineParam}` : 'all doctrine versions'}
            </p>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              <div>
                <p className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em] tabular-nums">{totalRequests.toLocaleString()}</p>
                <p className="text-[11px] text-[#8A9099] mt-0.5">Total requests</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-[#177245] tabular-nums">{pct(passedFirst)}</p>
                <p className="text-[11px] text-[#8A9099] mt-0.5">Passed first-pass Haiku</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-[#E4E4E0] tabular-nums">{pct(passedRetry + sonnetSucceeded)}</p>
                <p className="text-[11px] text-[#8A9099] mt-0.5">Passed on retry / Sonnet</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-[#A96A12] tabular-nums">{pct(sonnetFailed)}</p>
                <p className="text-[11px] text-[#8A9099] mt-0.5">Sonnet fell back to Haiku</p>
              </div>
              <div>
                <p className="text-2xl font-bold text-[#D4817E] tabular-nums">{pct(returned422)}</p>
                <p className="text-[11px] text-[#8A9099] mt-0.5">Returned 422 to coach</p>
              </div>
            </div>
          </Card>
          ...
          <Card className="mt-4">
            ...
            {ruleRows.length === 0 ? (
              <p className="text-sm text-[#8A9099]">No validator rules fired in this window. Every plan validated clean — possibly a sign the rules aren&apos;t tight enough, or the engine is in great shape.</p>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="text-left border-b border-[#2A2F39]">
                      <th className="pb-2 pr-3 text-[10px] font-medium text-[#8A9099]">Rule</th>
                      <th className="pb-2 pr-3 text-[10px] font-medium text-[#8A9099] text-right">Requests</th>
                      <th className="pb-2 pr-3 text-[10px] font-medium text-[#8A9099] text-right">Fire rate</th>
                      <th className="pb-2 text-[10px] font-medium text-[#8A9099]">Doctrine intent</th>
                    </tr>
                  </thead>
                  <tbody>
                    {ruleRows.map(r => {
                      ...
                      return (
                        <tr key={r.code} className={`border-b border-[#1A1E26] ${isHigh ? 'bg-[#FDEDED]/40' : ''}`}>
                          <td className="py-2.5 pr-3 align-top">
                            <p className="font-mono text-[12.5px] text-[#FAFAF8]">{r.code}</p>
                          </td>
                          <td className="py-2.5 pr-3 align-top text-right tabular-nums text-[#8A9099]">{r.requests_with_fire}</td>
                          <td className={`py-2.5 pr-3 align-top text-right tabular-nums font-semibold ${isHigh ? 'text-[#D4817E]' : 'text-[#FAFAF8]'}`}>
                            {(r.fire_rate * 100).toFixed(1)}%
                          </td>
                          <td className="py-2.5 align-top text-[12.5px] text-[#8A9099] leading-relaxed">{r.humanised}</td>
                        </tr>
                      )
                    })}
                  </tbody>
                </table>
                <p className="text-[11px] text-[#676D76] mt-3">
