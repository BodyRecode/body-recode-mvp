...
export default async function BannedTermsAuditPage() {
  ...
  return (
    <div className="max-w-4xl">
      <Link href="/dashboard/system-health" className="inline-flex items-center gap-1 text-[12.5px] text-[#676D76] hover:text-[#C2C6CC] transition-colors mb-4">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <Card padding="md">
          <p className="text-[11px] font-medium text-[#676D76] mb-1">Active clients scanned</p>
          <p className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em]">{(clients ?? []).length}</p>
        </Card>
        <Card padding="md">
          <p className="text-[11px] font-medium text-[#676D76] mb-1">Published artefacts checked</p>
          <p className="text-[20px] font-semibold text-[#FAFAF8] tracking-[-0.025em]">{totalArtefactsChecked}</p>
        </Card>
        <Card padding="md">
          <p className="text-[11px] font-medium text-[#676D76] mb-1">Leak rate</p>
          <p className={`text-2xl font-bold ${leakRate > 30 ? 'text-[#D4817E]' : leakRate > 10 ? 'text-[#A96A12]' : 'text-[#177245]'}`}>{leakRate}%</p>
          <p className="text-[10px] text-[#8A9099] mt-1">{totalArtefactsLeaked} of {totalArtefactsChecked} artefacts</p>
        </Card>
      </div>

      <Card padding="md" className="mb-6">
        <p className="text-[11px] font-medium text-[#676D76] mb-3">Doctrine version coverage</p>
          ...
          {doctrineSummary.map(d => {
            ...
            return (
              <div key={d.key} className="flex items-center gap-3 text-[12.5px]">
                <span className="text-[#FAFAF8] font-semibold min-w-[160px]">{d.label}</span>
                <span className="text-[#8A9099] font-mono text-[10px]">v{d.current}</span>
                <span className="flex-1 text-right text-[#8A9099]">
      ...
      {reports.length === 0 ? (
        <Card padding="lg" className="text-center">
          ...
          <p className="text-sm font-semibold text-[#FAFAF8]">All clean</p>
          <p className="text-[12.5px] text-[#8A9099] mt-1">No active client&apos;s published readings contain banned client-facing terms.</p>
        </Card>
      ) : (
        <>
          <div className="flex items-center gap-2 mb-3 text-[12.5px] text-[#8A9099]">
            ...
            {reports.map(r => (
              <Card key={r.clientId} padding="md">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-sm font-bold text-[#FAFAF8]">{r.clientName}</p>
                  <Link href={`/dashboard/clients/${r.clientId}`} className="text-[12.5px] font-medium text-[#FAFAF8] hover:text-[#1057CC]">Open profile →</Link>
                </div>
                <div className="space-y-2">
...
function LeakRow({ label, terms, clientHref, publishedAt, storedVersion, isStale, currentVersion }: { label: string; terms: string[]; clientHref: string; publishedAt: string | null; storedVersion: string | null; isStale: boolean; currentVersion: string }) {
  ...
  return (
    <div className={`rounded-lg border px-3 py-2 flex items-center justify-between gap-3 flex-wrap ${hasLeaks ? 'border-[#F1DEB8] bg-[#FDF6E9]/40' : 'border-[#2A2F39] bg-[rgba(27,109,252,0.08)]/40'}`}>
      <div className="min-w-0">
        <p className={`text-xs font-bold ${hasLeaks ? 'text-[#8A5A14]' : 'text-[#0A46B2]'}`}>{label}</p>
        <p className="text-[10px] text-[#8A9099] mt-0.5">
