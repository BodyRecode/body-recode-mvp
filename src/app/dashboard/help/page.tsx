        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          <Section id="coach-copilot" title="Coach Co-Pilot - Doctrine Tutor" colour="violet">
            ...
            <StatusList items={[
              ...
              { label: 'Draft a training program', desc: 'Tap "＋ Draft a program" in the client\'s co-pilot panel. It proposes a full generation spec (phase, goal, frequency, duration, training age, competency), each with a reason, using the same doctrine engine the Suggest flow uses. You review and edit the block name, then tap Generate. It saves a DRAFT only. Nothing reaches the client until you publish it on the program page.' },
              { label: 'Refine the draft', desc: 'Tap "✎ Refine the draft", then describe one change ("swap the barbell squat for a hip thrust", "drop the bench to 3 sets", "soften the note"). It shows you exactly what will change before anything happens; you tap Apply. Only the named part changes, the rest of the draft is untouched, and it stays a draft until you publish. It won\'t make a change that breaks the phase, a gate, or a stated injury.' },
            ]} />
          ...
          <Section id="client-portal" title="8. Client Portal" colour="teal">
            ...
            <p>Your side lives at <strong>/dashboard/messages</strong> (Clients → Messages). One card per client, with anyone waiting on you sorted to the top and flagged <strong>Awaiting reply</strong>. The reply box sits at the top of each card and the conversation reads <strong>newest first</strong> beneath it, matching what the client sees - so the latest exchange is never something you have to scroll to, and you are never typing at the far end of a long thread.</p>
            ...
            <Note>This is deliberately not a live chat. There is no typing indicator, no read receipt shown to the client, and no push notification. It is an async coaching thread - set the expectation with clients that you reply within your normal working rhythm. The Ask-about-this rows are styled as quiet hairline links rather than buttons for the same reason: the artefact is the point, and the question is the escape hatch when a line of it does not land.</Note>
