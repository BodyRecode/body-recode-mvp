      ...
      <div className="flex gap-8 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          <Section id="client-onboarding" title="9. Client Onboarding" colour="teal">
            ...
            <p className="mt-3"><strong>What the client sees:</strong> a 5-question form at <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">/intake-supplement/&#123;token&#125;</code>. Medications + 4 dietary context fields. ~3 minutes.</p>
          ...
          <Section id="email-sequences" title="15. Email Sequences and Automation" colour="teal">
            ...
            <Note><strong>Outlook-safe dark shell (2026-05-13).</strong> Every email goes through <code>darkEmailShell()</code> in <code>src/lib/email-shell.ts</code> - a nested-table layout with <code>bgcolor</code> attributes (not just CSS) so Outlook for Windows, M365, and outlook.com all render the dark canvas instead of forcing white. Without the shell, Outlook strips body background and light-gray text becomes invisible on white. Reported by Ruby-Cate (outlook.com) and Samantha (Mater M365) - both said the emails arrived blank.</Note>
          ...
          <Section id="ch-quiz" title="Body Decode Check-In" colour="teal">
            ...
            <p>Pattern is determined client-side from Q2 answer + gender (a/b/c/d gender-keyed to a slug via <strong>src/lib/pattern-mapping.ts</strong>). Progress score counts markers answered as &quot;better&quot;. The result key, all answers, and a timestamp are saved via <strong>POST /api/challenge/quiz</strong>. <strong>The API branches on currentDay:</strong> pre-Day-14 it sends the Day 7 Progress email (no pattern, no Blueprint CTA - just progress score + per-marker breakdown + Week 2 guidance + Day 14 teaser). Day-14+ (late-takers) it sends the Day 14 Body Decode Report email immediately (progress recap + pattern card + actions + Blueprint CTA). Email composition lives in shared builders at <strong>src/lib/challenge-checkin-emails.ts</strong> so the late-taker email and the scheduled Day 14 email are identical. The Blueprint CTA on the Day 14 Body Decode Report links to <code className="text-blue-500 text-xs bg-[#E5E5E5] px-1 py-0.5 rounded">/blueprint?source=challenge_day14_report</code>.</p>

            <p className="text-xs font-bold text-[#6B6B6B] uppercase tracking-wider mt-4 mb-2">Return visits</p>
            <p>The <code className="text-blue-500 text-xs bg-[#E5E5E5] px-1 py-0.5 rounded">/check-in</code> page fetches <code className="text-blue-500 text-xs bg-[#E5E5E5] px-1 py-0.5 rounded">quiz_result</code>, <code className="text-blue-500 text-xs bg-[#E5E5E5] px-1 py-0.5 rounded">quiz_answers</code> and currentDay. Three states: (1) no result + currentDay {'>='} 7: form shows. (2) result exists + currentDay {'<'} 14: <strong>Day 7 in-portal view</strong> shows - progress score card + per-marker breakdown + Week 2 guidance + Day 14 teaser. Same content as the Day 7 Progress email but rendered as a substantive page, not a wait-marker. <strong>No pattern reveal, no Blueprint CTA.</strong> (3) result exists + currentDay {'>='} 14: full Body Decode Report shows - same Day 7 progress block + pattern card + actions + Blueprint CTA.</p>
