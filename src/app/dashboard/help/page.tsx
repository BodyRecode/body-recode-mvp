      ...
      <div className="flex gap-8 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          <Section id="client-onboarding" title="9. Client Onboarding" colour="teal">
            ...
            <p><strong>What submit does:</strong> updates the most recent intake row with the dietary fields, writes medications to <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">clients.medications</code>, marks the invitation complete, then auto-regenerates the CFFS so the next program / nutrition / weekly synthesis use the updated context.</p>
          ...
          <Section id="medications-analysis" title="10c. Medications Analysis + Reading" colour="teal">
            ...
            <p>Existing clients onboarded before this expansion should be pushed a supplementary intake from their profile to capture this context if not already disclosed. Submitting auto-regenerates the CFFS so the updated context lands in the next program / nutrition / weekly synthesis pass.</p>
          ...
          <Section id="weekly-checkins" title="11. Weekly Check-Ins and CFWS" colour="teal">
            ...
            <p className="text-xs font-bold text-[#6B6B6B] uppercase tracking-wider mt-4 mb-2">Reopen check-in window (added 2026-05-27)</p>
          ...
          <Section id="email-sequences" title="15. Email Sequences and Automation" colour="teal">
            <Note><strong>Single source of truth:</strong> the full inventory of every email the platform sends — 113 distinct subject lines across 32+ client-facing paths plus coach notifications, drip workflows, and one-off extras — lives at <code>~/Dropbox/01_BODY_RECODE/06_SAAS_PLATFORM_BUILD/EMAIL_INVENTORY.md</code> (with a <code>.docx</code> mirror generated by pandoc). When you ship a new email, update that doc + the dashboard automation entry + this section in the same commit (see <code>feedback_email_inventory_update</code>). Section 15 below is the narrative summary; the inventory doc is the authoritative list.</Note>
            <p>The following outbound email sequences run automatically. All emails send from <strong>kade@bodyrecode.au</strong> via Resend. All automated emails use the <strong>Body Recode light brand template</strong> - Pure White canvas, Graphite Black text, Signal Blue accents, Body Recode logo header, Kade signature with photo at the bottom.</p>
            ...
            <Note><strong>Duplicate Subscription Guard (2026-05-22).</strong> The Stripe webhook now blocks a 2nd subscription per client. If a client completes the subscription Payment Link a second time while their <code>subscription_active</code> flag is already true (and the incoming sub isn&apos;t just a webhook retry of the existing one), the webhook auto-cancels the new Stripe subscription immediately and emails you a &ldquo;Duplicate subscription auto-cancelled&rdquo; alert with the cancelled subscription ID. You still need to issue a refund for the first invoice Stripe already captured if the duplicate was unintended &mdash; the guard prevents future weekly charges from compounding, not the first one. Triggered case: Samantha was billed for three separate subscriptions in 14 days because the static Payment Link is reusable and there was no client-level idempotency. See <code>src/app/api/webhooks/stripe/route.ts</code>.</Note>
            ...
            <p className="font-semibold text-[#1A1A1A] mt-4">Auto-Response Pipeline (2026-06-08)</p>
            <p>The Generate-then-Save manual flow is the fallback. By default, the system now runs the full pipeline automatically when a client submits a check-in:</p>
            <ol className="space-y-1 list-decimal list-inside text-[#3A3A3A] text-sm">
              ...
              <li>Auto-response worker waits 30 seconds (settle delay so rapid back-to-back writes complete), then gates on: <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">clients.auto_checkin_response_enabled</code> (default true; per-client opt-out toggle on the profile), <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">coach_skipped_at</code> not set, no existing feedback row (coach didn&apos;t beat the worker). <strong>Every gate exit (pass or fail) now stamps <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">weekly_checkins.auto_response_attempted_at</code> + <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">auto_response_failure_reason</code> (2026-06-14).</strong> Pre-2026-06-14 the gate returned silently on fail — Ruby-Cate&apos;s Week 6 (08:10 UTC Saturday) submitted successfully but no auto-pipeline state was ever recorded, making &quot;ran and gated&quot; indistinguishable from &quot;never ran&quot;. Now every state is visible in the DB.</li>
              <li>Worker calls the shared <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">generateFeedbackDraft</code> helper (same logic the manual Generate button uses). On a clean draft, it inserts the feedback row with <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">auto_generated_at</code> and <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">auto_send_scheduled_at = now + 4 hours</code>. <strong>Immediately after insert, the worker emails Kade a <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">[Draft]</code> preview of the three fields with a deep-link to the dashboard (2026-06-14).</strong> Inbox is the channel he watches; the dashboard surface is a backup. Preview-send failures are logged but never abort the client send. The client email is NOT sent yet — that&apos;s still 4h out.</li>
              <li>You get a 4-hour intervention window. The response form for this check-in shows a blue banner: <em>&quot;AI draft, auto-sending in 3h 24m&quot;</em> with three buttons: <strong>Send now</strong> (short-circuits the wait), <strong>Skip instead</strong> (marks the check-in skipped, no send), or just edit the textareas and click Save and re-send. You can also toggle auto-response OFF for this client during the window via the profile.</li>
              <li>Worker wakes at the scheduled time and re-checks: feedback row still exists, no <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">email_sent_at</code> (coach didn&apos;t send manually), no coach_skipped_at, auto-response still enabled. If all pass: sends branded email + BCCs <strong>kade@bodyrecode.au</strong> + logs to client_communications with <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">auto_sent: true</code>.</li>
              <li>If the AI generation failed (3 retries hit jargon leaks or another error), no feedback row is created. <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">weekly_checkins.auto_response_failure_reason</code> is stamped with the error. The check-in still surfaces in Today&apos;s Focus as &quot;needs response&quot; just like the pre-automation flow, and the response form shows an amber banner naming the failure reason so you write manually.</li>
            </ol>
            <p><strong>Per-client opt-out.</strong> On the client profile, inside the Weekly Synthesis section, there&apos;s an <em>Auto check-in response</em> toggle. Default ON for every client. Flip OFF for any client where you want a fully manual touch (high-stakes mid-program client, etc.). When off, the worker exits at the eligibility gate without generating anything.</p>
            <p><strong>Why 4 hours.</strong> Long enough that you can wake up, look at the response, and decide; short enough that the client gets the response same-day. The countdown updates in 30-second ticks on the dashboard.</p>

            <p><strong>Rescue cron (2026-06-14).</strong> <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">/api/cron/weekly-checkin-auto-rescue</code> runs every 4h (<code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">0 */4 * * *</code>). Re-fires the <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">weekly-checkin/submitted</code> Inngest event for any check-in submitted &gt;6h ago where the auto-response pipeline never touched it (<code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">auto_response_attempted_at IS NULL</code> AND no feedback row AND client opted in). Backstop for transient Inngest delivery failures and silent function drops. Stamps a <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">rescue:</code>-prefixed failure reason on rows it inspects-and-skips so we don&apos;t re-scan them every hour.</p>
            <p><strong>EMAIL_INVENTORY note:</strong> the email itself (template + body + signature) is unchanged. Only the trigger description changed — was &quot;coach clicks Save and email&quot;, now &quot;coach clicks Save and email OR scheduled Inngest send 4h after auto-generation&quot;. Same Resend send path, same COACH_BCC, same client_communications kind <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">weekly_checkin_feedback</code>; auto sends are flagged in meta with <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">auto_sent: true</code> for audit.</p>
          ...
          <Section id="training-program" title="20. Training Program - PTS (Performance Training System)" colour="teal">
            ...
            <ul className="space-y-1 list-disc list-inside text-[#3A3A3A] text-sm">
              <li><strong>Phase B partial (DONE 2026-05-10)</strong>: block-end email notifications. Daily cron at 7:30am Brisbane scans all active programs and emails <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">kade@bodyrecode.au</code> when (a) any client&apos;s block has 7 days remaining, or (b) any client&apos;s block hits day 0. Branded dark template. Idempotent — one email per (client × program × event), so re-runs of the cron never spam. New table <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">block_end_notifications_sent</code> enforces it.</li>
          ...
          <Section id="be-social-profiles" title="35. Social Profiles" colour="amber">
            ...
            <p>The <strong>Business → Personal Brand → Content Kit</strong> tab documents the visual identity and the 4-week launch plan. Scheduling is <strong>not</strong> separate - the posts live in the one Content Calendar on <strong>Marketing Strategy → Content Calendar</strong>, filtered by <strong>brand = Personal Brand</strong>, alongside Body Recode, AI Co-Founder and Studio of Ten. The 16 launch posts (4/week Mon/Wed/Fri/Sun, drawn from the Script Library) are seeded via <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">sql/2026-06-04_seed_personal_brand_calendar_posts.sql</code>.</p>
            <p className="mt-2"><strong>Brand colour = Clay.</strong> One signature colour across the whole feed - paper tint #F3E9E1, ink #2A1E16, accent/solid #B5552F - deliberately distinct from Body Recode (Signal Blue), AI Co-Founder (indigo) and Studio of Ten (dark). Pillars are told apart by the eyebrow label, not colour.</p>
