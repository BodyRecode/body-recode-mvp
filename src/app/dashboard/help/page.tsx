...
export default function HelpPage() {
      ...
      <div className="flex gap-8 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          <Section id="brand-voice" title="Brand Voice - Layered Buyer Language" colour="violet">
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm mt-1">
              <li><code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">~/Dropbox/01_BODY_RECODE/06_SAAS_PLATFORM_BUILD/2026-04-30_Buyer_Language_Rewrite.md</code> - full diagnostic, every before/after, IG bio variants</li>
              <li><code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">~/Dropbox/01_BODY_RECODE/06_SAAS_PLATFORM_BUILD/2026-05-01_Pre_Call_Brief_Template.md</code> - locked 13-section template for pre-call briefs</li>
              <li><code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">~/Dropbox/01_BODY_RECODE/06_SAAS_PLATFORM_BUILD/2026-05-01_May_Post_Revisions.json</code> - 22 revised IG post captions, all in the new voice</li>
            </ul>
          ...
          <Section id="support" title="Support Tickets - Report an Issue" colour="violet">
            ...
            <p className="mt-3">When Kade updates a ticket, you get an email with the status and any note verbatim - and the ticket in the drawer updates the next time you open it. Kade sees every ticket in his own admin inbox at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/dashboard/support</code>.</p>
          ...
          <Section id="lead-pipeline" title="1. Lead Pipeline" colour="teal">
            ...
            <p className="mt-2">Leads created this way are tagged with <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">source_detail: scorecard</code>. This is now the primary lead entry path.</p>
            ...
            <p className="mt-2">The zone is stored on the lead (<code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">scorecard_profile</code> + confidence) and shown on the <strong>coach notification email</strong> as a Fat Map Zone card alongside sex/age, so you arrive at the call already knowing the likely pattern instead of reconciling it from scratch. The same typing feeds the pre-call brief. Legacy leads (pre-2026-06-24, no sex/storage captured) fall back to the older score-pattern guess.</p>
            ...
            <ul className="space-y-1.5 list-disc list-inside text-[#43474F] text-sm mt-1">
              <li><strong>Depleted (5-8):</strong> Free 30-minute call only. A PDF will not unstick a body in protection mode. CTA links to <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">bodyrecode.au/book?from=scorecard_depleted</code>.</li>
            ...
            <p className="mt-2">The Body Decode Report itself: a personalised web-based analysis of their body state, section scores, and what to stop and start doing. Purchased via Stripe at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">$37 AUD</code> and delivered automatically by email as a unique link at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">app.bodyrecode.au/report/[token]</code>.</p>
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm mt-1">
            ...
            <p className="mt-2">Report purchases are recorded as <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">scorecard_reports</code> rows in the database. No manual handling required - Stripe webhook creates the report and sends the email automatically.</p>
          ...
          <Section id="client-portal" title="8. Client Portal" colour="teal">
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              ...
              <li>Supplement Stack (added 2026-07-21) - coach-assigned substances at <strong>/portal/[token]/supplements</strong>. Tile only appears when at least one substance is assigned. Substance library lives in code (see <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">src/lib/supplement-substances-seed.ts</code>) with three tiers per substance (Essential / Enhanced / Elite). Coach assigns the substance at <strong>/dashboard/clients/[id]/supplements</strong>; client sees all three tiers and picks the one that fits their budget. 28 substances in the library as of 2026-08-17. Add a new substance by running the deep-research skill, saving the report to <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">~/Dropbox/01_BODY_RECODE/00_PLAYBOOK/supplement_research/</code>, and adding an entry to the seed lib. Only OTC / Listed complementary medicines - S4/S8 stays with Arete Protocol.</li>
            ...
            <p>Every portal landing now has a <strong>Share feedback</strong> card under Resources. Clients submit free-text feedback under one of five categories: <code>portal_experience</code>, <code>coaching_experience</code>, <code>feature_request</code>, <code>bug</code>, <code>other</code>. Each submission writes to the <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">client_feedback</code> table and emails Kade a notification with the body inlined (so a reply can go straight from the email if needed).</p>
            <p>Every broadcast email sent to clients now ends with a <strong>Tell us what would help</strong> section linking to the same form. Pattern is canonical via <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">src/lib/broadcast-feedback-footer.ts</code> - drop it into any future broadcast HTML template so the feedback channel is always present.</p>
            ...
            <p>Migration: <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">sql/2026-05-13_client_feedback.sql</code>.</p>
            ...
            <p className="mt-2">To book an individual session from the dashboard, go to the Face-to-Face Sessions page and use the <strong>Book a session</strong> form in the Booked Sessions panel. Pick the date, time, and duration - this creates a <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">client_sessions</code> record. The session then shows as <strong>Confirmed</strong> in the client portal once the client confirms attendance via the reminder email.</p>
          ...
          <Section id="client-onboarding" title="9. Client Onboarding" colour="teal">
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              <li><strong>Not sent yet</strong> &mdash; click <strong>Add follow-up to portal</strong> to create a pending supplementary <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">intake_invitations</code> row with <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">kind=&apos;supplementary&apos;</code>. The portal landing page detects this and surfaces a teal &quot;A quick follow-up from Kade&quot; card next time the client signs in.</li>
              <li><strong>Pending</strong> &mdash; once the card exists, two extra actions appear: <strong>Email link</strong> (sends the client a branded reminder via Resend with the Outlook-safe shell + plain-text URL fallback, logged as <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">supplementary_intake_invite</code> in client_communications) and <strong>Copy follow-up link</strong> (puts the <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/intake-supplement/&#123;token&#125;</code> URL on the clipboard for DM/SMS). The default is still portal-only; the email is the explicit escape hatch for clients who don&apos;t log in between weeks.</li>
              <li><strong>Complete</strong> &mdash; row collapses to the completion timestamp. No actions.</li>
            </ul>
            <p className="mt-3"><strong>What the client sees:</strong> a 9-question form at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/intake-supplement/&#123;token&#125;</code>. Medications + 8 dietary/consumption fields (restrictions, preferences, typical day, meals per day, fluids, caffeine, alcohol, eating environment). ~4 minutes.</p>
            <p><strong>What submit does:</strong> updates the most recent intake row with the dietary fields, writes medications to <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">clients.medications</code>, marks the invitation complete, sends Kade a coach notification email, and stops there. <strong>The CFFS is NOT auto-regenerated</strong> (changed 2026-06-21, mirrors the Medications + Dietary editors). The Foundational Synthesis panel on the client profile renders an amber &quot;Supplementary newer than CFFS&quot; banner with a Regenerate button when <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">intake_invitations.completed_at &gt; cffs.generated_at</code>; the section also auto-opens and the section header shows a &quot;Supplementary newer &mdash; regenerate&quot; attention pill. Coach reviews the new dietary + medication context, then clicks Regenerate when ready.</p>
          ...
          <Section id="cffs" title="10. CFFS - Coach-Facing Foundational Synthesis" colour="teal">
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              <li><strong>Photo badge</strong> in the panel footer next to the Generated date. <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">Photos ✓ 3/3</code> in teal means the photos were attached at generation time. <code className="bg-[#EFF1F4] px-1 rounded text-[#666D7A] text-[12.5px]">Photos ✗ Not provided</code> in muted grey means no photos went into the prompt (older CFFS rows have no badge at all). Hovering shows the count.</li>
            ...
            <p>Migration: <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">sql/2026-05-13_cffs_visual_signal_fields.sql</code>.</p>
          ...
          <Section id="foundational-reading" title="10b. Foundational Reading - Client Facing" colour="teal">
            ...
            <ol className="space-y-1 list-decimal list-inside text-[#43474F] text-sm">
              ...
              <li>Auto-publishes it to the client portal at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/portal/[token]/foundational-reading</code>.</li>
            </ol>
            ...
            <Training title="Prompt safety rails">
              <p>The reading prompt inherits the CFFS doctrine: pattern-based interpretation, conservative under uncertainty, no prescriptions, no diagnostic labels, no causal claims. It also explicitly forbids motivational language, em dashes, and exclamation marks. The prompt is in <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">src/lib/client-reading-prompt.ts</code>.</p>
          ...
          <Section id="medications-analysis" title="10c. Medications Analysis + Reading" colour="teal">
            ...
            <p>Why: structured PED tracking belongs to <a className="text-blue-500 underline" href="/dashboard/business">Arete Protocol</a>, not Body Recode. BR captures via the existing free-text <code className="bg-[#EFF1F4] px-1 rounded text-[#1B6DFC] text-[12.5px]">clients.medications</code> field for interpretive purposes only, so the CFFS reads the body accurately. The CFFS prompt now has explicit reading rules for this context: do not read body comp gains or recovery capacity as a clean training-stimulus signal when performance compounds are present; never moralise; never recommend cessation; do not name specific compound classes back to the client in any narrative field that propagates to client-facing readings.</p>
          ...
          <Section id="weekly-checkins" title="11. Weekly Check-Ins and CFWS" colour="teal">
            ...
            <p><strong>The review queue.</strong> Every check-in that comes in lands at <strong>Check Ins</strong> in the sidebar (<code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/dashboard/checkins</code>). It is the one place to work through them rather than opening clients one at a time.</p>
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
          ...
          <Section id="signal-monitoring" title="11b. Signal Monitoring and Reassessment Triggers" colour="teal">
            <p>The system watches the four CFWS readiness signals (Capacity, Schedule, Regulation, Behaviour) week over week and surfaces flags when conditions defined in the <strong>Signal Monitoring and Reassessment Triggers v1.0</strong> doctrine are met. The doctrine itself is at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">~/Dropbox/01_BODY_RECODE/.../Performance_Coaching_Framework_Master_Folder_v1/Signal_Monitoring_and_Reassessment_Triggers_v1.0.md</code>.</p>
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              <li>The CFWS engine sets <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">reassessment_language_triggered: true</code></li>
            ...
            <p>Constant: <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">SIGNAL_BASED_TRIGGER_FLOOR_DAYS = 21</code> in <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">src/lib/readiness-monitor.ts</code>. Rationale captured in §3a of the doctrine doc.</p>
            ...
            <p className="mt-2">Therefore: <strong>an intake-derived CFFS will never have <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">reassessment_flagged: true</code></strong>. The CFFS prompt does not ask the LLM to set this field at generation; the server forces it to <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">false</code> on insert. The flag becomes settable only later, by the Signal Monitoring engine, when one of the trigger conditions actually fires against accumulated CFWS data.</p>
            <Note>Historical bug fixed 2026-05-09. Prior to the fix, the CFFS prompt asked the LLM to write <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">reassessment_flagged: true | false</code> at intake with no guidance, and the model defaulted to true on most "Remediation / Partially Resolved" classifications. Five clients carried stale flags as a result; all were cleared and the prompt + insert path are now defensive against the same drift.</Note>
          ...
          <Section id="recovery-regulation" title="11c. Recovery and Regulation" colour="teal">
            ...
            <p className="mt-2">Doctrine source: <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">~/Dropbox/01_BODY_RECODE/.../12_Recovery_and_Regulation_System/</code> (Layer 2) and <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">.../13D_Recovery_and_Regulation_Playbooks/</code> (Layer 3 execution). The full canonical reference renders live at <a className="text-blue-500 underline" href="/dashboard/recovery-regulation">Recovery</a>.</p>
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              <li><strong>Disabled</strong> (<code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">RRS_ROUTER_ENABLED=false</code>): router does not run.</li>
              <li><strong>Observe-only</strong> (Phase 0 default — <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">RRS_OBSERVE_ONLY=true</code>): router evaluates and writes shadow audit rows to <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">recovery_adjustments</code>. No states activated. No programs constrained.</li>
              <li><strong>Live soft gate</strong> (<code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">RRS_OBSERVE_ONLY=false</code>): states activate. Constraints flagged on program/nutrition gen as override-able policy.</li>
              <li><strong>Live hard gate</strong> (<code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">RRS_HARD_ENFORCEMENT=true</code>): states activate. Constraints hard-block program changes that violate doctrine.</li>
            </ul>
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              ...
              <li><strong>Audit every transition</strong> (12E_04) — every router evaluation + state transition writes a <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">recovery_adjustments</code> row</li>
            </ul>
            ...
            <p>Per-client active state visualisation lands in Phase 4. Until then, audit rows in the <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">recovery_adjustments</code> table are the only output (visible via Supabase).</p>
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              ...
              <li><strong>Phase 2 (DONE)</strong>: RSIB ingest from existing weekly check-in answers (the three questions per 13D_13 are already in Form A and Form B verbatim — no client-facing changes). Router evaluates after each CFWS lands. Per-client shadow log lives on the client profile, just below the readiness drift section. Backfill API at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">POST /api/recovery/backfill</code> derives RSIB from historical check-ins.</li>
              <li><strong>Phase 3 (CURRENT — soft-gate enforcement)</strong>: program generation and nutrition generation now read the active recovery state and apply the playbook&apos;s constraint envelope. Two layers: (a) the LLM is given a prompt section that tells it the active constraints, (b) post-LLM <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">clampProgramToRecoveryManifest</code> walks every exercise and enforces them as defence in depth. On the program-suggest page you&apos;ll see an amber <strong>Active recovery state</strong> banner with the constraints summary and a radio: <strong>Apply constraints</strong> (default, doctrinally correct) or <strong>Override with documented reason</strong> (writes a <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">constraint_overridden</code> audit row). Activation requires <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">RRS_OBSERVE_ONLY=false</code> in env — set in <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">.env.local</code> by default; production needs the flag flipped manually.</li>
              <li><strong>Phase 4</strong>: hard-gate enforcement (<code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">RRS_HARD_ENFORCEMENT=true</code>) — coach loses the override path entirely. Tier escalation alerts. Recovery state history view per client.</li>
            </ul>
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              ...
              <li><strong>Session removal / cap</strong>: removes the LAST N sessions of the week (preserves earliest = priority work). Caps total weekly sessions if <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">sessionsPerWeekCap</code> set.</li>
              <li><strong>Conditioning blocks stripped</strong>: any block whose label matches conditioning / capacity / finisher / cardio is removed when <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">conditioningBlocked</code> is true (most playbooks).</li>
              <li><strong>Testing exercises stripped</strong>: 1RM/3RM/5RM, max attempts, PR attempts removed when <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">testingBlocked</code> is true.</li>
              <li><strong>Progression-locked banner</strong>: stamped onto <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">weekly_pattern_summary</code> for the duration of the state.</li>
          ...
          <Section id="rpe-creep" title="11d. RPE Creep Monitor" colour="teal">
            ...
            <ol className="space-y-1.5 list-decimal list-inside text-[#43474F] text-sm">
              ...
              <li><strong>Hold the block</strong> via Weekly Review → set direction to <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">hold</code>. Keeps the prescription, signals no further progression this week.</li>
            ...
            <p className="text-[12.5px] text-[#98A0AD] mt-4">Doctrine: Signal Monitoring §10 (2026-05-11 addendum). Thresholds are named constants in <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">src/lib/rpe-creep-monitor.ts</code>; revise both in lockstep.</p>
          </Section>
          ...
          <Section id="email-sequences" title="15. Email Sequences and Automation" colour="teal">
            ...
            <p><strong>Rewritten 24 Aug 2026:</strong> four of the five sold the retired $37 report and one of them sold nothing else. All four now point at <strong>The Body Decode</strong> at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">bodyrecode.au/decode</code>. These emails live in the database, not in code, so they were edited in place rather than re-seeded. <strong>Never click Re-sync on this workflow</strong> — the live copy is newer than the code seed, so a re-sync replaces good copy with old copy.</p>
            <p>Fires when someone completes the Readiness Scorecard. <strong>5 emails over 13 days</strong>. Each email is personalised to the lead&apos;s score and body state using <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">{`{{scorecard_score}}`}</code>, <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">{`{{scorecard_state}}`}</code>, and <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">{`{{first_name}}`}</code>. Voice: leads with fat loss as the symptom (buyer language), introduces body state vocabulary as the diagnostic. Emails 1-2 push the $37 Body Decode Report; emails 3-4 push the free strategy call; email 5 names both options based on state.</p>
            ...
            <p>Two capture points inside The Body Decode, and they measure different things. <strong>Accuracy</strong> is captured directly under her read the moment she first sees it, on a 1-5 scale: did the diagnosis actually land? <strong>Recommendation</strong> is captured at the end of day 5 on a 0-10 scale, after the Blueprint card rather than before it. Both land in <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/dashboard/feedback</code> under moments <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">decode_read</code> and <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">decode_day5</code>. Read the low accuracy scores first: a read that misses tells you more than one that lands, and a high completion rate on a read that does not sound like her is a worse result than a low one. Consent emails are not automatic, so nothing is ever published without you sending the request yourself. Pull the whole cohort with <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">npx tsx --env-file=.env.local scripts/decode-cohort.ts --since 2026-09-01</code>.</p>

            <p className="font-semibold text-[#141821] mt-4">Body Decode Report Delivery + Follow-up Sequence (RETIRED 24 Aug 2026)</p>
            <p><strong>The $37 Body Decode Report is no longer sold, so this can no longer fire on a new purchase.</strong> The Body Decode (Funnel B Stage 1) gives every signup the same five-part read free on day 5, so the paid version was selling something she was about to be handed. Selling is closed at all three entry points. <strong>Delivery is deliberately still live</strong> — anyone who already bought keeps their report at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">app.bodyrecode.au/report/[token]</code>, and a session already in flight still completes. One tail remains: the live scorecard follow-up emails still link <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/get-report</code> and must be edited in the workflow UI, never re-seeded, because the live copy is newer than the code seed. That URL redirects to the scorecard meanwhile.</p>
            <div className="space-y-1">
            ...
            <Note><strong>Run the dry run before you send.</strong> <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">GET /api/admin/dormant-reactivation</code> shows exactly who would receive what, and who is excluded and why, without sending anything. Sending needs a POST with <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">?confirm=1</code>. Of the original 84: 8 were internal or test records including both of your own addresses, 13 had no body state so there was no read to send them, and 3 were inactive. 60 were sendable.</Note>
          ...
          <Section id="training-program" title="20. Training Program - PTS (Performance Training System)" colour="teal">
            ...
            <p>Top nav <strong>Programs</strong> (or <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">⌘K → Programs</code>) opens the floor view: every active client&apos;s current training block in a single mobile-first card grid. One tap on a card opens that client&apos;s training program page.</p>
            <ul className="space-y-1.5 list-disc list-inside text-[#43474F] text-sm mt-2">
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              <li>Client opens <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/portal/&#123;token&#125;/program</code> and taps the new <strong>Log a session →</strong> button.</li>
            ...
            <Note>Logged data is captured in three new tables: <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">session_completions</code> (one per session × week), <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">session_exercise_completions</code> (one per exercise, with prescribed-vs-actual + substitutions), <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">exercise_set_logs</code> (one per set). The prescription is snapshotted at session start so a mid-block program regeneration cannot retroactively change what was logged.</Note>

            <p className="text-[12.5px] font-medium text-[#666D7A] mt-4 mb-2">What&apos;s coming in Phase B + C</p>
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              <li><strong>Phase B partial (DONE 2026-05-10)</strong>: block-end email notifications. Daily cron at 7:30am Brisbane scans all active programs and emails <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">{coach().email}</code> when (a) any client&apos;s block has 7 days remaining, or (b) any client&apos;s block hits day 0. Branded dark template. Idempotent — one email per (client × program × event), so re-runs of the cron never spam. New table <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">block_end_notifications_sent</code> enforces it.</li>
              ...
              <li><strong>Phase C</strong>: auto-fill the Recovery Router&apos;s <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">session_repeatability</code> signal from logged data (compare last week&apos;s avg loads to this week&apos;s); surface &quot;missed &gt;2 sessions&quot; as a router signal.</li>
            </ul>
          </Section>
          ...
          <Section id="program-reading" title="20b. Program Reading - Client Facing" colour="teal">
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              ...
              <li>Auto-publishes it to the top of <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/portal/[token]/program</code>.</li>
              ...
              <li>Also makes the cream-on-black premium-deliverable version available at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/portal/[token]/program/reading</code> via the &quot;View as document&quot; link on the portal card.</li>
            </ul>
          ...
          <Section id="trajectory-reading" title="20c. Block-End Trajectory Reading - Client Facing" colour="teal">
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              ...
              <li><strong>Publish &amp; notify</strong> surfaces the reading at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/portal/[token]/program</code> and, the first time this block is published, emails the client (&ldquo;your block-end reading is ready&rdquo;). Republishing never re-emails.</li>
              <li>The standalone document lives at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/portal/[token]/program/trajectory-reading</code> via the &quot;View as document&quot; link.</li>
            </ul>
            ...
            <ul className="space-y-1.5 list-disc list-inside text-[#43474F] text-sm mt-2">
              <li><strong>Send Progress Check.</strong> At block-end a <strong>Send Progress Check</strong> button appears above the reading panel. One click emails the client a short (~5 min) re-assessment (it is <em>not</em> the 221-question intake) and logs <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">progress_check_invite</code>.</li>
            ...
            <div className="grid gap-1.5">
              {[
                ...
                <div key={item.label} className="flex items-start gap-3 bg-[#EFF1F4]/50 rounded-lg px-3 py-2">
                  <span className="text-amber-700 font-semibold text-[12.5px] shrink-0 mt-0.5 w-24">{item.label}</span>
          ...
          <Section id="be-bookings" title="24. Bookings" colour="amber">
            ...
            <p>Face-to-face clients can reschedule a session directly from their portal. When they book a slot, a <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">client_sessions</code> record is created with <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">status = scheduled</code>, and a branded confirmation email goes to both the client and you. Booked slots are blocked and won&apos;t appear for other clients.</p>

            <p className="text-[12.5px] font-medium text-[#666D7A] mt-4 mb-2">Session Reminder Emails</p>
            <p>Every day at 8:00 am Brisbane time, a cron job scans for sessions scheduled in the next 20–28 hours that haven&apos;t had a reminder sent yet. The client receives a branded email with a <strong>Confirm attendance →</strong> button. Clicking that link marks the session as confirmed (<code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">confirmed_at</code> is set) and notifies you by email. The session then shows a <strong>Confirmed</strong> badge in the portal and on the dashboard client profile. If the session is already confirmed, the link shows a friendly &quot;already confirmed&quot; message instead.</p>
            ...
            <p className="mt-1 font-mono text-[12.5px] text-blue-700 bg-[#FFFFFF] rounded-lg px-4 py-2">webcal://bodyrecode.au/api/calendar/feed?key=CALENDAR_FEED_KEY</p>
            ...
            <p className="mt-1 font-mono text-[12.5px] text-blue-700 bg-[#FFFFFF] rounded-lg px-4 py-2">webcal://bodyrecode.au/api/calendar/feed/stories?key=CALENDAR_FEED_KEY</p>
            <p className="mt-2">Use the <strong>same key</strong> as the bookings feed above (just change the path to <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">/feed/stories</code>). Subscribe the same way (New Calendar Subscription → auto-refresh Every 15 Minutes). Once subscribed it is fully automatic - new stories, copy changes, and the weekly Blueprint beat all flow in with no manual import. <strong>One-time:</strong> if you previously imported a static story-reminders file, delete that calendar first so it does not double up with the live feed.</p>
          ...
          <Section id="be-automations" title="25. Automations" colour="amber">
            ...
            <p>The 9-step sequence lives as a custom workflow in the DB. It uses trigger <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">form_submitted</code> with <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">{`{ form: 'scorecard' }`}</code>. Emails are personalised using:</p>
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm mt-2">
              <li><code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">{`{{scorecard_score}}`}</code> - e.g. 7</li>
              <li><code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">{`{{scorecard_state}}`}</code> - e.g. Transitioning State</li>
              <li><code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">{`{{first_name}}`}</code> - lead&apos;s first name</li>
            </ul>
          ...
          <Section id="be-campaigns" title="26. Campaigns" colour="amber">
            ...
            <ul className="space-y-1.5 list-disc list-inside text-[#43474F] text-sm">
              ...
              <li>Write the body - use <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">{`{{first_name}}`}</code> to personalise</li>
          ...
          <Section id="payments-tracker" title="29b. Client Payment Tracker" colour="amber">
            ...
            <p>Stripe is the source of truth. The cache (<code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">client_subscriptions</code> table) is maintained three ways:</p>
            <StatusList items={[
            ...
            <p className="text-[12.5px] text-[#98A0AD] mt-4">Doctrine / spec: <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">~/Dropbox/01_BODY_RECODE/06_SAAS_PLATFORM_BUILD/2026-05-13_PAYMENTS_FOUNDATION.md</code>. Feature registry entry 26.</p>
          </Section>
          ...
          <Section id="be-sources" title="31. Lead Sources" colour="amber">
            ...
            <Note>The leads.source column has a CHECK constraint that only accepts a fixed set of values (quiz, other, gym_floor, instagram, facebook, direct). Any unmapped <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">?source=</code> param falls through to <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">source=other</code> with the raw value preserved in source_detail. This was tightened 2026-04-30 after a gym_complementary submission failed. To add a new normalised source, edit <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">SOURCE_MAP</code> in <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">src/app/api/scorecard/submit/route.ts</code>.</Note>
          ...
          <Section id="partner-room" title="Partner Room - Private Investor / Partner Overview" colour="amber">
            ...
            <p className="mt-3 text-sm text-[#666D7A]">Guest link format: <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">app.bodyrecode.au/room/&#123;token&#125;</code>. Data lives in the <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">partner_rooms</code> table.</p>
          </Section>

          <Section id="be-ads" title="32. Ads" colour="amber">
            <p>Meta ads feed the top of the funnel. Clicks go to <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">performance.bodyrecode.au/scorecard</code> where the Meta Pixel (ID <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">972772552072010</code>) fires four events as the prospect moves through the funnel:</p>
            <StatusList items={[
            ...
            <ul className="space-y-1.5 list-disc list-inside text-[#43474F] text-sm">
              ...
              <li>Drop the file into <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">RAW_DATA/</code> before 9am Friday</li>
            ...
            <ul className="space-y-1.5 list-disc list-inside text-[#43474F] text-sm">
              ...
              <li><strong>Destination:</strong> <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">performance.bodyrecode.au/scorecard?source=facebook_ad_[adset]</code></li>
            </ul>

            <p className="text-[12.5px] font-medium text-[#666D7A] mt-4 mb-2">Pixel</p>
            <ul className="space-y-1.5 list-disc list-inside text-[#43474F] text-sm">
              <li>Pixel ID: <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">972772552072010</code> - named Body Recode in Meta</li>
          ...
          <Section id="be-content-engine" title="33. Content Engine" colour="amber">
            ...
            <p className="mt-4">Reel generation requires four environment variables: <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">ELEVENLABS_API_KEY</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">ELEVENLABS_VOICE_ID</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">HEYGEN_API_KEY</code>, and <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">HEYGEN_AVATAR_ID</code>. If any are missing the button will return an error.</p>
            ...
            <p>The graphic API generates 1080×1080 PNG cards. All styles use the brand dark background (<code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">#FFFFFF</code>). A full visual reference with example images is saved at <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">Dropbox/01_BODY_RECODE/07_MARKETING/04_CONTENT_LIBRARY/card-designs/card-designs-reference.html</code>.</p>
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              <li><strong>body-state</strong> - Left coloured border card. Use <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">accent=red</code> (Depleted), <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">accent=amber</code> (Transitioning), <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">accent=teal</code> (Ready).</li>
            </ul>
            ...
            <ul className="space-y-1 list-disc list-inside text-[#43474F] text-sm">
              ...
              <li><strong>carousel-slide</strong> - Interior numbered slides. Use <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">n=2</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">n=3</code> etc. for each slide number.</li>
              <li><strong>carousel-cta</strong> - Final slide. Teal pill CTA driving to scorecard or check-in. Use <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">label=</code> to override CTA button text.</li>
            </ul>
          ...
          <Section id="be-strategy" title="34. Strategy Hub" colour="amber">
            ...
            <p className="text-[12.5px] mt-2 text-[#43474F]"><strong>Neither token can read insights.</strong> Both were generated with four permissions and <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">instagram_manage_insights</code> is not among them, so reach, saves and shares are unreadable on <strong>every post either account has ever published</strong> — 44 on @body_recode_ and counting. Publishing is entirely unaffected; this is a read permission only.</p>
            <p className="text-[12.5px] mt-2 text-[#43474F]">Fix: regenerate both tokens with that permission added and update the env vars. Then <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">npx tsx --env-file=.env.local scripts/ig-insights.ts</code> reports reach, saves, shares and likes per brand and per format, with top and bottom performers. <strong>Until then every content decision on both accounts is taste, not evidence.</strong></p>
            <p className="text-[12.5px] mt-2 text-[#43474F]">Token + IG Business Account ID configured one-time via env vars (Vercel body-recode project only). Token is a long-lived User Access Token with the 4 perms: <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">instagram_basic</code>, <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">instagram_content_publish</code>, <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">pages_show_list</code>, <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">pages_read_engagement</code>. Regenerate via Graph API Explorer every ~60 days. Full setup walkthrough in <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">~/Dropbox/01_BODY_RECODE/06_SAAS_PLATFORM_BUILD/2026-06-30_Instagram_Native_Publishing.md</code>.</p>
          ...
          <Section id="be-social-profiles" title="35. Social Profiles" colour="amber">
            ...
            <div className="space-y-2">
              {[
                ...
                <div key={row.source} className="flex items-center gap-3 bg-[#EFF1F4] border border-[#E8EAEE] rounded-lg px-4 py-2.5">
                  ...
                  <span className="text-blue-500 font-mono text-[12.5px]">{row.url}</span>
                </div>
              ))}
            </div>
            ...
            <p className="mt-2"><strong>Card system</strong> (all via the graphic endpoint, clay is the default theme): <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">style=personal</code> = editorial text card (Source Serif 4 headline, clay eyebrow + rule, @kade_dunstone_ wordmark, no logo); add <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">num=</code> + <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">cue=swipe</code> for carousel slides and <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">theme=clay-solid</code> for the solid close slide. <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">style=personal-photo</code> = photo card with <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">layout=top|overlay|split|inset</code> and <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">photo=1-8</code>. Square + story. ~1 in 4 posts carries a photo; framework posts are carousels; the rest are clay cards. A carousel row stores comma-separated slide URLs in its <em>graphic</em> field.</p>
            ...
            <p className="mt-2">Full strategy at <strong>Business → Marketing Strategy → LinkedIn tab</strong>. 12-week pipeline document at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">~/Dropbox/01_BODY_RECODE/06_SAAS_PLATFORM_BUILD/LINKEDIN-BODY-RECODE-12-WEEK-PIPELINE.md</code>.</p>
            <p className="mt-2">Leads land tagged <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">source=linkedin</code> in the CRM, filterable separately from Instagram on the leads page. Realistic ramp to first booking is 6-10 weeks - slow-burn channel, not primary conversion engine.</p>
          ...
          <Section id="ch-landing" title="Landing Page" colour="teal">
            ...
            <p>New expression of interest via the Fit Scorecard at <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">bodyrecode.au/collective/apply</code> triggers a synchronous send-pair from <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">POST /api/collective/submit</code> after the row insert.</p>
            ...
            <p className="mt-2"><strong>Distinct Collective sub-brand:</strong> warm-charcoal (#2C2418) accent, editorial "The Collective" wordmark, brand-first signature (no Kade personal photo) — signals the coach-facing B2B programme identity vs consumer BR emails&apos; Signal Blue + Kade-founder-byline shape. Composed from Collective-brand primitives in <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">src/lib/collective-email-shell.ts</code> wrapping the same shared <code>darkEmailShell</code>.</p>
            ...
            <p>New signup to <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">product_waitlist</code> for <strong>challenge</strong> / <strong>blueprint</strong> / <strong>membership</strong> triggers a synchronous send-pair from <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">POST /api/product-waitlist</code> before the response returns.</p>
            ...
            <p className="mt-2"><strong>Related file:</strong> <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">src/lib/product-waitlist-welcome-email.ts</code>.</p>
            ...
            <p>Single-event broadcast that converts the accumulated <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">product_waitlist</code> rows on launch day. Same hour <strong>NEXT_PUBLIC_CHALLENGE_LIVE</strong> flips to <strong>&apos;true&apos;</strong>, run the broadcast script: <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">cd ~/body-recode-mvp &amp;&amp; set -a &amp;&amp; source .env.local &amp;&amp; set +a &amp;&amp; npx tsx scripts/launch-day-waitlist-email.ts --live</code>.</p>
          ...
          <Section id="ch-portal" title="Participant Portal" colour="teal">
            ...
            <p>Day is calculated server-side on every page load as <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">Math.floor((now - enrolledAt) / 86400000) + 1</code>, clamped between 1 and 14. Day 1 is the day of enrollment. Day 2 starts 24 hours after enrollment.</p>
          ...
          <Section id="ch-day-zero" title="Day 0 Body Decode Intake" colour="teal">
            ...
            <p>Critical UX: scorecard signups must NOT see the intake form again. The portal checks <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">leads.scorecard_profile</code> on load — if populated, the intake auto-skips and the enroller goes straight to PAR-Q. So the Day 0 intake is exclusively for non-scorecard enrollers.</p>
            ...
            <p className="mt-2">Continue Challenge always stays the default action — honours the free 14-day Challenge promise the enroller signed up for. Ascension is the OPTION, not a redirect. CTAs route to <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">/blueprint?from=challenge_day_zero&state=transitioning</code> and <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">/membership?from=challenge_day_zero&state=ready</code> for attribution.</p>

            <p className="text-[12.5px] font-medium text-[#666D7A] mt-4 mb-2">How data persists</p>
            <p>POST <strong>/api/challenge/day-zero-intake</strong> writes to the SAME columns the public scorecard writes to: <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">leads.scorecard_score</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">scorecard_body_state</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">scorecard_profile</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">scorecard_section_scores</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">biological_sex</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">age_band</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">fat_storage</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">cycle_status</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">approach_response</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">investment_readiness</code>, <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">lead_quality</code>. Uses the shared <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">typeFatMapProfile()</code> from <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">src/lib/fat-map-profile.ts</code>. Same body-state thresholds as scorecard (≤8 Depleted, ≤11 Transitioning, ≥12 Ready). Stamps <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">challenge_enrollments.body_decode_intake_completed_at</code> so the portal advances past the gate, and logs a <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">day_zero_intake_completed</code> lead event.</p>

            <p className="text-[12.5px] font-medium text-[#666D7A] mt-4 mb-2">Source attribution</p>
            <p>Whether a lead&apos;s Fat Map zone was classified via the scorecard or via Day 0 intake is derivable from <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">challenge_enrollments.body_decode_intake_completed_at</code> — non-null means the data came from inside the portal. No separate scorecard_source column needed.</p>
            ...
            <p>Three fixes landed together, all enforcing <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">Fat_Map_Definitions_LOCKED.md</code> v2.0.</p>
            <ul className="list-disc ml-5 space-y-1 mt-2">
              ...
              <li><strong>The Day 7 Check-In gained a phase-2 route.</strong> The oestrogen option previously ended &quot;Storage on hips and thighs&quot;, which is phase 1 only, so phase-2 women picked the post-meal sluggishness option and typed as Insulin-Drift. That option now covers both phases, and a third signal question (<code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">sq3</code>) asks the direction-of-travel discriminator. It is shown <em>only</em> to women in the menopausal band, via <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">isMenopausalBand()</code> in <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">src/lib/pattern-mapping.ts</code>. Only the &quot;moved toward my middle&quot; answer overrides the sq2 result.</li>
              <li><strong>Brief regeneration was silently mistyping leads.</strong> <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">/api/admin/regenerate-pre-call-briefs</code> never passed biological_sex, age_band, fat_storage or cycle_status, so every regenerated brief was typed as if the lead had answered nothing. Fixed.</li>
            </ul>
          ...
          <Section id="ch-forms" title="PAR-Q and Health Declaration" colour="teal">
            ...
            <p>Both forms call <strong>POST /api/challenge/forms</strong> with the token and form type. The API saves a timestamp to <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">parq_completed_at</code> or <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">health_dec_completed_at</code> on the enrollment. PAR-Q answers are also saved as JSON to <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">parq_responses</code>.</p>
            ...
            <p>Anyone part-way through the old two-step flow still sees only the piece they owe — <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">ParqForm</code> and <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">HealthDecForm</code> are both retained for that case. On completion the section is replaced by a cleared confirmation banner and the Training and Nutrition cards unlock without a page reload.</p>
          ...
          <Section id="ch-leaks" title="Where the Challenge actually leaks" colour="teal">
            ...
            <p><strong>3. Portal visits are now logged.</strong> They were logged nowhere at all. There is a <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">current_day</code> column but the portal recalculates the day from <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">enrolled_at</code> and never writes to it, so it said nothing about whether anyone turned up. That made the largest leak in the funnel invisible: we could not tell whether people went quiet on day 2 or day 9, and those need completely different fixes. <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">logPortalVisit()</code> writes one <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">challenge_portal_opened</code> lead event per enrollment per day. Read it with <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">npx tsx --env-file=.env.local scripts/challenge-attendance.ts</code>, which charts attendance across the 14 days and names the biggest single day-to-day drop.</p>
          ...
          <Section id="ch-sms" title="SMS Coaching Sequence" colour="teal">
            ...
            <p>Templates use <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">%FIRST%</code> and <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">%URL%</code> placeholders. The <code className="bg-[#EFF1F4] px-1 rounded text-blue-700 text-[12.5px]">renderSms()</code> helper substitutes them at send time. Source of truth: <strong>SMS_MORNING</strong> + <strong>SMS_AFTERNOON_BOOST</strong> constants in <strong>src/lib/inngest-functions.ts</strong>.</p>
          ...
          <Section id="ch-database" title="Database and Supabase" colour="teal">
            ...
            <pre className="text-[12.5px] bg-[#EFF1F4] text-blue-700 rounded-lg p-4 mt-2 overflow-x-auto whitespace-pre-wrap">{`ALTER TABLE challenge_enrollments
  ...
  ADD COLUMN IF NOT EXISTS quiz_answers jsonb;`}</pre>
          ...
          <Section id="bp-purchase" title="Purchase Flow" colour="teal">
            <p>When Stripe fires <strong>checkout.session.completed</strong> with <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">metadata.type === &apos;blueprint_purchase&apos;</code>, the webhook runs the following steps:</p>
          ...
          <Section id="mb-purchase" title="Purchase Flow" colour="teal">
            <p>When Stripe fires <strong>checkout.session.completed</strong> with <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">metadata.type === &apos;membership_purchase&apos;</code>, the webhook runs the following steps:</p>
          ...
          <Section id="mb-automation" title="Automation" colour="teal">
            ...
            <p>Triggered by the <strong>membership/enrolled</strong> Inngest event. Sleeps 7 days then advances <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">current_week</code> within the current block. On each advance it sends a check-in prompt email. If no check-in is submitted within 2 days, a reminder fires. After week 6, advances <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">current_block</code> to the next block and resets <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">current_week</code> to 1. Checks <code className="text-blue-500 text-[12.5px] bg-[#EFF1F4] px-1 py-0.5 rounded">cancelled_at</code> before each step.</p>
...
function Section({ id, title, colour, children }: { id: string; title: string; colour: 'teal' | 'amber' | 'violet'; children: React.ReactNode }) {
  ...
  return (
    <div id={id} className="bg-[#FFFFFF] border border-[#E8EAEE] rounded-xl overflow-hidden scroll-mt-8">
...
function StatusList({ items }: { items: { label: string; desc: string }[] }) {
  return (
    <div className="grid gap-1.5">
      {items.map(item => (
        <div key={item.label} className="flex items-start gap-3 bg-[#EFF1F4]/50 rounded-lg px-3 py-2">
          <span className="text-blue-500 font-semibold text-[12.5px] shrink-0 mt-0.5">{item.label}</span>
          <span className="text-[#666D7A] text-[12.5px]">{item.desc}</span>
        </div>
      ))}
    </div>
...
function Note({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-[#FFFFFF] border border-[#E8EAEE] rounded-xl px-4 py-3 text-[12px] text-[#666D7A] leading-relaxed">
...
function SeqRow({ day, label }: { day: string; label: string }) {
  return (
    <div className="flex items-start gap-3 bg-[#EFF1F4]/40 rounded-lg px-3 py-2">
      <span className="text-[12.5px] font-semibold text-blue-500 shrink-0 w-20">{day}</span>
