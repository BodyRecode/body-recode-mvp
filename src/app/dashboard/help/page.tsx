      ...
      <div className="flex gap-8 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-3">
          ...
          <Section id="zoom-1" title="2. Zoom - Call Companion" colour="teal">
            ...
            <ol className="space-y-1.5 list-decimal list-inside text-[#3A3A3A] text-sm mt-2">
              ...
              <li><strong>Tie the hot spot to how this works</strong> - the same five stages as performance.bodyrecode.au/how-it-works, in the same order: the Foundational Read and the eight signal domains, the two ways the read places her (readiness, then pattern), what gets built and the portal it lives in, the weekly read loop, and the Progress Read at the end of each block. Each one carries your script underneath it, and their readiness and pattern are highlighted on the stage 2 cards.</li>
            ...
            <Note><strong>Your scripts live in one file.</strong> Every word of spoken copy is in <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">src/lib/companion-content.ts</code>. Edit that file, not the page. <strong>How it works was rebuilt 25 Aug 2026</strong> to mirror the live how-it-works page word for word, because the lead may have read it before the call. The old anatomical-zone card is gone; the read now places her two ways, readiness first, then pattern. The spoken scripts carry no pattern count, since four is true of the model but only three are available to a woman.</Note>
          ...
          <Section id="client-portal" title="8. Client Portal" colour="teal">
            ...
            <p className="text-xs font-bold text-[#6B6B6B] uppercase tracking-wider mt-4 mb-2">Coach ↔ Client Messages</p>
            ...
            <p className="text-xs font-bold text-[#6B6B6B] uppercase tracking-wider mt-4 mb-2">Draft a Reply (2026-07-28)</p>
            ...
            <p className="text-xs font-bold text-[#6B6B6B] uppercase tracking-wider mt-4 mb-2">Anchored Messages (2026-07-28)</p>
          ...
          <Section id="email-sequences" title="15. Email Sequences and Automation" colour="teal">
            ...
            <p className="font-semibold text-[#1A1A1A] mt-4">The Body Decode daily arc (automatic, new 24 Aug 2026)</p>
            ...
            <p className="font-semibold text-[#1A1A1A] mt-4">The Body Decode feedback capture (24 Aug 2026)</p>
            <p>Two capture points inside The Body Decode, and they measure different things. <strong>Accuracy</strong> is captured directly under her read the moment she first sees it, on a 1-5 scale: did the diagnosis actually land? <strong>Recommendation</strong> is captured at the end of day 5 on a 0-10 scale, after the Blueprint card rather than before it. Both land in <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">/dashboard/feedback</code> under moments <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">decode_read</code> and <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">decode_day5</code>. Read the low accuracy scores first: a read that misses tells you more than one that lands, and a high completion rate on a read that does not sound like her is a worse result than a low one. Consent emails are not automatic, so nothing is ever published without you sending the request yourself. Pull the whole cohort with <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">npx tsx --env-file=.env.local scripts/decode-cohort.ts --since 2026-09-01</code>.</p>
            ...
            <p className="font-semibold text-[#1A1A1A] mt-4">Dormant Lead Reactivation (you trigger it)</p>
            ...
            <Note><strong>Run the dry run before you send.</strong> <code className="text-blue-500 text-xs bg-[#E5E5E5] px-1 py-0.5 rounded">GET /api/admin/dormant-reactivation</code> shows exactly who would receive what, and who is excluded and why, without sending anything. Sending needs a POST with <code className="text-blue-500 text-xs bg-[#E5E5E5] px-1 py-0.5 rounded">?confirm=1</code>. Of the original 84: 8 were internal or test records including both of your own addresses, 13 had no body state so there was no read to send them, and 3 were inactive. 60 were sendable.</Note>

            <p className="font-semibold text-[#1A1A1A] mt-4">SMS send timing</p>
            ...
            <p className="font-semibold text-[#1A1A1A] mt-4">Duplicate workflow protection</p>
            ...
            <ul className="space-y-1 list-disc list-inside text-[#d4cfc9] text-sm">
              ...
              <li><strong>Session counts are grounded like meal counts (2026-08-17).</strong> Any session count in any field must match <code className="bg-stone-100 px-1 rounded text-[#1B6DFC] text-xs">programs.training_frequency</code> or what the client actually logged this block week. A mismatch triggers the same auto-retry as a meal-count mismatch. Sets, reps, loads, RPE values and exercise names stay banned — the anchor can say &quot;hold all three sessions&quot;, never &quot;drop the squat to 3 sets of 8&quot;.</li>
            </ul>
          ...
          <Section id="trajectory-reading" title="20c. Block-End Trajectory Reading - Client Facing" colour="teal">
            ...
            <p className="text-xs font-bold text-[#6B6B6B] uppercase tracking-wider mt-6 mb-2">Progress Read - re-scoring body state at block-end (2026-08-11)</p>
          ...
          <Section id="be-strategy" title="34. Strategy Hub" colour="amber">
            ...
            <p className="text-xs mt-2 text-[#3A3A3A]"><strong>Stories publish only when the row is marked story_auto.</strong> Meta&apos;s API strips link stickers, countdown stickers and polls, and the tap on a sticker is the completion signal that recovers story ranking - so a story carrying one still goes up by hand. <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">story_auto</code> marks the plain-image ones that were never going to have a sticker, and those publish themselves like any other post. Two differences from a feed post: a story has no caption, so all the text has to be in the image, and Meta will not accept a scheduled publish time for stories, so our own cron fires them at the moment they should be live rather than handing them over in advance. Everything else on the calendar is tracked the same way.</p>
            <p className="text-xs font-bold text-[#6B6B6B] uppercase tracking-wider mt-4 mb-2">Two accounts, from 2026-08-20</p>
            <p className="text-xs mt-2 text-[#3A3A3A]"><strong>@body_recode_ and @kade_dunstone_ both publish natively.</strong> The personal brand was the only channel with no automation, so it died whenever Kade got busy — five days quiet in July, then <strong>twelve days from 8 Aug with 73 finished posts sitting behind it</strong>, every one with a caption and a graphic. The morning cadence alert fired for fifteen straight days, so another alert was never the fix.</p>
            <p className="text-xs mt-2 text-[#3A3A3A]"><strong>Each account has its own credential pair and there is NO fallback.</strong> If the personal vars are missing, a personal post refuses to publish and says so; it never drops back to the Body Recode token. That prevents Kade&apos;s personal posts appearing on @body_recode_ in front of the client audience, which has no undo. The code can only check the vars <em>exist</em> though — it cannot tell that the wrong Page&apos;s token was pasted in, which is why the setup guide makes you verify <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">GET /&lt;id&gt;?fields=username</code> returns <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">kade_dunstone_</code> before going further.</p>
            <p className="text-xs mt-2 text-[#3A3A3A]"><strong>The BR footer is skipped on personal posts.</strong> It reads &quot;More from our founder → @kade_dunstone_&quot;, which is right on the brand account and nonsense on the personal one.</p>
            <StatusList items={[
            ...
            <p className="text-xs font-bold text-[#6B6B6B] uppercase tracking-wider mt-4 mb-2">Measurement · the missing permission</p>
            <p className="text-xs mt-2 text-[#3A3A3A]"><strong>Neither token can read insights.</strong> Both were generated with four permissions and <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">instagram_manage_insights</code> is not among them, so reach, saves and shares are unreadable on <strong>every post either account has ever published</strong> — 44 on @body_recode_ and counting. Publishing is entirely unaffected; this is a read permission only.</p>
            <p className="text-xs mt-2 text-[#3A3A3A]">Fix: regenerate both tokens with that permission added and update the env vars. Then <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">npx tsx --env-file=.env.local scripts/ig-insights.ts</code> reports reach, saves, shares and likes per brand and per format, with top and bottom performers. <strong>Until then every content decision on both accounts is taste, not evidence.</strong></p>
            <p className="text-xs mt-2 text-[#3A3A3A]">Token + IG Business Account ID configured one-time via env vars (Vercel body-recode project only). Token is a long-lived User Access Token with the 4 perms: <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">instagram_basic</code>, <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">instagram_content_publish</code>, <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">pages_show_list</code>, <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">pages_read_engagement</code>. Regenerate via Graph API Explorer every ~60 days. Full setup walkthrough in <code className="bg-[#E5E5E5] px-1 rounded text-blue-700 text-xs">~/Dropbox/01_BODY_RECODE/06_SAAS_PLATFORM_BUILD/2026-06-30_Instagram_Native_Publishing.md</code>.</p>
          ...
          <Section id="ch-automation" title="Automation Sequence" colour="teal">
            ...
            <p className="text-xs font-bold text-[#6B6B6B] uppercase tracking-wider mt-4 mb-2">challengeCheckinPromptFunction - Body Decode Check-In prompt (Inngest)</p>
