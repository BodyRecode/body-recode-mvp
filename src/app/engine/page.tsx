...
export const metadata: Metadata = {
  title: 'Inside the engine | Body Recode™',
  description:
    'The technical map of the Body Recode™ interpretation engine. What it reads, how it reads it, what it produces, and the point at which it stops.',
...
const STAGES = [
  ...
  { n: '01', label: 'CFFS, the core interpretation', tag: 'where it ends up' },
  { n: '02', label: 'CFWS, the weekly re-read', tag: 'kept current' },
  { n: '03', label: 'Signal monitoring', tag: 'self-governance' },
]
...
const INPUTS = [
  ...
  },
  {
    ...
    model: CLINICAL,
    desc: 'Client uploads a lab PDF or photo. The engine transcribes every marker against the range the lab itself printed. Transcription, not diagnosis. Only a panel the coach has approved reaches the interpretation.',
    store: 'blood_panels.*',
  },
]
...
const INFRA = [
  { k: 'Model', v: `Tiered by consequence, never chosen at the call site. ${CLINICAL} for anything whose output reaches a client's body, which is the read and the lab extraction. ${OPERATIONAL} for drafting, routing and extraction that is cheap to retry. One multimodal call: photos and lab files run alongside the text.` },
  ...
  { k: 'Governed, not assumed', v: 'Drift and reassessment are surfaced, never silent. The read is watched across the whole block.' },
]
  ...
  return (
    <div style={{ background: CANVAS, minHeight: '100vh', color: TXT_BODY, overflowX: 'hidden' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(8,9,11,0.72)', backdropFilter: 'blur(12px)', WebkitBackdropFilter: 'blur(12px)', borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 24px', height: 72, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: 22, alignItems: 'center', flexWrap: 'wrap' }}>
            <a href="/#engine" style={navLink}>Diagram</a>
            <a href="#stage-01" style={navLink}>CFFS</a>
            <a href="#stage-02" style={navLink}>CFWS</a>
            <a href="#line" style={navLink}>The line</a>
      ...
      <section style={{ padding: '110px 24px 80px', borderBottom: `1px solid ${BORDER}`, background: `radial-gradient(900px 420px at 50% -10%, rgba(27,109,252,0.10), transparent)` }}>
        <div style={{ maxWidth: 880, margin: '0 auto' }}>
          ...
          <Prose style={{ maxWidth: 660, fontSize: 17 }}>
            The homepage shows the engine as a diagram. This is the version with the working parts showing. Structured
            answers go in, a single cautious interpretation comes out as the CFFS, and it is re-read every week as the
            CFWS. It prescribes nothing at any point. Where it stops is set out at the bottom of this page, and it is
      ...
      <section style={{ padding: '64px 24px', borderBottom: `1px solid ${BORDER}`, background: CANVAS_2 }}>
        <div style={{ maxWidth: 1040, margin: '0 auto' }}>
          ...
          <p style={{ fontFamily: MONO, fontSize: 11, color: TXT_MUTE, lineHeight: 1.7, marginTop: 20, letterSpacing: '0.02em' }}>
            This is the engine on its own. Whatever gets built on top of a read is a much larger piece of software than
            the read itself, and that is the point: the hard, slow, defensible part is small.
          </p>
        </div>
      </section>
      ...
      <Section id="overview">
        ...
        <SectionHeading>Four stages, then it hands over.</SectionHeading>
        <Prose style={{ marginBottom: 44 }}>
          The questions feed the CFFS. The CFFS is re-read every week as the CFWS. A monitor with no model in it
          watches for drift and says when the read has gone stale. That is the entire engine. It hands the read to
          whoever is going to act on it and takes no part in what they do next.
        </Prose>
      ...
      <Section id="stage-00" wide>
        <StageHead n="00" title="What goes in" sub="Four inputs. Almost all of it is plain capture with no model involved; the only model at this stage transcribes blood panels. The forms belong to whoever is collecting the data. The question set and what each answer means belong to the engine." />
        <Grid min={250} gap={12}>
          {INPUTS.map((it) => (
        ...
        <Callout>
          The read is triggered by hand, and only once <strong style={{ color: TXT_BODY }}>both</strong> the questions and the
          baseline photos are in, because the Fat Map needs the photos. The engine starts at the intake. Anything earlier,
          including any short quiz used to bring people in, sits outside it and never feeds it.
        </Callout>
      </Section>
      ...
      <section id="stage-01" style={{ padding: '100px 24px', background: CANVAS_2, borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ maxWidth: 1040, margin: '0 auto' }}>
          <StageHead n="01" title="The CFFS" sub="Coach-Facing Foundational Synthesis, which is the written read. This is the heart of it, and it is where the interpreting stops." accent />
          ...
          <Grid min={230} gap={12} style={{ marginTop: 16 }}>
            ...
            <MiniCard title="One live read" body="Generating archives the previous CFFS. reassessment_flagged is always set by the server, never by the model, because it is a fact about time rather than a judgement." />
          </Grid>
        </div>
      </section>
      ...
      <Section id="stage-02" wide>
        <StageHead n="02" title="The CFWS" sub="Coach-Facing Weekly Synthesis, the weekly re-read. Still interpretation, still written for whoever acts on it, still prescribes nothing." />
        <Grid min={260} gap={20}>
          <div>
            <ColLabel>Reads</ColLabel>
            <p style={{ fontSize: 13, color: TXT_DIM, lineHeight: 1.8 }}>
              The week's check-in, measured against the original read rather than against nothing. The form that collects
              it sits outside the engine. The engine reads the answers the same way it reads the intake.
            </p>
          </div>
          <div>
            <ColLabel>Rule</ColLabel>
            <p style={{ fontSize: 13, color: TXT_DIM, lineHeight: 1.8 }}>
              Readiness moves <strong style={{ color: TXT_BODY }}>one notch at a time</strong> from where the original read
              put it. Red is the floor. It only jumps two notches on a named safety event, and no single answer on one form
              can drag it down by itself.
            </p>
          </div>
          <div>
            <ColLabel>Produces</ColLabel>
            <p style={{ fontSize: 13, color: TXT_DIM, lineHeight: 1.8 }}>
              A weekly write-up plus the four readiness colours, stored in <span style={{ fontFamily: MONO, color: BLUE_LIGHT }}>cfws.*</span>.
              Weeks of those in sequence are what the end-of-block review reads.
            </p>
          </div>
        </Grid>
      ...
      <section id="stage-03" style={{ padding: '100px 24px', background: CANVAS_2, borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ maxWidth: 1040, margin: '0 auto' }}>
          <StageHead n="03" title="Signal monitoring" sub="How the engine polices itself. It decides when a read has drifted far enough to need doing again. No model involved, and it prescribes nothing." />
          <Grid min={230} gap={12}>
            ...
            <MiniCard title="Effort creep" body="Logged effort running a full point above what was prescribed counts as creep. Two points, or any set at 9.5 or above, counts as severe. The logging happens outside the engine. The monitor reads it as one more sign of drift." />
          </Grid>
          <Callout>
            This is the last thing the engine does. The monitor's recommendation loops back round to a fresh read. It never
            reaches forward into what anyone does about it.
          </Callout>
        </div>
      </section>

      {/* THE HANDOFF */}
      <Section id="handoff" wide>
        <SectionLabel>The Handoff</SectionLabel>
        <SectionHeading>Where the engine stops, and what happens next.</SectionHeading>
        <Prose style={{ marginBottom: 32 }}>
          The engine finishes at the read and hands it over. What gets built on top of it, the training, the food, the
          workload, the clinical decisions, is not the engine's business and is not part of what it does. That is
          deliberate, and it is the reason the read can be handed to anyone.
        </Prose>
        <Callout>
          The traffic runs one way. A plan that wants more load, or a person who wants a faster result, never changes the
          read. If the read says the body cannot take what somebody wants to do with it, the read stands and the plan
          moves. That asymmetry is the whole agreement between what the engine says and what anyone does about it.
        </Callout>
      </Section>
      ...
      <Section wide>
        ...
        <Prose style={{ marginBottom: 40 }}>
          ...
          client language. The Fat Map reads at two resolutions, and location alone does not separate the four, because three of
          ...
          long arc, which means months, not weeks.
        </Prose>
      ...
      <section style={{ padding: '100px 24px', background: CANVAS_2, borderTop: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>
        <div style={{ maxWidth: 1040, margin: '0 auto' }}>
          ...
          <Prose style={{ marginBottom: 40 }}>
            The same discipline runs under every stage: the same model strategy, the same JSON handling, the same
      ...
      <Section wide>
        ...
        <Grid min={300} gap={12} style={{ marginTop: 32 }}>
          <MiniCard title="It reads, it never prescribes" body="The engine finishes at the read. Whatever gets prescribed is decided by a person afterwards, built from the read and never the other way round." />
      ...
      <footer style={{ padding: '64px 24px', textAlign: 'center' }}>
        <p style={{ fontFamily: MONO, fontSize: 11, letterSpacing: '0.18em', color: TXT_MUTE, textTransform: 'uppercase', marginBottom: 14 }}>{brand().name}™ · The Interpretation Engine</p>
        <div style={{ display: 'flex', gap: 20, justifyContent: 'center', flexWrap: 'wrap' }}>
          <a href="/" style={navLink}>Home</a>
          <a href="/#engine" style={navLink}>The diagram</a>
          <a href="/#enquire" style={navLink}>Enquire</a>
        </div>
      </footer>
    </div>
  )
