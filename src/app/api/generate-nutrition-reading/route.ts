...
export async function POST(request: NextRequest) {
  ...
  const gateContext = await loadGateContext(admin, plan.client_id)
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (gateViolations.length > 0) {
      ...
      console.warn('[nutrition-reading] safety gate breach:', gateViolations.map(v => v.code).join(', '))
    }
    ...
    return NextResponse.json(
      {
        error: gatesSeen.length > 0
          ...
          : `Nutrition reading leaked internal terminology after 3 attempts (${leaksSeen.length ? leaksSeen.join(', ') : lastError ?? 'unknown'}). Click Regenerate to try a fresh start.`,
      },
      { status: 500 }
    )
  }
