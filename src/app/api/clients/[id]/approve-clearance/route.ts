...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (client?.email && process.env.RESEND_API_KEY) {
    ...
    await resend.emails.send({
      ...
      <p style="font-size:15px;color:#4A4A4A;line-height:1.9;margin:0 0 20px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">Take your time with the intake. The more accurate it is, the better the read I get.</p>
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 28px;">
        <tr>
          <td bgcolor="#1B6DFC" style="background-color:#1B6DFC;border-radius:8px;">
            <a href="${portalUrl}" style="display:inline-block;padding:14px 28px;color:#FFFFFF;font-size:14px;font-weight:700;text-decoration:none;letter-spacing:0.02em;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">Go to my portal</a>
          </td>
        </tr>
      </table>
