...
-- 208-question form completed once at onboarding) from supplementary intakes
...
COMMENT ON COLUMN intake_invitations.kind IS
  '`foundational` = the full 208-question intake at onboarding. `supplementary` = a short follow-up form that asks only fields added after the original intake. Supplementary submissions UPDATE the most recent intake row rather than inserting a new one.';
