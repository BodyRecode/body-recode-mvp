...
- **Doctrine: (A) now, (B) later.** (A) partners run BR's doctrine, branded as theirs, with their own prescriptions on top. (B) each partner's method injected into the engine, reserved for after the first ten.
...
- One-page founding-partner agreement + IP licence (needs legal/Ange). **S**
